...
static char ac_option_hint[  ][ LEN_HINT ] = {                                         //操作提示数组
    ...
    //"编码质量(qp) [0, 100]",
    "编码模式 0:cbr , 1:vbr, 2:CONST_QP, 4:AVBR (2,4 only av100 and av130 support)",
    "目标码率, 单位:kbps, [大于 0]",
...
static struct option option_long[ ] = {
    ...
    /*"[NUM]		save file flag, 0 not save, otherwise save, default 1",*/
    { "save_file_flag"  , required_argument , NULL , 'f' } , 
    { 0                   , 0                 , 0    , 0   } ,
};
...
static void *video_encode_from_vi_th(void *arg)
{
    ...
    if (save_file_flag)
    {
        ...
        if (H264_ENC_TYPE == venc_th->en_type)
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%dx%d.h264", 
            save_path, br_mode,target_kbps,chn_id,res.width, res.height);
        else
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%dx%d.h265", 
            save_path, br_mode,target_kbps,chn_id, res.width, res.height);
...
run_again:
    ...
    if (save_file_flag)
    {
        ...
        memset(venc_th->save_file, 0x00, sizeof(venc_th->save_file));
        if (H264_ENC_TYPE == venc_th->en_type)
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%ux%u.h264", 
                    save_path, br_mode,target_kbps,chn_index,res.width, res.height);
        else
            sprintf(venc_th->save_file, "%s/venc_br%d_%dkbps_chn%d_%ux%u.h265", 
                    save_path, br_mode,target_kbps,chn_index, res.width, res.height);
...
static int start_venc(int dev_id)
{
    ...
    if (0 == input_mode)
    {
        ...
        ak_thread_create(&venc_stream_th[dev_id], video_encode_from_vi_th, 
                         &enc_pair[dev_id], ANYKA_THREAD_MIN_STACK_SIZE, THREAD_PRIO);
#endif
    }
    else
    {
        ak_thread_create(&venc_stream_th[dev_id], video_encode_from_sdcard_th, 
                        &enc_pair[dev_id], ANYKA_THREAD_MIN_STACK_SIZE, THREAD_PRIO);
    }
...
#endif
{
    ...
    /* check the br mode */
#if defined(__CHIP_AK3918AV100_SERIES) || defined(__CHIP_AK3918EV300L_SERIES) || defined(__CHIP_AK3918AV130_SERIES)
    /* check the br mode */
    ...
    if(encoder_type == HEVC_ENC_TYPE)
    {
        ak_print_error_ex(MODULE_ID_APP, "not support encode type:h265\n");
        return AK_FAILED;
    }
#endif
