...
/**
 * init_aenc - 注册编码的回调函数
 * notes: 实际使用时，如果是MP3、AAC、AMR格式的编码，需要进行注册，
 *        只需要注册使用的类型即可，不需要所有类型都注册，G711A/U类型不需要注册
 */
static int init_aenc(int enc_type)
{
    aenc_encode_s stencode = {0};
    stencode.enType = enc_type;
    switch (enc_type) {
...
static int dvr_init_aenc( struct dvr_conf *p_dvr_conf )
{
    ...
    if (init_aenc(aenc_param_in.type) != AK_SUCCESS ) {
        ak_print_error_ex(MODULE_ID_APP, "init_aenc failed\n" );
        return AK_FAILED;
    }
