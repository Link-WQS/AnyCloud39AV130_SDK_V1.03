...
static void usage(const char * name)
{
    ak_print_normal(MODULE_ID_APP, "Usage: please insmod /usr/modules/ak_efuse.ko first, then %s.\n"
        "cat /sys/kernel/efuse/efuse_OR to compare the value. if lock status is 1, cannot set custom id.\n", name);
 ...
 */
#ifdef AK_RTOS
static int uuid_ext_sample(int argc, char **argv)
#else
...
#endif
{
    ...
    ret = ak_uuid_get_custom_id_length(&length);                                    //获取分区长度
    if(ret < 0){
        ak_print_error_ex(MODULE_ID_APP,"ak_uuid_get_custom_id_length failed\n");
    }
    ak_print_notice_ex(MODULE_ID_APP,"custom  length = %d\n",length);

    unsigned char* custome_ptr = NULL;
    ret = ak_uuid_get_custom_id(&custome_ptr);                                  //获取分区数据
    if(ret < 0)
        ak_print_error_ex(MODULE_ID_APP,"ak_uuid_get_custom_id failed\n");
    ak_print_notice_ex(MODULE_ID_APP," custom_id=");
    for(int k = 0;k< length;k++)
    {
        printf("\033[1;33m %x\033[m", custome_ptr[k]);
    }
    printf("\n");
    unsigned char* buf =malloc(length);                                     //根据长度分配空间
    memset(buf, 0, length);
    memcpy(buf, "Custom data", strlen("Custom data"));

    ret = ak_uuid_set_custom_id(buf,length);                                //设置custom分区
    if(ret > 0)
    {
        ak_print_error_ex(MODULE_ID_APP," set_custom_data failed\n");
    }
    unsigned char* p = NULL;
    ak_uuid_get_custom_id(&p);                                             //获取设置后的分区的数据

    ak_print_notice_ex(MODULE_ID_APP," custom_id=");
    for(int k = 0;k<length;k++)
    {
        printf("\033[1;33m %x\033[m", p[k]);
    }
    printf("\n");
    ak_uuid_close();
exit:
...
#ifdef AK_RTOS
SHELL_CMD_EXPORT_ALIAS(uuid_ext_sample, ak_uuid_ext_sample, UUID Sample);
#endif
