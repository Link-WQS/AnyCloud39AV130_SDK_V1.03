...
/**
 * init_adec - 注册解码的回调函数，打开解码
 * notes: 实际使用时，如果是MP3、AAC、AMR格式的解码，需要进行注册，
 *        只需要注册使用的类型即可，不需要所有类型都注册，G711A/U类型不需要注册
 */
static int init_adec(void)
{
    ...
    adec_decode_s stdecode = {0};

    switch (dec_type) {
        case AK_AUDIO_TYPE_MP3:
            // register mp3 decoder
            stdecode.pfnDecOpen = ADEC_OpenMp3;
            stdecode.pfnDecVersion = ADEC_Mp3_GetVersionInfo;
            stdecode.enType = dec_type;
            if (ak_adec_registerdecoder(&stdecode)) {
                ak_print_error_ex(MODULE_ID_APP, "ak_adec_registerdecoder failed\n");
                return AK_FAILED;
            }
            break;
        case AK_AUDIO_TYPE_AAC:
            // register aac decoder
            stdecode.pfnDecOpen = ADEC_OpenAac;
            stdecode.pfnDecVersion = ADEC_Aac_GetVersionInfo;
            stdecode.enType = dec_type;
            if (ak_adec_registerdecoder(&stdecode)) {
                ak_print_error_ex(MODULE_ID_APP, "ak_adec_registerdecoder failed\n");
                return AK_FAILED;
        ...
        case AK_AUDIO_TYPE_AMR:
            ...
            stdecode.pfnDecOpen = ADEC_OpenAmr;
            stdecode.pfnDecVersion = ADEC_Amr_GetVersionInfo;
            stdecode.enType = dec_type;
            if (ak_adec_registerdecoder(&stdecode)) {
                ak_print_error_ex(MODULE_ID_APP, "ak_adec_registerdecoder failed\n");
                return AK_FAILED;
...
static void* get_frame_then_play(void *arg)
{
    ...
    int play_len = 0;
    struct ak_dev_buf_status buf_status = {0};
    int flush_time = 0;
    struct ak_audio_data_attr pcm_attr = {0};
    pcm_attr.sample_rate = sample_rate;
    pcm_attr.sample_bits = AK_AUDIO_SMPLE_BIT_16;
    pcm_attr.channel_num = channel_num;
    ...
    /*wait for finish*/
    if (ak_ao_get_buf_status(ao_handle_id, &buf_status)) {
        ak_print_error_ex(MODULE_ID_APP,"ak_ao_get_buf_status failed \n");
        return NULL;
    }
    /*get flush time to play remain data*/
    flush_time =  ak_audio_len_to_interval(&pcm_attr, buf_status.buf_remain_len);
    if(ak_ao_flush(ao_handle_id))
    {
        ak_print_error_ex(MODULE_ID_APP,"ak_ao_flush failed \n");
    }
    /*play remain data*/
    ak_sleep_ms(flush_time);
...
#endif
{
    ...
    ret = ak_thread_create(&(send_stream_tid), read_play_file,
