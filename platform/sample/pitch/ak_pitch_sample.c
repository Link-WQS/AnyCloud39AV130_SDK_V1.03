...
static int pitch_tempo = 7; // pitchTempo

static int vqe_pitch_handle_id = -1; // pitch handle
static FILE *pitch_file    = NULL;
...
static struct option option_long[] = {
    ...
    { "pitch_tempo", required_argument , NULL , 'i' } , //"[PITCH_TEMPO] 变声参数选择" ,
    {0, 0, 0, 0}
};
...
static int help_hint(char *pc_prog_name)
{
    ...
    printf( "\nexample: \r\n"
    "  %s -t 60 -s 8000 -c 1 -v 5 -d 0 -i 7 -p /mnt/ \n", pc_prog_name);
    printf("\n\n");
    return 0;
}
...
static int create_pcm_file(const char *path)
{
    ...
    sprintf(file_path, "%s/pitch.pcm", path);

    /* open Pitch file */
    pitch_file = fopen(file_path, "wb");
    if (NULL == pitch_file) {
        ak_print_error_ex(MODULE_ID_APP, "open Pitch pcm file: %s error\n", file_path);
        ret = AK_FAILED;
        goto create_pcm_file_end;
    } else {
        ak_print_normal_ex(MODULE_ID_APP, "open Pitch pcm file: %s OK\n", file_path);
    }
...
static int init_vqe_pitch(void)
{
    ...
    /* pitch_tempo 参数的范围是[0-10], 0~4提高音调，5正常音调，6~10降低音调 */
    /* 童声是2，男声是6或7 */
    vqe_pitch_attr.pitchTempo = pitch_tempo;
    // print pitch_tempo
...
static void* vqe_loop(void *arg)
{
    ...
    while (1) {
        ...
        if(ret != AK_SUCCESS){
            ak_print_error_ex(MODULE_ID_APP, "ERROR: pitch process: ret = %d\n", ret);
            goto VqeLoopEnd;
        }else{
            if(fwrite(pitch_data, 1, pitch_len, pitch_file) < 0){
                ak_print_error_ex(MODULE_ID_APP, "ERROR: fwrite pitch_file\n");
                break;
            }
        }
