...
#define PRINT(fmt, ...) printf("[UVC_DEV] " fmt, ##__VA_ARGS__)
...
static int uvc_video_set_format(struct uvc_device *dev)
{
    ...
    if (ret < 0) {
        PRINT("UVC: Unable to set format %s (%d).\n", strerror(errno), errno);
        return ret;
    }

    PRINT("UVC: Setting format to: %c%c%c%c %ux%u\n",
            (dev->fcc) & 0xff,
            ((dev->fcc)>>8) & 0xff,
            ((dev->fcc)>>16) & 0xff,
            ((dev->fcc)>>24) & 0xff,
...
static int uvc_video_stream(struct uvc_device* dev, int enable)
{
    ...
    if (!enable) {
        ...
        if (ret < 0) {
            PRINT("UVC: VIDIOC_STREAMOFF failed: %s (%d).\n", strerror(errno),
                errno);
            return ret;
        }

        PRINT("UVC: Stopping video stream.\n");
    ...
    if (ret < 0) {
        PRINT("UVC: Unable to start streaming %s (%d).\n", strerror(errno),
            errno);
        return ret;
    }
    PRINT("%s --> ioctl to VIDIOC_STREAMON\n", __func__);

    PRINT("UVC: Starting video stream.\n");
...
int uvc_open(struct uvc_device** uvc, char* devname)
{
    ...
    if (fd == -1) {
        PRINT("UVC: device open %s failed: %s (%d).\n", devname, strerror(errno), errno);
        return ret;
    }

    ret = ioctl(fd, VIDIOC_QUERYCAP, &cap);
    if (ret < 0) {
        PRINT("UVC: unable to query uvc device: %s (%d)\n", strerror(errno),
            errno);
        goto err;
    }

    if (!(cap.capabilities & V4L2_CAP_VIDEO_OUTPUT)) {
        PRINT("UVC: %s is no video output device\n", devname);
        goto err;
    }
    ...
    PRINT("uvc device is %s on bus %s\n", cap.card, cap.bus_info);
    PRINT("uvc open succeeded, file descriptor = %d\n", fd);
        ...
        unsigned char *data,unsigned int len)
{
    ...
    /*
    if(dev->video_empty_stream(dev->user_dev)){
        return 0;
    }*/
    CLEAR(ubuf);
    ...
    if (dev->run_standalone) {
        ...
        if (ret < 0) {
            PRINT("VIDIOC_DQBUF failed index=%d\n", ubuf.index);
            return ret;
        }
        if(len>dev->mem[ubuf.index].length)
        {
            PRINT("stream len = %u > mem len=%u\n",
                    len,dev->mem[ubuf.index].length);
        }
        //ubuf.bytesused = dev->video_fill_stream(dev->user_dev,dev->mem[ubuf.index].start,ubuf.index);
        ubuf.bytesused = len;
        memcpy(dev->mem[ubuf.index].start,data,len);
        dev->dqbuf_count++;
        ret = ioctl(dev->uvc_fd, VIDIOC_QBUF, &ubuf);
        if (ret < 0) {
            PRINT("VIDIOC_QBUF failed index=%d\n", ubuf.index);
            return ret;
        }

        dev->qbuf_count++;
    } else {
        PRINT("dev->run_standalone=%d\n", dev->run_standalone);
    }
...
static int uvc_video_qbuf_mmap(struct uvc_device* dev)
{
    ...
    for (i = 0; i < dev->nbufs; ++i) {
        PRINT("%s --> i=%u, start=%p, buf=%p\n", __func__, i, dev->mem[i].start,
            &dev->mem[i].buf);
        ...
        dev->mem[i].buf.bytesused = 0;

        /* UVC standalone setup. */
        if (dev->run_standalone) {
          //dev->mem[i].buf.bytesused = dev->video_fill_stream(dev->user_dev,dev->mem[i].start,i);
        }
...
static int uvc_video_qbuf_userptr(struct uvc_device* dev)
{
    ...
    if (dev->run_standalone) {
        for (i = 0; i < dev->nbufs; ++i) {
            ...
            if (ret < 0) {
                PRINT("UVC: VIDIOC_QBUF failed : %s (%d).\n", strerror(errno),
                    errno);
                return ret;
            }
...
static int uvc_video_reqbufs_mmap(struct uvc_device* dev, int nbufs)
{
    ...
    if (ret < 0) {
        if (ret == -EINVAL)
            PRINT("UVC: does not support memory mapping\n");
        else
            PRINT("UVC: Unable to allocate buffers: %s (%d).\n",
                strerror(errno), errno);
        goto err;
    }
    ...
    if (rb.count < 2) {
        PRINT("UVC: Insufficient buffer memory.\n");
        ret = -EINVAL;
        goto err;
    }
    ...
    for (i = 0; i < rb.count; ++i) {
        ...
        if (ret < 0) {
            PRINT("UVC: VIDIOC_QUERYBUF failed for buf %u: "
                  "%s (%d).\n",
        ...
        dev->mem[i].start = mmap(NULL /* start anywhere */,
            dev->mem[i].buf.length, PROT_READ | PROT_WRITE /* required */,
            MAP_SHARED /* recommended */, dev->uvc_fd,
            dev->mem[i].buf.m.offset);

        if (MAP_FAILED == dev->mem[i].start) {
            PRINT("UVC: Unable to map buffer %u: %s (%d).\n", i,
                strerror(errno), errno);
            dev->mem[i].length = 0;
            ret                = -EINVAL;
            goto err_free;
        ...
        PRINT("UVC: Buffer %u mapped at address %p, length=%d.\n", i,
            dev->mem[i].start, dev->mem[i].length);
    }

    dev->nbufs = rb.count;
    PRINT("UVC: %s --> %u buffers allocated-1.\n", __func__, rb.count);
...
static int uvc_video_reqbufs_userptr(struct uvc_device* dev, int nbufs)
{
    ...
    if (ret < 0) {
        if (ret == -EINVAL)
            PRINT("UVC: does not support user pointer i/o\n");
        else
            PRINT(
                "UVC: VIDIOC_REQBUFS error %s (%d).\n", strerror(errno), errno);
        goto err;
    }
    ...
    dev->nbufs = rb.count;
    PRINT("UVC: %s --> %u buffers allocated-2.\n", __func__, rb.count);
        ...
        for (i = 0; i < rb.count; ++i) {
            ...
            if (V4L2_PIX_FMT_YUYV == dev->fcc)
                for (j = 0; j < dev->height; ++j)
                    memset(
                        dev->dummy_buf[i].start + j * bpl, dev->color++, bpl);
...
static int uvc_handle_streamon_event(struct uvc_device* dev)
{
    ...
    PRINT("%s --> uvc_video_reqbufs\n", __func__);
    ret = uvc_video_reqbufs(dev, dev->nbufs);
    if (ret < 0)
        goto err;
    PRINT("%s run_standalone=%d\n", __func__, dev->run_standalone);
    /* Common setup. */

    /* Queue buffers to UVC domain and start streaming. */
    PRINT("%s --> uvc_video_qbuf\n", __func__);
    ret = uvc_video_qbuf(dev);
    if (ret < 0)
        goto err;

    if (dev->run_standalone) {
        PRINT("%s --> uvc_video_stream\n", __func__);
    ...
    uint8_t cs, uint8_t entity_id, uint8_t len, struct uvc_request_data* resp)
{
    ...
    PRINT("control request (req 0x%02x cs 0x%02x entity_id 0x%02x resp length "
          "%d)\n",
        req, cs, entity_id, resp->length);
}/*end of func*/
    ...
    uint8_t cs, struct uvc_request_data* resp)
{
    ...
    PRINT("streaming request (req %02x cs %02x)\n", req, cs);
    ...
    switch (req) {
        ...
        case UVC_GET_CUR:
            ...
            //PRINT("GET CUR, ctrl->dwMaxVideoFrameSize=%d\n",
    ...
    if (req != UVC_SET_CUR)
        PRINT("streaming request "
                "(bFormatIndex=%d,bFrameIndex=%d,"
                "dwMaxVideoFrameSize=%d,dwMaxPayloadTransferSizef=%d)\n",
    ...
    struct usb_ctrlrequest* ctrl, struct uvc_request_data* resp)
{
    ...
    switch (ctrl->wIndex & 0xff) {
        ...
        case UVC_INTF_STREAMING:
            uvc_events_process_streaming(
                dev, ctrl->bRequest, ctrl->wValue >> 8, resp);
            break;
    ...
    uint8_t entity_id, struct uvc_request_data* data)
{
    ...
    PRINT("Control Request data phase (cs %02x entity %02x)\n", cs, entity_id);

    return 0;
}

static int uvc_events_process_data(
    struct uvc_device* dev, struct uvc_request_data* data)
{
    ...
    switch (dev->control) {
        case UVC_VS_PROBE_CONTROL:
            PRINT("setting probe control, length = %d\n", data->length);
            target = &dev->probe;
            break;

        case UVC_VS_COMMIT_CONTROL:
            PRINT("setting commit control, length = %d\n", data->length);
            target = &dev->commit;
            break;

        default:
            PRINT("setting unknown control, length = %d\n", data->length);
            ...
            if (*val > PU_BRIGHTNESS_MAX_VAL) {
                ...
                ret = uvc_events_process_control_data(
                    dev, UVC_PU_BRIGHTNESS_CONTROL, 2, data);
                if (ret < 0)
                    goto err;
    ...
    PRINT("bFormatIndex %d, bFrameIndex %d, "
    ...
    if (dev->control == UVC_VS_COMMIT_CONTROL) {
        ...
        if (dev->bulk) {
            PRINT("%s --> uvc_handle_streamon_event\n", __func__);
            uvc_handle_streamon_event(dev);
        }
    }
    if (dev->control == UVC_VS_PROBE_CONTROL) {
        PRINT("format->fcc = %c%c%c%c,frame->width = %u,frame->height = %u\n",
                format->fcc & 0xff,
                (format->fcc >> 8) & 0xff,
                (format->fcc >> 16) & 0xff,
                (format->fcc >> 24) & 0xff,
    ...
    if (dev->control == UVC_VS_COMMIT_CONTROL) {
        PRINT("dev->fcc = %c%c%c%c,dev->width = %u,dev->height = %u\n",
                dev->fcc & 0xff,
                (dev->fcc >> 8) & 0xff,
                (dev->fcc >> 16) & 0xff,
                (dev->fcc >> 24) & 0xff,
...
__u32 uvc_events_process(struct uvc_device* dev)
{
    ...
    if (ret != 0) {
        PRINT("VIDIOC_DQEVENT failed: %s (%d)\n", strerror(errno), errno);
        return 0;
    }
    ...
    switch (v4l2_event.type) {
        ...
        case UVC_EVENT_DATA:
            ...
            if (dev->bulk && (dev->control == UVC_VS_COMMIT_CONTROL)) {
                return UVC_EVENT_STREAMON;
            }
            break;

        case UVC_EVENT_STREAMON:
            if (!dev->bulk) {
                PRINT("%s --> UVC_EVENT_STREAMON\n", __func__);
                uvc_handle_streamon_event(dev);
            }
            break;
    ...
    return v4l2_event.type;
send_res:

    ret = ioctl(dev->uvc_fd, UVCIOC_SEND_RESPONSE, &resp);
    if (ret < 0) {
        PRINT("UVCIOC_S_EVENT failed: %s (%d)\n", strerror(errno), errno);
        return 0;
    }
    return  v4l2_event.type;
}
...
void uvc_events_init(struct uvc_device* dev)
{
    ...
    if (dev->bulk) {
        ...
        dev->probe.dwMaxPayloadTransferSize
            = dev->commit.dwMaxPayloadTransferSize = dev->width * dev->height;
    }
