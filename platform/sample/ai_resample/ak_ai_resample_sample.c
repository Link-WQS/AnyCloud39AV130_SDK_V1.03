...
static int open_pcm_file(int sample_rate, int channel_num, const char *path, FILE **fp, int save_time, int dev_sel)
{
    ...
    sprintf(file_path, "%s/%s_%d_%d_%d_%s.pcm", path, time_str, out_samplerate, channel_num, save_time, dev);
...
static int init_vqe_resample(void)
{
    ...
    if (pcm_attr_in.sample_rate == 8000)
    {
        pcm_attr_out.sample_rate = 16000;
    }
    else if(pcm_attr_in.sample_rate == 16000)
    {
        pcm_attr_out.sample_rate = 8000;
    } 
...
static void* ai_capture_loop(void *arg)
{
    ...
    while (1) {
        ...
        if (NULL != fp) {
            if(fwrite(resample_data, resample_len, 1, fp) != 1) {
                ak_print_error_ex(MODULE_ID_APP, "writing to file error.\n");
            }
        }
...
#endif
{
    ...
    {
        ak_print_error_ex(MODULE_ID_APP, "volume is invalid, should be [-90, 20]\n");
        goto main_end;
    }
