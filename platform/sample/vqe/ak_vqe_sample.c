...
static const char save_def_path[] = "/mnt/";
static FILE *fp = NULL;
static int sample_rate = 8000;
static int volume = 5;
static int save_time = 20000;                     // set save time(ms)
static char *save_path= (char *)save_def_path;   // set save path
...
static FILE *aslc_file   = NULL;
static FILE *eq_file     = NULL;
...
static struct option option_long[] = {
    ...
    { "sample-rate"    , required_argument , NULL , 's' } , //"[NUM]  采样率[8000 16000]",
...
static void param_init(void)
{
    ...
    save_path= (char *)save_def_path;    // set save path
...
/*
 * CreatePcmFile: create pcm file.
 * path[IN]: vqe file path
 * return: 0 is success, -1 is error.
 */
static int create_pcm_file(const char *path)
{
    int ret = AK_SUCCESS;

    char *file_path = (char *)ak_mem_alloc(MODULE_ID_APP, FILE_PATH_LEN);
    if(!file_path){
        ak_print_error_ex(MODULE_ID_APP, "ERROR: malloc file_path\n");
        ret = AK_FAILED;
        goto create_pcm_file_end;
    }

    /* Create Eq File */
    memset(file_path,0, FILE_PATH_LEN);
    sprintf(file_path, "%s/Eq.pcm", path);
    /* open Eq file */
    eq_file = fopen(file_path, "wb");
    if (NULL == eq_file) {
        ak_print_error_ex(MODULE_ID_APP, "open Eq pcm file: %s error\n", file_path);
        ret = AK_FAILED;
        goto create_pcm_file_end;
    } else {
        ak_print_normal_ex(MODULE_ID_APP, "open Eq pcm file: %s OK\n", file_path);
    }

    /* Create Res File */
    memset(file_path,0, FILE_PATH_LEN);
    sprintf(file_path, "%s/Resample.pcm", path);
    /* open Res file */
    res_file = fopen(file_path, "wb");
    if (NULL == res_file) {
        ak_print_error_ex(MODULE_ID_APP, "open Res pcm file: %s error\n", file_path);
        ret = AK_FAILED;
        goto create_pcm_file_end;
    } else {
        ak_print_normal_ex(MODULE_ID_APP, "open Res pcm file: %s OK\n", file_path);
    }

    /* Create Aslc File */
    memset(file_path,0, FILE_PATH_LEN);
    sprintf(file_path, "%s/Aslc.pcm", path);
    /* open Aslc file */
    aslc_file = fopen(file_path, "wb");
    if (NULL == aslc_file) {
        ak_print_error_ex(MODULE_ID_APP, "open Aslc pcm file: %s error\n", file_path);
        ret = AK_FAILED;
        goto create_pcm_file_end;
    } else {
        ak_print_normal_ex(MODULE_ID_APP, "open Aslc pcm file: %s OK\n", file_path);
    }

    ak_mem_free(file_path);
    file_path = NULL;
    return AK_SUCCESS;
...
static void* vqe_loop(void *arg)
{
    ...
    ak_print_normal(MODULE_ID_APP, "*** vqe start ***\n");
    ...
    while (1) {
        ...
        if(ret != AK_SUCCESS){
            ak_print_error_ex(MODULE_ID_APP, "ERROR: eq process: ret = %d\n", ret);
            goto VqeLoopEnd;
        }else{
            if(fwrite(eq_data, eq_len, 1, eq_file) != 1){
                ak_print_error_ex(MODULE_ID_APP, "ERROR: fwrite eq_file\n");
            }
        }

        ret = ak_vqe_process(vqe_res_handle_id, frame.data, frame.len, &res_data, &res_len);
        if(ret != AK_SUCCESS){
            ak_print_error_ex(MODULE_ID_APP, "ERROR: res process: ret = %d\n", ret);
            goto VqeLoopEnd;
        }else{
            if(fwrite(res_data, res_len, 1, res_file) != 1){
                ak_print_error_ex(MODULE_ID_APP, "ERROR: fwrite res_file\n");
            }
        }

        ret = ak_vqe_process(vqe_aslc_handle_id, frame.data, frame.len, &aslc_data, &aslc_len);
        if(ret != AK_SUCCESS){
            ak_print_error_ex(MODULE_ID_APP, "ERROR: aslc process: ret = %d\n", ret);
            goto VqeLoopEnd;
        }else{
            if(fwrite(aslc_data, aslc_len, 1, aslc_file) != 1){
                ak_print_error_ex(MODULE_ID_APP, "ERROR: fwrite aslc_file\n");
            }
        }
...
VqeLoopEnd:
    ...
    ak_print_normal_ex(MODULE_ID_APP, "*** vqe finish ***\n\n");
    ak_thread_exit();
    return NULL;
} //end of vqe_loop
...
#endif
{
    ...
    ak_ai_set_lib_print_level(ai_handle_id,  AUDIO_PRINT_LEVEL_DEFAULT | AUDIO_PRINT_LEVEL_PARAM );
    ...
    /* create thread loop */
    ret = ak_thread_create(&(vqe_loop_tid), vqe_loop, NULL, ANYKA_THREAD_MIN_STACK_SIZE, 19);
    if (ret) {
        goto vqe_end;
    }

    /* wait thread end */
    ak_thread_join(vqe_loop_tid);
...
main_end:
    ak_print_normal(MODULE_ID_APP, "******** exit vqe demo ********\n");
    ak_sdk_exit();
    return ret;
}
