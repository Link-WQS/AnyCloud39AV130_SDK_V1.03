...
typedef struct ak_adec_decode_s
{
    int enType;
    void *(*pfnDecOpen)(void *pvOpenArg, void *pstAttr); // value can be ADEC_OpenXXX
    int (*pfnDecProcess)(void *pvAdecHandle, void *pvProcArg); // no need to set
    int (*pfnDecClose)(void *pvAdecHandle); // no need to set                        
    int (*pfnDecReset)(void *pvAdecHandle); // no need to set                         
    int (*pfnDecBufferClear)(void *pvAdecHandle); // no need to set                 
    int (*pfnDecBufFill)(void *pvAdecHandle, void *pvFillArg); // no need to set    
    int (*pfnDecBufGetFreeLen)(void *pvAdecHandle); // no need to set                 
    int (*pfnDecGetInputDataSize)(void *pvAdecHandle); // no need to set              
    int (*pfnDecSetInbufMinLen)(void *pvAdecHandle, unsigned int u32Len); // no need to set
    int (*pfnDecSetBufMode)(void *pvAdecHandle, int s32BufMode); // no need to set
    const char *(*pfnDecVersion)(void); // value can be  ADEC_xxx_GetVersionInfo       
}adec_decode_s;
...
/**
 * ak_adec_set_lib_print_level - set codec lib print level
 ...
 * notes:
 */
int ak_adec_set_lib_print_level(int print_level);

/**
 * ak_adec_registerdecoder - decoder register
 * @pstDecode[IN]: decode callback function
 * return: 0 success, otherwise failed
 * notes:
 */
int ak_adec_registerdecoder(adec_decode_s * pstDecode);
