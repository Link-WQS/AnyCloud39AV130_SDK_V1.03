			  ...
			  bool beacon,bool notransmit,u8 *transmitbssid)
{
	...
	*/
	...
	if((elems->ssid_len == hw_priv->scan.ssids[0].length) && 
	(memcmp(elems->ssid,hw_priv->scan.ssids[0].ssid,elems->ssid_len) == 0)){
			ieee80211_atbm_scan_cancel(&local->hw);
			atbm_printk_err("scan channel[%d] ssid=%s \n",channel_center_freq(channel),hw_priv->scan.ssids[0].ssid);
	}
#endif
