...
//	int ret =0;
	...
	atbm_printk_always("[%s] set_beacon[%d]\n",sdata->name,sdata->u.ap.csa_mlme.state);
	if(sdata->u.ap.csa_mlme.state != IEEE80211_CSA_MLME_STATE_IDLE){
		return -EBUSY;
	}
#endif
	old = rtnl_dereference(sdata->u.ap.beacon);
...
#endif
#ifdef CONFIG_ATBM_AP_SUPPORT_CSA
void ieee80211_csa_beacon_update_after(struct ieee80211_sub_if_data *sdata)
{
			     ...
			     struct cfg80211_csa_settings *params)
{
	...
	if(local->csa_sdata != NULL){
		err = -EBUSY;
		atbm_printk_err("csa_sdata[%s] busy\n",local->csa_sdata->name);
		goto exit;
	}
	...
	if(csa_mlme->state != IEEE80211_CSA_MLME_STATE_IDLE){
		err = -EBUSY;
	...
	local->csa_sdata = sdata;

	/*
	*trigger work
	*/
	__set_bit(CSA_CFG880211_RUNNING, &local->csa);
	__set_bit(CSA_WAIT_START,&local->csa);
	
	csa_mlme->state = IEEE80211_CSA_MLME_STATE_DELAY;
	
	if(params->count > 20){
		csa_mlme->count = 20;
		ieee80211_queue_delayed_work(&local->hw, &local->csa_work, 
				msecs_to_jiffies((params->count - 20) * sdata->vif.bss_conf.beacon_int));
	}else {
