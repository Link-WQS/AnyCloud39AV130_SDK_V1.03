...
#endif
static void ieee80211_sta_chanswitch_notify(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_channel_state *chan_state = ieee80211_get_channel_state(sdata->local, sdata);
...
#endif
#endif
	atbm_printk_always("[%s]:notify ap channel switch[%d][%d]\n",sdata->name,
					channel_hw_value(chan_state->oper_channel),
					sdata->vif.bss_conf.channel_type);
}
static enum work_action ieee80211_sta_chanswitch_wk_start(struct ieee80211_work *wk)
{
	struct ieee80211_sub_if_data *sdata = wk->sdata;
	struct ieee80211_sta_csa_request *request = &wk->channel_switch.csa;
	struct ieee80211_channel_state *chan_state = ieee80211_get_channel_state(sdata->local, sdata);
	unsigned long timeout;
	
	if(request->ie.count <= 1){
		chan_state->oper_channel = wk->chan;
		return WORK_ACT_TIMEOUT;
	}
	
	timeout = request->mactime + msecs_to_jiffies(request->ie.count * sdata->vif.bss_conf.beacon_int);
...
static enum work_done_result ieee80211_sta_chanswitch_wk_done(struct ieee80211_work *wk,
				      struct sk_buff *skb)
{
	struct ieee80211_sub_if_data *sdata = wk->sdata;
	struct ieee80211_sta_csa_request *request = &wk->channel_switch.csa;
	...
	atbm_printk_always("[%s]:chanswitch_wk_done\n",sdata->name);
	...
	if(wk->type != IEEE80211_WORK_STA_CHANNEL_SWITCH){
		goto destroy;
	}
	...
	ieee80211_set_channel_type(sdata->local,sdata,request->chantype);
	ieee80211_bss_info_change_notify(sdata, BSS_CHANGED_HT_CHANNEL_TYPE);
	ieee80211_sta_chanswitch_notify(sdata);
destroy:
	mutex_unlock(&sdata->local->mtx);
	return WORK_DONE_DESTROY;
}

static int ieee80211_sta_chanswitch_wk(struct ieee80211_sub_if_data *sdata,
				struct ieee80211_sta_csa_request *request)
{
	...
	wk->type = IEEE80211_WORK_STA_CHANNEL_SWITCH;
	memcpy(wk->filter_bssid,ifmgd->associated->bssid,ETH_ALEN);
	memcpy(wk->filter_sa,   ifmgd->associated->bssid,ETH_ALEN);
	wk->start     = ieee80211_sta_chanswitch_wk_start;
	wk->done      = ieee80211_sta_chanswitch_wk_done;
	wk->sdata     = sdata;
	wk->chan      = request->chan;
	wk->chan_type = request->chantype;
	memcpy(&wk->channel_switch.csa,request,sizeof(struct ieee80211_sta_csa_request));
	
	atbm_printk_always("[%s]: chanswitch_wk [%d][%d][%d]\n",sdata->name,
			wk->channel_switch.csa.ie.mode,wk->channel_switch.csa.ie.new_ch_num,wk->channel_switch.csa.ie.count);
					  ...
					  u64 timestamp)
{
	...
	struct ieee80211_sta_csa_request csa;
	enum ieee80211_chan_mode  channel_mode = ieee80211_get_channel_mode(sdata->local,sdata);
	ASSERT_MGD_MTX(ifmgd);

	if (!ifmgd->associated)
		return;
	
	if(channel_mode != CHAN_MODE_UNDEFINED){
		atbm_printk_err("chanswitch mode err (%d)\n",channel_mode);
		return;
	}
	...
	ieee80211_sta_chanswitch_wk(sdata,&csa);
}
#endif
	...
	struct sk_buff* skb)
{
	...
	if (ifmgd->associated &&
		...
		ieee80211_reset_ap_probe(sdata);
#endif // CONFIG_ATBM_MAC80211_NO_USE
}
	...
	struct ieee80211_rx_status* rx_status)
{
	...
	if (bss_conf->cqm_rssi_thold &&
		...
		if (sig < thold && (last_event == 0 || sig < last_event - hyst)) {
...
#ifdef CONFIG_MAC80211_ATBM_VERBOSE_DEBUG
		if (net_ratelimit()) {
			atbm_printk_debug("%s: cancelling probereq poll due to a received beacon\n", sdata->name);
		}
#endif
		ifmgd->flags &= ~IEEE80211_STA_BEACON_POLL;
		mutex_lock(&local->iflist_mtx);
...
void ieee80211_mlme_notify_scan_completed(struct ieee80211_local* local)
{
	...
	mutex_unlock(&local->iflist_mtx);
#else  // CONFIG_ATBM_MAC80211_NO_USE
	mutex_lock(&local->iflist_mtx);
	list_for_each_entry(sdata, &local->interfaces, list)
	{
