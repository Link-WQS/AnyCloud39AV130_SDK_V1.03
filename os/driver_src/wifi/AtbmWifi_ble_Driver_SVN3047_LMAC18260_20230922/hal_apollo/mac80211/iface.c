...
#ifdef CONFIG_ATBM_AP_SUPPORT_CSA
static void ieee80211_notify_csa_state(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_csa_mlme *csa_mlme = &sdata->u.ap.csa_mlme;

	switch(csa_mlme->state){
	case IEEE80211_CSA_MLME_STATE_IDLE:
		break;
	case IEEE80211_CSA_MLME_STATE_DELAY:
		break;
	...
	case IEEE80211_CSA_MLME_STATE_FINISHED:
		...
		csa_mlme->state = IEEE80211_CSA_MLME_STATE_IDLE;
#if (CONFIG_CPTCFG_CFG80211==1)
		cfg80211_ch_switch_notify(sdata->dev,&csa_mlme->settings.chandef);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3,12,0))
		atbm_printk_err("not support channel sw");
#elif (LINUX_VERSION_CODE <= KERNEL_VERSION(5,19,1))
		cfg80211_ch_switch_notify(sdata->dev,&csa_mlme->settings.chandef);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(6,3,0))
		cfg80211_ch_switch_notify(sdata->dev,&csa_mlme->settings.chandef,0);
#else
		cfg80211_ch_switch_notify(sdata->dev,&csa_mlme->settings.chandef,0,0);
#endif
		break;
	}
}

static void __ieee80211_csa_completed(struct ieee80211_hw *hw)
{
	struct ieee80211_local *local = hw_to_local(hw);
	struct ieee80211_sub_if_data *sdata = local->csa_sdata;
	struct ieee80211_csa_mlme *csa_mlme = &sdata->u.ap.csa_mlme;
	
	atbm_printk_always("[%s]:csa_completed[%d]\n",sdata->name,csa_mlme->state);
	
	if(csa_mlme->state == IEEE80211_CSA_MLME_STATE_FINISHED){
		struct ieee80211_channel_state *chan_state;
		/*
		*change channel and set beacon
		*/

		/*
		*it is dangerous to hold rtnl lock
		*ieee80211_do_stop will clear work,MUST not enter here.
		*/

		chan_state = ieee80211_get_channel_state(local, sdata);
		
		chan_state->tmp_channel = csa_mlme->chan;
		ieee80211_hw_config(local, 0);
		chan_state->oper_channel = csa_mlme->chan;
		...
		csa_mlme->state = IEEE80211_CSA_MLME_STATE_FAIL;
	}
	
	if(csa_mlme->block_tx)
		ieee80211_wake_queues_by_reason(&sdata->local->hw,
					IEEE80211_QUEUE_STOP_REASON_CSA);
	
	ieee80211_notify_csa_state(sdata);

	local->csa_sdata = NULL;
	local->csa = 0;
}
static void ieee80211_csa_work(struct atbm_work_struct *work)
{
	struct ieee80211_local *local =
		container_of(work, struct ieee80211_local, csa_work.work);
	struct ieee80211_sub_if_data *sdata;
	
	mutex_lock(&local->mtx);

	if(local->csa_sdata == NULL){
		goto out;
	}
	
	sdata = local->csa_sdata;

	if (test_and_clear_bit(CSA_COMPLETED, &local->csa)) {
		goto out_complete;
	}

	if (test_and_clear_bit(CSA_WAIT_START, &local->csa)){
		if(ieee80211_ap_csa_start(sdata)){
			sdata->u.ap.csa_mlme.state = IEEE80211_CSA_MLME_STATE_FAIL;
			goto out_complete;
		}
		set_bit(CSA_WORING,&local->csa);
		goto out;
	}
	/*
...
out_complete:
	__ieee80211_csa_completed(&local->hw);
out:
...
void ieee80211_ap_csa_init(struct ieee80211_local *local)
{
	ATBM_INIT_DELAYED_WORK(&local->csa_work, ieee80211_csa_work);
	
	local->csa_sdata = NULL;
	local->csa       = 0;
}
static enum work_action ieee80211_wk_csa_start(struct ieee80211_work *wk)
{
	struct ieee80211_sub_if_data *sdata = wk->sdata;
	struct ieee80211_csa_mlme *csa_mlme = &sdata->u.ap.csa_mlme;
	const struct atbm_element *csa_ie = NULL;
	struct ieee80211_csa_request request;
	struct beacon_data *csa_beacon;
	/*
	*timeout
	*/
	if(csa_mlme->state != IEEE80211_CSA_MLME_STATE_WK){
		if(csa_mlme->state == IEEE80211_CSA_MLME_STATE_START){
			goto timeout;
		}
		goto done;
	}
	...
	if(csa_beacon)
		csa_ie = ieee80211_find_elem(ATBM_WLAN_EID_CHANNEL_SWITCH,
				csa_beacon->tail,csa_beacon->tail_len);

	csa_mlme->state = IEEE80211_CSA_MLME_STATE_START;

	ieee80211_notify_csa_state(sdata);
	
	if(csa_ie == NULL || csa_ie->datalen != 3){
		atbm_printk_err("[%s]:can not find csa ie\n",sdata->name);
		goto timeout;
	}
	
	request.chan = csa_mlme->chan;
	memcpy(&request.ie,csa_ie->data,3);
	request.ie.count = csa_mlme->count;
	request.start = true;
	...
	wk->timeout = jiffies + msecs_to_jiffies((csa_mlme->count + 2) * (sdata->vif.bss_conf.beacon_int));

	atbm_printk_always("[%s]:csa wk start[%d][%d][[%d]\n",sdata->name,request.ie.mode,request.ie.new_ch_num,request.ie.count);
	return WORK_ACT_NONE;
timeout:
	csa_mlme->state = IEEE80211_CSA_MLME_STATE_TIMEOUT;
done:
	return WORK_ACT_TIMEOUT;
}
static enum work_done_result ieee80211_wk_csa_done(struct ieee80211_work *wk,
				      struct sk_buff *skb)
{
	struct ieee80211_sub_if_data *sdata = wk->sdata;
	struct ieee80211_csa_mlme *csa_mlme = &sdata->u.ap.csa_mlme;
	...
	atbm_printk_always("[%s]:wk_csa_done[%d]\n",sdata->name,csa_mlme->state);
	...
	if(csa_mlme->state == IEEE80211_CSA_MLME_STATE_FINISHED){
		/*
		*csa has been finished
		*/
	}else if(csa_mlme->state == IEEE80211_CSA_MLME_STATE_TIMEOUT){
		struct ieee80211_csa_request request;
		atbm_printk_always("[%s]:csa timeout\n",sdata->name);
		/*
		*csa timeout,firmware cost long time ,concel csa
		*/
		csa_mlme->state = IEEE80211_CSA_MLME_STATE_FAIL;
		memset(&request,0,sizeof(struct ieee80211_csa_request));
		request.start = false;
		drv_do_csa(sdata->local,sdata,&request);
		set_bit(CSA_COMPLETED, &local->csa);
		ieee80211_queue_delayed_work(&local->hw, &local->csa_work,msecs_to_jiffies(0));
	}
...
int ieee80211_ap_csa_start(struct ieee80211_sub_if_data *sdata)
{
	struct ieee80211_work *wk;
	struct ieee80211_channel_state *chan_state = ieee80211_get_channel_state(sdata->local, sdata);
	...
	wk->type = IEEE80211_WORK_CSA;
	memcpy(wk->filter_bssid,sdata->vif.addr,ETH_ALEN);
	memcpy(wk->filter_sa,   sdata->vif.addr,ETH_ALEN);
	wk->start     = ieee80211_wk_csa_start;
	wk->done      = ieee80211_wk_csa_done;
	wk->sdata     = sdata;
	wk->chan      = chan_state->oper_channel;
	wk->chan_type = chan_state->_oper_channel_type;
	atbm_printk_always("[%s]:csa wk queue\n",sdata->name);
	sdata->u.ap.csa_mlme.state = IEEE80211_CSA_MLME_STATE_WK;
...
void ieee80211_csa_cancel(struct ieee80211_sub_if_data *sdata)
{
	...
	ieee80211_work_purge(sdata,sdata->vif.addr,IEEE80211_WORK_CSA,true);
	
	mutex_lock(&local->mtx);
	
	if (local->csa_sdata != sdata)
		goto out;

	if (test_bit(CSA_WORING, &local->csa)) {
		struct ieee80211_csa_request request;
	...
	atbm_cancel_delayed_work(&local->csa_work);
	/* and clean up */
	__ieee80211_csa_completed(&local->hw);
out:
	mutex_unlock(&local->mtx);
