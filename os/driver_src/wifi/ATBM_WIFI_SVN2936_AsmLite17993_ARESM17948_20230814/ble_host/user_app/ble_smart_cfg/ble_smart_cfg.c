...
static int ble_test_task(void* param)
{
	int rc,i,cnt;
	for(i=0;i<66250;i++)
	{
		...
		test_f[0]++;
		iot_printf(" send %d\n",sizeof(test_f));
		rc=gatt_svr_chr_notify(ble_conn_handle, blesmt_notify_handle, test_f, sizeof(test_f));
		cnt=0;
		if(atbm_ble_status != ATBM_BLE_STATUS_CONNECT){
			break;
		}
		while(rc && (cnt<50)&&(atbm_ble_status == ATBM_BLE_STATUS_CONNECT))
		{
			cnt++;
...
//		if(!ble_test_thread)
//			ble_test_thread = atbm_createThread(ble_test_task, (atbm_void*)ATBM_NULL, BLE_APP_PRIO);
...
//			ble_gap_update_params(event->connect.conn_handle, &params);
		    ...
		    smt_conn_handle = event->connect.conn_handle;
		}
		else {
	...
	case BLE_GAP_EVENT_SUBSCRIBE:
	    ...
	    if(ble_svc_gatt_changed_val_handle==event->subscribe.attr_handle)
	    {
		    ...
		    {	   
			   ...
			   // rc=gatt_svr_chr_indicate(event->connect.conn_handle, ble_svc_gatt_changed_val_handle, ble_indicate, strlen(ble_indicate));
			    //assert(rc == 0);
		    }
	    }
		#endif
...
static int ble_smart_cfg_task(void* param)
{
	...
	*/
	...
	if(!ble_is_exit()){
		if (smt_conn_handle != 0xFFFF) {
			ble_gap_terminate(smt_conn_handle, BLE_ERR_REM_USER_CONN_TERM);
		}
		ble_gap_adv_stop();
	}
