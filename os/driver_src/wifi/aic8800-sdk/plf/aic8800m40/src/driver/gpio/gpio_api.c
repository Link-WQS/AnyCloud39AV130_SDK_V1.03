...
static uint32_t is_gpioa_dir_in(int gpidx)
{
    unsigned int gpmsk = 0x01UL << (gpidx);

    if (AIC_GPIOA->MR & gpmsk)
        return !(AIC_GPIOA->DR & gpmsk);
...
__RAMTEXT void gpio_regs_recover(void)
{
    ...
    for (idx = 0; idx < GPIOA_IDX_MAX; idx++) {
        if (is_gpioa_dir_in(idx)) {
            if (AIC_GPIOA->VR == gpio_registiers.gpioa.VR) {
                AIC_GPIOA->IRR = (0x01UL << idx);
            }
        }
    }
