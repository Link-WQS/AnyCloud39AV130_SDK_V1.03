...
/*
 * Register the bt assert notify handler, the handler should be registered before bt_hci_open
 * Host received controller's assert notification
 * @param handler
 *
 */
void bt_assert_notify_irq_handler_register(BT_WAKEUP_IND_IRQ_HANDLER handler);
