...
osal_s32 device_panic_msg_record_etc(osal_void)
{
    ...
    plat_pm_power_action(hcc_get_channel_bus(HCC_CHANNEL_AP), HCC_BUS_POWER_PATCH_LOAD_PREPARE, pm_data);
...
void plat_exception_reset_work_etc(struct work_struct *work)
{
    ...
    mdelay(1000);  // 休眠1000ms
#if defined(CONFIG_HCC_SUPPORT_UART) && defined(CONFIG_PLAT_UART_IO_CFG)
    gpio_state_change_cfg2uart();
#endif
    oal_print_warning("in ...\n");
    ...
    hcc_switch_status(HCC_BUS_FORBID);
    // 5 调wifi和蓝牙业务恢复接口
    if (g_dfr_plat_complete_wifi_cb != NULL) {
        g_dfr_plat_complete_wifi_cb();
    } else {
        oal_print_err("g_dfr_plat_complete_wifi_cb not registewr\n");
    }
    if (g_dfr_plat_complete_bt_cb != NULL) {
        g_dfr_plat_complete_bt_cb();
    } else {
        oal_print_err("g_dfr_plat_complete_bt_cb not registewr\n");
    }
