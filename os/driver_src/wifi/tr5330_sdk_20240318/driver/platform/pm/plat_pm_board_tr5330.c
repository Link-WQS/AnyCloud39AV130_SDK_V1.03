...
#define RESET_DEVICE_DELAY_MS 20
...
static td_void plat_reboot_system(td_void)
{
    td_u32 i;
...
td_s32 tr5330_board_power_reset_hcc(td_void)
{
    td_s32 ret;
    td_u32 val;

    ret = hcc_read_reg(HCC_CHANNEL_AP, CLK_RST_CRG_CFG_GLB_RST_CRG_CTL0_REG, &val);
    if (ret != EOK) {
        oal_print_err("reset_device_by_hcc::hcc_read_reg fail, ret=[%d] \n", ret);
        return ret;
    }
    val |= RST_MASK;
    ret = hcc_write_reg(HCC_CHANNEL_AP, CLK_RST_CRG_CFG_GLB_RST_CRG_CTL0_REG, val);
    if (ret != EOK) {
...
static td_s32 tr5330_board_power_on_gpio(td_void)
{
    td_s32 ret;
    td_s32 power_gpio_idx = get_pm_board_info()->power_gpio;
    ret = gpio_direction_output(power_gpio_idx, GPIO_HIGHLEVEL);
    if (ret != EOK) {
        oal_print_err("tr5330_board_power_on_gpio: set gpio[%d] high fail, ret=[%d] \n", power_gpio_idx, ret);
        return ret;
    }
    osal_mdelay(RESET_DEVICE_DELAY_MS);
...
static td_s32 tr5330_board_power_off_gpio(td_void)
{
    td_s32 ret;
    td_s32 power_gpio_idx = get_pm_board_info()->power_gpio;

    ret = gpio_direction_output(power_gpio_idx, GPIO_LOWLEVEL);
    if (ret != EOK) {
        oal_print_err("tr5330_board_power_off_gpio: set gpio[%d] low fail, ret=[%d] \n", power_gpio_idx, ret);
        return ret;
    }
    osal_mdelay(RESET_DEVICE_DELAY_MS);
...
/* 通过拉GPIO进行device侧复位 */
td_s32 tr5330_board_power_reset_gpio(td_void)
{
    td_s32 ret;

    ret = tr5330_board_power_off_gpio();
    if (ret != EOK) {
        oal_print_err("tr5330_board_power_reset_gpio: fail, ret=[%d] \n", ret);
        return ret;
    }

    ret = tr5330_board_power_on_gpio();
    if (ret != EOK) {
        oal_print_err("tr5330_board_power_reset_gpio: fail, ret=[%d] \n", ret);
        return ret;
    }
...
/* device上电 */
td_s32 tr5330_board_power_on(td_void)
{
    if (get_pm_board_info()->power_gpio_support) {
        return tr5330_board_power_on_gpio();
    }
    return PM_BOARD_POWER_SUCCESS;
}

/* device下电 */
td_s32 tr5330_board_power_off(td_void)
{
    if (get_pm_board_info()->power_gpio_support) {
        return tr5330_board_power_off_gpio();
    } else {
        tr5330_board_power_reset_hcc();
        return PM_BOARD_POWER_SUCCESS;
    }
}

/* 复位device */
td_s32 tr5330_board_power_reset(td_bool sys_boot)
{
    td_s32 ret;
#ifdef CONFIG_HCC_SUPPORT_USB
    usb_set_exception_status(TD_TRUE);
#endif
    if (sys_boot) {
    ...
    if (get_pm_board_info()->power_gpio_support) {
        ret = tr5330_board_power_reset_gpio();
    } else {
        ret = tr5330_board_power_reset_hcc();
    }
    if (sys_boot) {
...
static td_s32 tr5330_board_reinit_hcc_bus(hcc_bus *pst_bus)
{
    if ((pst_bus->bus_type != HCC_BUS_UART)
#ifdef CONFIG_PLAT_SUPPORT_DFR
        && (plat_is_device_in_recovery() == TD_FALSE)
#endif
    ) {
...
/* 业务首次加载, 激活device */
td_s32 tr5330_board_service_enter(td_void)
{
    td_s32 ret;
    ...
    // 3.hcc 重新初始化
    if (tr5330_board_reinit_hcc_bus(pst_bus) != OAL_SUCC) {
        oal_wake_unlock(&pm_data->st_wakelock);
        return OAL_FAIL;
    }
...
/* 所有业务退出, 复位device */
td_s32 tr5330_board_service_exit(td_void)
{
#if defined(_PRE_OS_VERSION) && defined(_PRE_OS_VERSION_LINUX) && (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    // 1.获取管理结构
    struct pm_drv_data *pm_data = pm_get_drvdata();
...
#endif
    // 2.复位device
    tr5330_board_power_reset(TD_FALSE);
