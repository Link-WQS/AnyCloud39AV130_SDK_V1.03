...
td_u16 hcc_tx_queue_proc(hcc_handler *hcc, hcc_trans_queue *queue)
{
    ...
    if (queue->queue_ctrl->fc_enable != 0 && queue->queue_ctrl->flow_type == HCC_FLOWCTRL_DATA) {
        remain_pkt_nums = hcc_flow_ctrl_credit_update(hcc, remain_pkt_nums);
    }
...
TD_PRV td_void hcc_service_resource_init(hcc_handler *hcc)
{
