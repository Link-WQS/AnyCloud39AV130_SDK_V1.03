...
static td_bool g_usb_exception_reset_work = TD_FALSE;
osal_u32 plat_exception_reset_work(osal_u8 *data);
td_void usb_exception_reset_work(td_void)
{
    if (!g_usb_exception_reset_work) {
        g_usb_exception_reset_work = TD_TRUE;
        plat_exception_reset_work(TD_NULL);
    }
}

td_void usb_set_exception_status(td_bool status)
{
    g_usb_exception_reset_work = status;
}
...
功能描述  : 分配urb流水资源
**************************************************************************** */
static td_s32 usb_rx_urb_list_init(td_void)
{
    td_u32 i;
...
功能描述  : urb流水资源回收
**************************************************************************** */
static td_void usb_rx_urb_list_deinit(td_void)
{
    td_u32 i;
...
static td_s32 usb_xfer_wifi_rx_data_thread(td_void *data)
{
    ...
    if (usb_rx_urb_list_init() != EXT_ERR_SUCCESS) {
        ...
        usb->is_wlan_rx_thread_running = TSK_USB_EXIT;
        oal_usb_log(BUS_LOG_ERR, "init urb list failed");
        return EXT_ERR_FAILURE;
    }
...
static td_s32 oal_usb_system_init_config(struct usb_interface *interface, struct usb_device *udev)
{
    ...
    iface_desc = interface->cur_altsetting;
    usb_set_exception_status(TD_FALSE);
