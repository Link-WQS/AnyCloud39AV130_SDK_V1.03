...
int osal_klib_unlink(const char *file_path)
{
    int ret = OSAL_SUCCESS;
    struct path path;
    size_t path_size;
    ...
    oam_warn("unlink file_path: %s\n", file_path);

    path_size = strlen(file_path) + 1;
    path_name = osal_kmalloc(path_size, OSAL_GFP_KERNEL);
    if (path_name == TD_NULL) {
        oam_error("path_name malloc fail. \r\n");
        return EXT_ERR_FAILURE;
    }
    if (strncpy_s(path_name, path_size, file_path, path_size)  != EOK) {
        oam_error("path_name strncpy_s fail. \r\n");
        osal_kfree(path_name);
        return EXT_ERR_FAILURE;
    } else {
        char *path_tail = path_name + path_size;
        while (*path_tail != '/') {
            path_tail--;
        }
        *path_tail = '\0';
    }
    oam_warn("unlink path_name: %s\n", path_name);

    if (kern_path(path_name, 0, &path)) {
        oam_error("get kern_path fail. unlink path_name: %s \r\n", path_name);
    }
...
#if defined(LINUX_VERSION_CODE) && (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 0))
    ret = vfs_unlink(&init_user_ns, path.dentry->d_inode, fp->f_path.dentry, NULL);
#elif defined(LINUX_VERSION_CODE) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
    ret = vfs_unlink(path.dentry->d_inode, fp->f_path.dentry, NULL);
#else
    ret = vfs_unlink(path.dentry->d_inode, fp->f_path.dentry);
#endif
