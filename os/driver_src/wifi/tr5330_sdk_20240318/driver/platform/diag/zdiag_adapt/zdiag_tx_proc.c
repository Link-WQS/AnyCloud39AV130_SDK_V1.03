...
OSAL_STATIC oal_completion g_diag_task_done;
...
static td_u32 zdiag_local_file_output(td_u8 *buf, td_u16 buf_len)
{
    ...
    *(td_u32 *)(mem + 1) = buf_len - all_heads_len;
    ...
    if (ret == EXT_ERR_DIAG_NOT_SUPPORT) {
        oal_print_err("diag offline switch off, config not support! \n");
    } else if (ret == EXT_ERR_PTR_NULL) {
        oal_print_err("diag offline null ptr! \n");
    } else if (ret != EXT_SUCCESS) {
        oal_print_err("diag offline log ret:%d \n", ret);
    }
...
/* Description : send data:tr_dms_chl_tx_data_s* (and stat dataPrams) to hso by port. */
...
// similar with 86's (z)diag_proc_task_body
static td_s32 zdiag_tx_proc_task(osal_pvoid unused)
{
    ...
    ctx->tx_task_break_flag = OSAL_FALSE;
    for (;;) {
        osal_sem_down_timeout(&ctx->diag_tx_sem, jiffies); // sem_down similar to sem_lock
        if ((ctx->tx_task_break_flag == OSAL_TRUE) ||
            unlikely(osal_kthread_should_stop())) { // encap kthread_should_stop
            zdiag_print("zdiag tx proc break. \n");
            break;
        }
...
ext_errno zdiag_tx_proc_init(td_void)
{
    ...
    if (ret != OSAL_SUCCESS) {
        oal_print_err("diag proc tx_queue spinlock init failed, ret:%d", ret);
        return EXT_ERR_FAILURE;
    }
    ...
    if (ctx->tx_queue == TD_NULL) {
        ...
        oal_print_err("diag proc tx_queue fail!\r\n");
        return EXT_ERR_FAILURE;
    }
    ctx->tx_fail_cnt = 0;
    ctx->tx_succ_cnt = 0;
    ...
    init_completion(&g_diag_task_done);
    ...
    osal_sem_init(&ctx->diag_tx_sem, 0); // encap sema_init
...
ext_errno zdiag_tx_proc_exit(td_void)
{
    ...
    if (ctx->diag_tx_proc_task != TD_NULL) {
        ctx->tx_task_break_flag = OSAL_TRUE;
        oal_wait_for_completion_timeout(&g_diag_task_done,
