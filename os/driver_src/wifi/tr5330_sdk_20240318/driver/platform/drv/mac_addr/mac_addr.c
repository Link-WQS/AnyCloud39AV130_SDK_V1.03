...
#define FORK_MAC_ADDR 4
osal_u32 fork_addr(osal_u8 mac[], osal_u8 mac_len, addr_idx idx)
{
    ...
    us_addr[FORK_MAC_ADDR] += idx;  // 4 字节增加idx
    for (tmp = FORK_MAC_ADDR; tmp > 0; tmp--) {
...
osal_u32 get_dev_addr(osal_u8 *pc_addr, osal_u8 addr_len, osal_u8 type)
{
    ...
    if (g_mac_derivation_ptr != NULL) {
        ...
        return fork_addr(pc_addr, addr_len, addr_idx);
    }
}
