...
#define wlan_data_queue_to_hcc(_qid) ( \
        (((_qid) == WLAN_UDP_DATA_QUEUE)) ? DATA_UDP_DATA_QUEUE : \
        (((_qid) == WLAN_TCP_DATA_QUEUE)) ? DATA_TCP_DATA_QUEUE : \
        (((_qid) == WLAN_TCP_ACK_QUEUE)) ? DATA_TCP_ACK_QUEUE : \
        DATA_LO_QUEUE)

#define hcc_data_queue_to_wlan(_qid) ( \
        (((_qid) == DATA_UDP_DATA_QUEUE)) ? WLAN_UDP_DATA_QUEUE : \
        (((_qid) == DATA_TCP_DATA_QUEUE)) ? WLAN_TCP_DATA_QUEUE : \
        (((_qid) == DATA_TCP_ACK_QUEUE)) ? WLAN_TCP_ACK_QUEUE : \
        WLAN_NORMAL_QUEUE)
#endif
    ...
    const oal_ieee80211_regdomain_stru *regd)
{
    ...
    /* trylock再unlock保证进入内核时mtx锁为解锁状态 */
    mutex_trylock(&wiphy->mtx);
    wiphy_unlock(wiphy);
#endif
    wiphy_apply_custom_regulatory(wiphy, regd);

    if (rollback_lock == OAL_TRUE) {
        rtnl_lock();
    }
}
