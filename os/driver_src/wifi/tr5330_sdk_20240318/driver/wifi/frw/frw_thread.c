...
static WIFI_HMAC_TCM_TEXT osal_s32 frw_netbuf_que_handle(osal_void *data)
{
    ...
    if (g_frw_thread_ctrl.frw_threads[FRW_THREAD_ID_TXDATA].status == FRW_THREAD_EXIT) {
        return OAL_FAIL;
    }
...
#if defined(_PRE_WLAN_FEATURE_TR5330)
/* 注册给hcc的钩子函数，负责处理所有从hcc接收到的异常回复消息 */
static osal_void frw_host_rcv_msg_fail_cb(osal_void)
{
    frw_thread_que *que = frw_get_thread_que(FRW_THREAD_ID_MSG, FRW_THREAD_QUE_SEND_SYNC_MSG);
    frw_msg_node *node = OSAL_NULL;

    /* 加500ms延时保证该消息的前一个同步消息已执行完成 */
    osal_msleep(500);
    node = frw_node_fetch((osal_void *)que);
    if (osal_unlikely(node == OSAL_NULL)) {
        oam_error_log0(0, OAM_SF_FRW, "{frw_host_rcv_msg_fail:: send sync que empty}");
        return;
    }
    oam_warning_log2(0, OAM_SF_FRW, "{msg_trace::HMAC SYNC_RSP_FAIL msg_id %d seq %d}", node->msg_id, node->seq);
    /* 同步消息不携带回复，可认为消息已经执行成功，只是回复上报失败，如果同步消息携带回复，返回异常给业务 */
    if (node->msg.rsp_buf_len == 0) {
        node->cb_return = OAL_SUCC;
    } else {
        node->cb_return = OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
...
{
    frw_thread *th = (frw_thread *)param;
    return (osal_adapt_kthread_should_stop() != 0 || frw_thread_should_wake(th) || th->status == FRW_THREAD_EXIT);
}
...
#endif
    /* 进入线程循环主逻辑 */
    while (th->status != FRW_THREAD_EXIT) {
        th->is_running = OSAL_FALSE;
        ret = osal_wait_interruptible(&th->wq, frw_thread_should_stop, th);
        if ((osal_adapt_kthread_should_stop() != 0) || (th->status == FRW_THREAD_EXIT)) {
            break;
        }
...
#if defined(_PRE_WLAN_FEATURE_TR5330)
    hcc_hmac_wifi_msg_register(D2H_MSG_REPORT_RSP_FAIL, (osal_void *)frw_host_rcv_msg_fail_cb);
#endif

    return OAL_SUCC;
}

static osal_void frw_clear_netbuf(osal_u32 queue_id, const frw_msg_node *cur_node)
{
...
static osal_s32 frw_queue_clear(frw_thread *th)
{
