    ...
    hal_to_dmac_device_mac_test_stru *hal_mac_test_device)
{
    ...
    // 使用ram3的作为数采空间 tr5330V100 DIAG_CTL维测功能使用手册.docx 信息表 DIAG_RAM 地址 当前给定32K
...
osal_void hal_set_mac_diag_mode_deal_diag8_test(osal_u32 cfg_value)
{
    ...
    if (cfg_value < HAL_MAC_DIAG8_NODE_H || cfg_value > HAL_MAC_DIAG8_NODE_PTR_PSI) {
    ...
    // 待观测的节点编号
    diag_ctrl1.bits._Ct2O4lz2nOov4mOsmx_ = cfg_value; // 0：H内容   1：HV内容  2：SNR内容 3：{PTR_PSI}
    hal_reg_write(HH503_MAC_CTRL0_BANK_BASE_0x18C, diag_ctrl1.u32);
