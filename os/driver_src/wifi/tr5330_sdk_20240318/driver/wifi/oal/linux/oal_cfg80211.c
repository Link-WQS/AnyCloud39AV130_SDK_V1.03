    ...
    oal_gfp_enum_uint8 gfp)
{
    ...
    oal_nla_put_u32(msg, PRIV_NL80211_ATTR_IFINDEX, netdev->ifindex);
    oal_nla_put_u32(msg, PRIV_NL80211_ATTR_WIPHY_FREQ, l_freq);
    oal_nla_put_u32(msg, PRIV_NL80211_ATTR_RX_SIGNAL_DBM, rssi);  // report rssi to hostapd
    OAL_NLA_PUT(msg, PRIV_NL80211_ATTR_FRAME, len, buf);
#else
    ...
    osal_u32      freq, enum PRIV_NL80211_CHAN_WIDTH  chan_width, enum PRIV_NL80211_RADAR_EVENT event)
{
    ...
    if (OAL_PTR_NULL == p_hdr) {
        ...
        oal_nla_put_u32(buf, PRIV_NL80211_ATTR_RADAR_EVENT, event);
        oal_nla_put_u32(buf, PRIV_NL80211_ATTR_WIPHY_FREQ, freq);
        oal_nla_put_u32(buf, PRIV_NL80211_ATTR_CHANNEL_WIDTH, chan_width);
#else
        if (net_dev) {
...
osal_u32 oal_cfg80211_disconnected_etc(oal_net_device_stru        *net_device,
    osal_u16                  reason, osal_u8                  *ie, osal_u32                  ie_len,
    oal_gfp_enum_uint8          gfp)
{
#if defined(LINUX_VERSION_CODE) && (LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0))
    cfg80211_disconnected(net_device, reason, ie, ie_len, 0, gfp);
...
oal_module_symbol(oal_cfg80211_external_auth_req);
#endif
#endif
#ifdef __cplusplus
    #if __cplusplus
