...
*****************************************************************************/
static osal_void mac_ie_get_wlan_suite(osal_u8 **ie, osal_u32 *suite, osal_u32 size, osal_u16 suites_count)
{
    osal_u16 suite_idx;
...
osal_u32 hmac_ie_get_rsn_cipher(osal_u8 *rsn_ie, mac_crypto_settings_stru *crypto)
{
    ...
    suites_count = OAL_MAKE_WORD16(ie[0], ie[1]);
    ie += 2;            /* 2是Pairwise Suite字段长度 */

    /* Pairwise Cipher Suite 最多存2个 */
    mac_ie_get_wlan_suite(&ie, crypto->pair_suite, WLAN_PAIRWISE_CIPHER_SUITES, suites_count);

    suites_count = OAL_MAKE_WORD16(ie[0], ie[1]);
    ie += 2;            /* 2是AKM Suite Count的长度 */

    /* AKM Suite 最多存2个 */
    mac_ie_get_wlan_suite(&ie, crypto->akm_suite, WLAN_AUTHENTICATION_SUITES, suites_count);
    ...
    for (suite_idx = 0; suite_idx < OAL_MAKE_WORD16(ie[0], ie[1]); suite_idx++) {
        ie += 4;        /* 4是每个PMKID的长度 */
    }
