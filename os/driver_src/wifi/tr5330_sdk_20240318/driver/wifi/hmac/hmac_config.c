...
#define HMAC_CCPRIV_CMD_NAME_MAX_LEN  16                             /* 字符串中每个单词的最大长度 */
...
#else
        entry->timestamp = (osal_u32)get_seconds();
#endif
...
#else
        new_pmksa->timestamp = (osal_u32)get_seconds();
#endif
...
osal_s32 hmac_config_get_verion(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    version_t version;
    unref_param(hmac_vap);
    unref_param(msg);

    version.device.ver = hmac_get_device_version();
    version.device_rom.ver = hmac_get_device_rom_version();
    version.host.ver = hmac_get_version();

    wifi_printf("SDK Version: %d.%d.%d\r\n",
                version.host.bits.a, version.host.bits.b, version.host.bits.c);
