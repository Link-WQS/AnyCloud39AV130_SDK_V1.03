    ...
    osal_u8 *go_on)
{
    ...
    /* 如果pen_report_bss返回为TR_TRUE代表扫描接口已经上报,hmac_rx_filter_mgmt则不会释放netbuf,
    ...
    osal_list_for_each(entry, &(bss_mgmt->bss_list_head)) {
        ...
        if (scanned_bss->bss_dscr_info.new_scan_bss == OAL_TRUE) {
           ...
           /* 上报beacon帧或者probe rsp帧 */
            ...
            adjust_hso_param.channel = hmac_ie_get_chan_num_etc(bss_dscr->mgmt_buff + MAC_80211_FRAME_LEN,
                (osal_u16) bss_dscr->mgmt_len, offset, bss_dscr->st_channel.chan_number);
...
#if defined(_PRE_WLAN_FEATURE_TR5330) && !defined(_PRE_OS_VERSION_LITEOS)
