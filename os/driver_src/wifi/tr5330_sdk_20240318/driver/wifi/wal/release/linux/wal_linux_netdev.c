...
osal_s32 hwifi_config_init_ini_etc(oal_net_device_stru *net_dev)
{
    ...
    ret_len = snprintf_s(ac_wlan_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "wlan%d",
        hmac_device->device_id);
    if (ret_len < 0) {
        oam_warning_log0(0, OAM_SF_ANY, "{hwifi_config_init_ini_etc::set wlan name fail}");
    }
    ret_len = snprintf_s(ac_p2p_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "p2p%d",
        hmac_device->device_id);
    if (ret_len < 0) {
...
OAL_STATIC osal_s32 wal_netdev_open_ap_pre_check(oal_net_device_stru *net_dev)
{
    ...
    if ((osal_strcmp((const osal_s8 *)net_dev->name, P2P_NETDEV_IFACE_NAME)) == 0) {
...
osal_s32  wal_netdev_open_etc(oal_net_device_stru *net_dev, osal_u8 entry_flag)
{
    ...
    /* 记录wlan0 开的时间 */
    if (osal_adapt_strncmp("wlan0", net_dev->name, osal_strlen("wlan0")) == 0) {
...
OAL_STATIC osal_s32  _wal_netdev_set_mac_addr(oal_net_device_stru *net_dev, void *p_addr)
{
    ...
    ret = set_dev_addr(net_dev->dev_addr, WLAN_MAC_ADDR_LEN, net_dev->ieee80211_ptr->iftype);
...
#ifdef _PRE_WLAN_FEATURE_P2P
        if (osal_strcmp("p2p0", (const osal_s8 *)net_dev->name) == 0) {
            p2p_mode = WLAN_P2P_DEV_MODE;
        }
#endif
#ifdef CONTROLLER_CUSTOMIZATION
        if (osal_strcmp("ap0", (const osal_s8 *)net_dev->name) == 0) {
...
STATIC osal_s32 wal_deinit_wlan_vap_etc(oal_net_device_stru *net_dev)
{
    ...
    ret = snprintf_s(p2p_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "p2p%d",
        hmac_device->device_id);
    if (ret < -1) {
...
#ifdef _PRE_WLAN_FEATURE_P2P
    if (get_dev_addr((osal_u8 *)(net_dev->dev_addr), WLAN_MAC_ADDR_LEN, wdev->iftype) != OAL_SUCC) {
...
/*****************************************************************************
 ...
 被调函数  : wal_init_wlan_netdev_ext(char *dev_name, enum nl80211_iftype type, osal_s8 *wlan_netdev_name,
                osal_s8 *hwlan_netdev_name, oal_wiphy_stru *wiphy)
*****************************************************************************/
...
/*****************************************************************************
 ...
 被调函数  : wal_init_wlan_netdev_etc(oal_wiphy_stru *wiphy, char *dev_name);
*****************************************************************************/
OAL_STATIC osal_u32 wal_init_wlan_netdev_ext(char *dev_name, enum nl80211_iftype type,
    const osal_s8 *wlan_netdev_name, const osal_s8 *wlan1_netdev_name, oal_wiphy_stru *wiphy)
{
    ...
    if ((type == NL80211_IFTYPE_STATION) && ((osal_strcmp(wlan_netdev_name, (const osal_s8 *)dev_name)) == 0)) {
    ...
    else if ((type == NL80211_IFTYPE_AP) && ((osal_strcmp(wlan1_netdev_name, (const osal_s8 *)dev_name)) == 0)) {
...
/*****************************************************************************
 函 数 名  : wal_init_wlan_netdev_etc
 功能描述  : 初始化wlan0和p2p0设备
...
osal_s32 wal_init_wlan_netdev_etc(oal_wiphy_stru *wiphy, char *dev_name)
{
    ...
    osal_s8                    wlan_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
#ifdef CONTROLLER_CUSTOMIZATION
    osal_s8                    wlan1_netdev_name[MAC_NET_DEVICE_NAME_LENGTH] = {"ap0"};
#else
    osal_s8                    wlan1_netdev_name[MAC_NET_DEVICE_NAME_LENGTH] = {"wlan1"};
#endif
    osal_s8                    p2p_netdev_name[MAC_NET_DEVICE_NAME_LENGTH];
    osal_u32                  ret;

    if ((wiphy == OSAL_NULL) || (dev_name == OSAL_NULL)) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_init_wlan_netdev_etc::wiphy or dev_name is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    snprintf_s(wlan_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "wlan%d",
        hmac_device->device_id);
    snprintf_s(p2p_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "p2p%d",
        hmac_device->device_id);

    /* 信道跟随--添加hwlan name判断 */
    if (osal_strcmp((const osal_s8 *)wlan_netdev_name, (const osal_s8 *)dev_name) == 0) {
        type = NL80211_IFTYPE_STATION;
    } else if (osal_strcmp((const osal_s8 *)wlan1_netdev_name, (const osal_s8 *)dev_name) == 0) {
        type = NL80211_IFTYPE_AP;
    } else if (0 == (osal_strcmp((const osal_s8 *)p2p_netdev_name, (const osal_s8 *)dev_name))) {
    ...
    ret = wal_init_wlan_netdev_ext(dev_name, type, (const osal_s8 *)wlan_netdev_name,
        (const osal_s8 *)wlan1_netdev_name, wiphy);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_init_wlan_netdev_etc::wal_init_wlan_netdev_ext err code %d}", ret);
        return ret;
    }
    return OAL_SUCC;
}
