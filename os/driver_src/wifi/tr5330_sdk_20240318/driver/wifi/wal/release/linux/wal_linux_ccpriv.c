...
OAL_STATIC osal_u32 uapi_ccpriv_set_duty_ratio(oal_net_device_stru *net_dev, osal_s8 *param)
{
    ...
    if (ratio > WAL_CCPRIV_MAX_DUTY_RATIO) {
        ratio = WAL_CCPRIV_MAX_DUTY_RATIO;
    }
    g_duty_ratio = ratio;
    return TR_SUCCESS;
}
...
OAL_STATIC osal_u32 uapi_get_back_off_time(osal_u32 rate, osal_u32 len, osal_u32 ratio)
{
    ...
    back_off_time = (hmac_get_al_tx_protocol() == 0) ? (back_off_time / WAL_CCPRIV_MAX_LONG_SLOT) :
        (back_off_time / WAL_CCPRIV_SHORT_SLOT);
    back_off_time = (back_off_time > WAL_CCPRIV_MAX_BACK_OFF_TIME) ? WAL_CCPRIV_MAX_BACK_OFF_TIME : back_off_time;
...
OAL_STATIC osal_void uapi_set_al_tx_duty_ratio(oal_net_device_stru *net_dev, osal_u8 tx_flag, osal_u32 len)
{
    ...
    osal_u32 rate;
    osal_u32 back_off_time;
    osal_s8 *slot_time = WAL_CCPRIV_SET_SHORT_SLOT_ORDER;
    ...
    slot_time = (hmac_get_al_tx_protocol() == 0) ? WAL_CCPRIV_SET_LONG_SLOT_ORDER : WAL_CCPRIV_SET_SHORT_SLOT_ORDER;
...
{
    osal_s8 mode_str[WAL_CCPRIV_CMD_NAME_MAX_LEN] = {0};     /* 预留协议模式字符串空间 */
    osal_u32 off_set;
    osal_u32 ret;
...
OAL_STATIC osal_u32  uapi_ccpriv_set_freq(oal_net_device_stru *net_dev, osal_s8 *pc_param)
{
    osal_s32                   l_freq;
    osal_u32                  off_set;
    osal_s8                    ac_freq[WAL_CCPRIV_CMD_NAME_MAX_LEN];
    osal_u32                  ul_ret;
    osal_s32                   l_ret;
...
OAL_STATIC osal_u32 uapi_ccpriv_add_vap(oal_net_device_stru *cfg_net_dev, osal_s8 *pc_param)
{
    ...
    osal_s8 ac_mode[WAL_CCPRIV_CMD_NAME_MAX_LEN];
    ...
    net_dev = uapi_ccpriv_init_net_dev(cfg_net_dev, ac_name, wdev);
    if (net_dev == OAL_PTR_NULL) {
...
OAL_STATIC osal_u32 uapi_ccpriv_adjust_tx_power(oal_net_device_stru *net_dev, osal_s8 *pc_param)
{
    ...
    if (osal_unlikely(ret != TR_SUCCESS)) {
...
OAL_STATIC osal_u32 uapi_ccpriv_get_version(oal_net_device_stru *net_dev, osal_s8 *param)
{
    osal_s32 ret;
    frw_msg cfg_info = {0};

    unref_param(param);
    ret = wal_sync_post2hmac_no_rsp(wal_util_get_vap_id(net_dev), WLAN_MSG_W2H_CFG_GET_VERSION,
        (osal_u8*)&cfg_info, OAL_SIZEOF(cfg_info));
    if (OAL_UNLIKELY(ret != OAL_SUCC)) {
        oam_error_log1(0, OAM_SF_ANY,
                       "{uapi_ccpriv_get_version::send_cfg_to_device return err code [%d]!}", ret);
        return (osal_u32)ret;
    }
    return OAL_SUCC;
}
...
{
    oal_net_device_stru  *net_dev;
    osal_s8              ac_dev_name[WAL_CCPRIV_CMD_NAME_MAX_LEN];
    osal_u32            ul_ret;
...
OAL_STATIC osal_u32 wal_ccpriv_parse_cmd_etc(osal_s8 *pc_cmd)
{
    ...
    **************************** ***********************************************/
    ...
    if (ul_ret == OAL_SUCC) {
        ...
        wal_record_external_log_by_name(WLAN_WIFI_CCPRIV_CMD, (osal_u8 *)info.name, ul_ret);
        return ul_ret;
    }
...
OSAL_STATIC osal_u32 uapi_ccpriv_set_soft_retry_num(oal_net_device_stru *net_dev, osal_s8 *param)
{
    ...
    ret = wal_get_cmd_one_arg_digit(param, cmd, WAL_CCPRIV_CMD_NAME_MAX_LEN, &offset, &value);
    if (ret != OAL_SUCC) {
