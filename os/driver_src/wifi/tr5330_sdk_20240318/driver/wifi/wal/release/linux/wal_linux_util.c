    ...
    osal_u32 *pul_cmd_offset, osal_u32 param_max_len)
{
    ...
    while ((',' != *pc_cmd_copy) && ('\0' != *pc_cmd_copy)) {
        ...
        if (OAL_UNLIKELY(pos >= param_max_len)) {
            oam_warning_log1(0, OAM_SF_ANY,
                "{wal_get_parameter_from_cmd::pos >= WAL_CCPRIV_CMD_NAME_MAX_LEN, pos %d!}", pos);
...
STATIC osal_u32 wal_generate_mac_addr(oal_net_device_stru *net_dev, hmac_vap_stru *hmac_vap)
{
    ...
    osal_u8 iftype = net_dev->ieee80211_ptr->iftype;
    if (is_p2p_mode(hmac_vap->p2p_mode)) {
        iftype = NL80211_IFTYPE_P2P_DEVICE;
    }
    return get_dev_addr(wlan_addr, WLAN_MAC_ADDR_LEN, iftype);
}
