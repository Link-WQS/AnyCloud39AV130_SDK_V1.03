    ...
    oal_beacon_parameters *beacon_info)
{
    ...
    if (wmm_ie != OSAL_NULL) {
...
#elif defined(_PRE_PRODUCT_ID_TR52_HOST)
        uapsd_param.uapsd_switch = OSAL_FALSE;
#endif
        ...
        if (OAL_UNLIKELY(ret != TR_SUCCESS)) {
            ret = TR_FAILURE;
...
#endif
{
    ...
    cfg_msg_init((osal_u8 *)&auc_value, OAL_SIZEOF(auc_value), OSAL_NULL, 0, &cfg_info);
...
{
    osal_s32 ret;
    osal_s8 vap_netdev_name[MAC_NET_DEVICE_NAME_LENGTH] = {0};

    /* 主路辅路的netdev规格:主路wlan0 p2p0,辅路wlan1 p2p1 */
    snprintf_s(vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "wlan%u", dev_id);
    ret = wal_init_wlan_netdev_etc(hmac_device->wiphy, (char *)vap_netdev_name);
    if (ret != OAL_SUCC) {
    ...
    snprintf_s(vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "p2p%u", dev_id);
    ret = wal_init_wlan_netdev_etc(hmac_device->wiphy, vap_netdev_name);
    if (ret != OAL_SUCC) {
...
#ifdef CONTROLLER_CUSTOMIZATION
    snprintf_s(vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "ap0");
#else
    snprintf_s(vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH, "wlan1");
#endif
    ret = wal_init_wlan_netdev_etc(hmac_device->wiphy, vap_netdev_name);
    if (ret != OAL_SUCC) {
    ...
    osal_u32 chip, osal_u8 device, osal_u8 dev_id)
{
    ...
    oal_wiphy_apply_custom_regulatory(hmac_device->wiphy, wal_regdb_get_default_regdom());
