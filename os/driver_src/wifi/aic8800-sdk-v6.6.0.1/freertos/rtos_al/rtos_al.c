...
#define TASK_NUM_TRACED_MAX     18

void rtos_task_info(void)
{
    uint32_t local_buffer[TASK_NUM_TRACED_MAX * sizeof(TaskStatus_t) / sizeof(uint32_t)];
    UBaseType_t array_size = uxTaskGetNumberOfTasks();
    TaskStatus_t *task_status_array;
    if (TASK_NUM_TRACED_MAX < array_size) {
        dbg("TaskNum=%d\n", array_size);
        task_status_array = rtos_malloc(array_size * sizeof(TaskStatus_t));
    } else {
        task_status_array = (TaskStatus_t *)local_buffer;
    }
    if (task_status_array) {
        ...
        if (isr_active) {
            dbg("GetSystemStateFromISR\n");
            array_size = uxTaskGetSystemStateFromISR(task_status_array, array_size, NULL);
        } else {
