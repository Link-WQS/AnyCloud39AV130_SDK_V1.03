...
#if (PLF_AIC8800M40)
static int cmd_flash_reg_cfg(int argc, char * const argv[])
{
    unsigned int func = console_cmd_strtoul(argv[1], NULL, 0);
    if (func == 0) {
        int chk_res = flash_status_register_check();
        UART_PRINT("status reg check: %d\n", chk_res);
    } else {
        flash_status_register_recover();
        UART_PRINT("status reg recover\n");
    }
    return 0;
}
#endif
...
void flash_test(void)
{
    ...
    #if (PLF_AIC8800M40)
    UART_PRINT("Flash status_reg=%08X\n", flash_status_register_read(0));
    #endif
    ...
    #if (PLF_AIC8800M40)
    console_cmd_add("cfgsr",        "cfg status reg",         2, 2, cmd_flash_reg_cfg);
    #endif
