...
void ble_task_need_pon_set(bool need_pon)
{
    ble_task_need_pon = need_pon;
    TRACE("%s,%d\n",__func__,ble_task_need_pon);
}

bool ble_task_need_pon_get(void)
{
    return ble_task_need_pon;
}
...
#if PLF_AIC8800M40 == 1
static void bt_assert_notify_hanlder(void)
{
    dbg("%s\n",__func__);
}
#endif
...
#if PLF_AIC8800M40 == 1
    bt_assert_notify_irq_handler_register(bt_assert_notify_hanlder);
#endif
