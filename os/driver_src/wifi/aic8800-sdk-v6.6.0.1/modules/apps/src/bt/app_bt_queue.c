...
void app_bt_delete_us_timer(void)
{
    if(app_bt_user_sleep_timer){
        rtos_timer_delete(app_bt_user_sleep_timer,0);
        app_bt_user_sleep_timer = NULL;
    }
}
...
#endif
        case BT_DEINIT_DONE:
            status = bt_task_delete();
            break;
        case BT_DEINIT_START:
            {
                aic_adp_bt_deinit();
            }
            break;
...
int app_bt_queue_deinit(void)
{
    if(app_bt_queue){
        rtos_queue_delete(app_bt_queue);
        app_bt_queue = NULL;
    }
    app_bt_handler_register(HANDLER_REG_1, 0);
    app_bt_delete_us_timer();
    return 0;
}

int app_bt_deint_start_msg_send(void)
{
    APP_BT_MSG msg;
    msg.msg_id = BT_DEINIT_START;
    app_bt_queue_send(&msg);
    return 0;
}

int app_bt_deint_done_msg_send(void)
{
    APP_BT_MSG msg;
    msg.msg_id = BT_DEINIT_DONE;
    app_bt_queue_send(&msg);
    return 0;
}
