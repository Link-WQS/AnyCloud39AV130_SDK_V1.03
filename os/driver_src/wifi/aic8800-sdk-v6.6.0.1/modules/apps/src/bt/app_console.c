...
static int app_bt_onoff_control(int argc, char * const argv[])
{
    unsigned int on_off;
    ...
    if(on_off){
        if(bt_task_need_pon_get() == false){
            bt_task_need_pon_set(true);
            bt_task_init();
        }
    }else{
        bt_task_deinit();
...
#endif
    console_cmd_add("bt_on", "bt_on/off  eg: bt_on <1/0>", 2, 2, app_bt_onoff_control);
}
#endif
#endif
#endif
