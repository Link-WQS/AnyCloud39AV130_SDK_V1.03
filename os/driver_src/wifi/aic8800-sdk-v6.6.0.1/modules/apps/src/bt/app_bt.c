...
{
    TRACE("APP:app_bt_deinit\n");
    for (uint8_t i=0;i<LINK_MAX;i++){
#if PLF_AON_SUPPORT == 1
        //app_con_ptr[i]->reconnect.cb_param = (void *)app_con_ptr[i];
#else
        if (app_con_ptr[i]->reconnect.connect_timer_handle){
            rtos_timer_delete(app_con_ptr[i]->reconnect.connect_timer_handle,0);
            app_con_ptr[i]->reconnect.connect_timer_handle = NULL;
        }
#endif
    }
    if(app_bt_pd_timer){
        rtos_timer_delete(app_bt_pd_timer,0);
        app_bt_pd_timer = NULL;
    }
}
...
bool app_bt_activity_enabled(void)
{
    bool app_active = false;
    if(app_get_bt_state() > BT_ACTIVED){
        app_active = true;
    }
    if (bt_scan_mode != BT_NOSCAN) {
       app_active = true;
    }
    return app_active;
}
