...
static int app_ble_send_get_rssi(int argc, char * const argv[])
{
    unsigned int conidx = 0;
    ...
    conidx = console_cmd_strtoul(argv[1], NULL, 10);
    dbg("%s,conidx %d\n",__func__,conidx);
    app_ble_get_rssi_by_indix(conidx);
    return ERR_NONE;
}
...
void app_ble_test_command_add(void)
{
    ...
    console_cmd_add("ble_getrssi", "ble get rssi by conidx  eg: ble_getrssi <conidx> ", 1, 2, app_ble_send_get_rssi);
}
#endif // (PLF_CONSOLE)
