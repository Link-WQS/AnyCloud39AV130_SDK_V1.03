...
int do_start_p2p(int argc, char * const argv[])
{
    ...
    size_t ssid_len = strlen("DIRECT-AIC-IoT-P2P");
    memcpy(p2p_cfg.ssid.array, "DIRECT-AIC-IoT-P2P", ssid_len);
    p2p_cfg.ssid.length = ssid_len;
    p2p_cfg.ssid.array[ssid_len] = '\0';

    size_t pass_len = strlen("kkkkkkkk");
    memcpy(p2p_cfg.key, "kkkkkkkk", pass_len);
...
int do_stop_p2p(int argc, char * const argv[])
{
    int fvif_idx = 0;
...
int do_connect (int argc, char * const argv[])
{
    ...
    if (flash_ssid_rd_flag) {
        ...
        if (argc > 3) {  // <Password>
            ...
            if ((len == (4 + 1)) && (strncmp("VOID", argv[3], 4) == 0)) {
                pw[0] = '\0';
            } else {
    ...
    if (argc > 4) {
        timeout_ms = (int)console_cmd_strtoul(argv[4], NULL, 10);
        if (WLAN_CONNECT_PARAM_2_CFG(timeout_ms) == WLAN_CONNECT_CFG_DISABLE_AUTO_RECONN) {
            dbg("STA auto reconnect disabled!\n");
        }
    }
    ...
    if (0 != wlan_start_sta(ssid, pw, timeout_ms)) {
        goto EXIT;
    }
    ...
    if (info == NULL) {
        dbg("Get ap info fail\n");
        goto EXIT;
    }
    dbg("BSSID %04x:%04x:%04x,band %d, Freq %d, ssid %s, noise %d\r\n",
        info->bssid.array[0],info->bssid.array[1],info->bssid.array[2], info->band, info->freq, info->ssid, wlan_get_current_channel_noise());
