...
#define SCAN_FREQ_LIST_SPICIFIED    0
...
static int do_scan(int argc, char * const argv[])
{
    ...
    int *p_freq_list = NULL;
    #if (SCAN_FREQ_LIST_SPICIFIED)
    int freq_list[14] = {
        (2412 | FHOST_PASSIVE_SCAN_FLAG),2417,2422,2427,2432,
        (2437 | FHOST_PASSIVE_SCAN_FLAG),2442,2447,2452,2457,
        (2462 | FHOST_PASSIVE_SCAN_FLAG),2467,2472,
        0}; // end of array
    p_freq_list = &freq_list[0];
    #endif
    ...
    nb_res = fhost_scan(console_cntrl_link, fvif_idx, p_freq_list);
    dbg("Got %d scan results\n", nb_res);

    nb_res = 0;
    while(fhost_get_scan_results(console_cntrl_link, nb_res++, 1, &result)) {
        char *p_ssid = "[Hidden ssid]";
        if (result.ssid.length < MAC_SSID_LEN) {
            result.ssid.array[result.ssid.length] = '\0'; // set ssid string ending
        }
        if (result.ssid.length) {
            p_ssid =  (char *)&result.ssid.array[0];
        }
        dbg("%2d:(%3d dBm) CH=%3d AKM=%3d G=%x P=%x BSSID=%02x:%02x:%02x:%02x:%02x:%02x "
            #if (PLF_AIC8800) || (PLF_AIC8800MC)
            "Format=%02x WG=%x WP=%x "
            #endif
            "SSID=%s\n",
            nb_res, (int8_t)result.rssi, phy_freq_to_channel(result.chan->band, result.chan->freq),
            result.akm, result.group_cipher, result.pairwise_cipher,
            ...
            #if (PLF_AIC8800 || PLF_AIC8800MC)
            result.format, result.wpa_group_cipher, result.wpa_pairwise_cipher,
            #endif
            p_ssid);
    }
    fhost_set_vif_type(console_cntrl_link, fvif_idx, VIF_UNKNOWN, false);
...
/* add by anyka for EVB_WBS_AIC8800 board */
// #define WAKEUP_IO_B0
// #define WAKEUP_IO_B1
#define WAKEUP_IO_B2
// #define WAKEUP_IO_B3
...
#define CONFIG_GPIOB0_1_RISE_EDGE_EN 0
...
int do_enter_hibernate(int argc, char * const argv[])
{
    ...
    if (level >= PM_LEVEL_LIGHT_SLEEP) {
        ...
        gpiob_irq_init(CONSOLE_DEEPSLEEP_IO0_INDEX, GPIOIRQ_TYPE_EDGE_RISE, gpio_irq_test_handler);
        #else
        ...
        gpiob_irq_init(CONSOLE_DEEPSLEEP_IO0_INDEX, GPIOIRQ_TYPE_EDGE_FALL, gpio_irq_test_handler);
        #endif
        ...
        gpiob_irq_init(CONSOLE_DEEPSLEEP_IO1_INDEX, GPIOIRQ_TYPE_EDGE_RISE, gpio_irq_test_handler);
        #else
        // EDGE_FALL => analog_pull_up
        anareg1_gpiob_pull_up_int_setb(CONSOLE_DEEPSLEEP_IO1_INDEX);
        anareg1_gpiob_pull_dn_int_clrb(CONSOLE_DEEPSLEEP_IO1_INDEX);
        gpiob_irq_init(CONSOLE_DEEPSLEEP_IO1_INDEX, GPIOIRQ_TYPE_EDGE_FALL, gpio_irq_test_handler);
        #endif
...
#endif
#if NX_FHOST_MONITOR
#include "fhost_console_monitor.c"
#endif
...
#define CONFIG_STA_RX_RRBREQ_BEF_CONN_EN    0

static int do_roc_cfg(int argc, char * const argv[])
{
    int ret = 0;
    int func = console_cmd_strtoul(argv[1], NULL, 0);
    if (func == 0) { // cancel roc
        dbg("cancel roc\n");
        rwnx_send_cancel_roc_req();
    } else { // set roc
        if (argc > 4) {
            int fvif_idx = (int)console_cmd_strtoul(argv[2], NULL, 0);
            int freq = (int)console_cmd_strtoul(argv[3], NULL, 0);
            uint32_t duration_ms = (uint32_t)console_cmd_strtoul(argv[4], NULL, 0);
            struct mm_remain_on_channel_cfm roc_cfm;
            #if (CONFIG_STA_RX_RRBREQ_BEF_CONN_EN)
            if (wlan_get_connect_status() == WLAN_DISCONNECT) {
                ipc_host_cntrl_start();
                struct fhost_cntrl_link *sta_cntrl_link = fhost_cntrl_cfgrwnx_link_open();
                if (sta_cntrl_link == NULL) {
                    dbg(D_ERR "Failed to open link\n");
                    ASSERT_ERR(0);
                }
                // Reset STA interface (this will end previous wpa_supplicant task)
                if (fhost_set_vif_type(sta_cntrl_link, fvif_idx, VIF_UNKNOWN, false) ||
                    fhost_set_vif_type(sta_cntrl_link, fvif_idx, VIF_STA, false)) {
                    return -1;
                }
                // set STA vif & PrbReq rx_filter
                dbg("set PrbReq rx_filter\n");
                fhost_cntrl_mm_set_filter(0x3503858C); // Bit8 (acceptProbeReq) 0 -> 1
                fhost_cntrl_cfgrwnx_link_close(sta_cntrl_link);
            }
            #endif
            dbg("set roc: fvif_idx=%d, freq=%d, duration_ms=%d\n", fvif_idx, freq, duration_ms);
            rwnx_send_roc_req(fvif_idx, freq, duration_ms, &roc_cfm);
            dbg("roc_cfm: op_code=%d, status=%d, ch_ctxt_idx=%d\n", roc_cfm.op_code, roc_cfm.status, roc_cfm.chan_ctxt_index);
        }
        else {
            dbg("invalid argc\n");
        }
...
int do_hwconfig(int argc, char * const argv[])
{
    ...
    if (hwcfg_id == ACS_TXOP_REQ) {
        ...
        if (argc == 16) {
            ...
            req.remove_1m2m     = (uint8_t)console_cmd_strtoul(argv[15], NULL, 10);
            ret = rwnx_send_vendor_hwconfig_req(&req);
        } else {
            dbg("invalid argc\n");
            ret = -2;
...
int do_swconfig(int argc, char * const argv[])
{
    int ret = 0;
    struct mm_set_vendor_swconfig_req req = {0,};
    struct mm_set_vendor_swconfig_cfm cfm = {0,};
    uint32_t swcfg_id = console_cmd_strtoul(argv[1], NULL, 10);
    if (swcfg_id == BCN_CFG_REQ) {
        if (argc == 3) {
            req.swconfig_id = swcfg_id;
            req.bcn_cfg_req.tim_bcmc_ignored_enable = (bool)console_cmd_strtoul(argv[2], NULL, 16);
    ...
    } else if (swcfg_id == TEMP_COMP_SET_REQ) {
        if (argc == 4) {
            req.swconfig_id = swcfg_id;
            req.temp_comp_set_req.enable = (uint8_t)console_cmd_strtoul(argv[2], NULL, 16);
            #if PLF_MODULE_TEMP_COMP
            req.temp_comp_set_req.reserved[0] = (uint8_t)temp_comp_calibed_temp_level_get();
            #endif
            req.temp_comp_set_req.tmr_period_ms  = (uint32_t)console_cmd_strtoul(argv[3], NULL, 10);
            ret = rwnx_send_vendor_swconfig_req(&req, &cfm);
            if (!ret) {
    ...
    } else if (swcfg_id == EXT_FLAGS_SET_REQ) {
        if (argc == 3) {
            req.swconfig_id = swcfg_id;
            req.ext_flags_set_req.user_flags = (uint32_t)console_cmd_strtoul(argv[2], NULL, 16);
            ret = rwnx_send_vendor_swconfig_req(&req, &cfm);
            if (!ret) {
    ...
    } else if (swcfg_id == EXT_FLAGS_GET_REQ) {
        if (argc == 2) {
            req.swconfig_id = swcfg_id;
            ret = rwnx_send_vendor_swconfig_req(&req, &cfm);
            if (!ret) {
    ...
    } else if (swcfg_id == EXT_FLAGS_MASK_SET_REQ) {
        if (argc == 4) {
            req.swconfig_id = swcfg_id;
            req.ext_flags_mask_set_req.user_flags_mask = (uint32_t)console_cmd_strtoul(argv[2], NULL, 16);
            req.ext_flags_mask_set_req.user_flags_val  = (uint32_t)console_cmd_strtoul(argv[3], NULL, 16);
            ret = rwnx_send_vendor_swconfig_req(&req, &cfm);
            if (!ret) {
    ...
    else if (swcfg_id == DYN_DPD_CAL_RES_SET_REQ) {
        if (argc == 5) {
            req.swconfig_id = swcfg_id;
            req.dyn_dpd_cal_res_set_req.dpd_temp_lvl = (uint8_t)console_cmd_strtoul(argv[2], NULL, 0);
            req.dyn_dpd_cal_res_set_req.set_val  = (uint8_t)console_cmd_strtoul(argv[3], NULL, 16);
            req.dyn_dpd_cal_res_set_req.set_mask = (uint8_t)console_cmd_strtoul(argv[4], NULL, 16);
            ret = rwnx_send_vendor_swconfig_req(&req, &cfm);
            if (!ret) {
                dbg("dpd_res[%d] cur_val=0x%x\n", cfm.dyn_dpd_cal_res_set_cfm.dpd_temp_lvl, cfm.dyn_dpd_cal_res_set_cfm.cur_val);
            }
        } else {
            dbg("invalid argc\n");
            ret = -2;
...
static void fhost_command_add(void)
{
    ...
    console_cmd_add("roc",      "0/1 <arg> - cancel/set roc", 2, 5, do_roc_cfg);
    #if (PLF_AIC8800M40)
    console_cmd_add("hwcfg", "wifi hwconfig", 2, 16, do_hwconfig);
    console_cmd_add("swcfg", "wifi swconfig", 2,  4, do_swconfig);
    #endif
    #if NX_FHOST_MONITOR
    fhost_monitor_command_add();
