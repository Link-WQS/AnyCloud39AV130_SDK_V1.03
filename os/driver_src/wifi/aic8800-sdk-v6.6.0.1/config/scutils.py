...
def env_init():
    ...
    # Used Network Stack Version (available values: v2_0_x, v2_1_x)
    env['NETS_VER']      = ARGUMENTS.get('NETS_VER', 'v2_0_x').lower()
    ...
    # Fhostif IPC flowctrl for SDIO interface
    env['FHOSTIF_IPC_FC'] = ARGUMENTS.get('FHOSTIF_IPC_FC', 'off').lower()
    ...
    #TINYUSB
    env['TINYUSB'] = ARGUMENTS.get('TINYUSB', 'off').lower()

    #TINYUSB demo
    env['TUSB_DEMO'] = ARGUMENTS.get('TUSB_DEMO', 'cdc_dual_ports').lower()
    ...
    #DPD
    if env['PLF'] in ['aic8800m40']:
        env['DPD'] = ARGUMENTS.get('DPD', 'on').lower()
    else:
        env['DPD'] = ARGUMENTS.get('DPD', 'off').lower()
    ...
    #******************************
    env['USER_CODE'] = ARGUMENTS.get('USER_CODE', 'src')
...
def modules_import(env, modules_list, abs_path):
    ...
    for module in modules_list:
        if isdir(join(abs_path, module, 'api')):
            inclist += [join(abs_path, module, 'api')]
        if isfile(join(abs_path, module, 'includelist.txt')):
            inclist += file_list_read(env, join(abs_path, module, 'includelist.txt'), join(abs_path, module, 'src'))
