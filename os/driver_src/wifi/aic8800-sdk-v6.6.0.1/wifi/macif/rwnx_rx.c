    ...
    if ((machdr_ptr->fctl & MAC_FCTRL_TYPE_MASK) == MAC_FCTRL_DATA_T) {
        ...
        if (co_ntohs(prot_type) == LLC_ETHERTYPE_EAP_T) { // eapol packet
            ...
            if (protocol == 1) {
                ...
                if (icmp_type == 0x08)
                    dbg("ICMP(air_rx) Request: %d\n",
                        data[protocol_offset + 11 + 7] | (data[protocol_offset + 11 + 6] << 8));
                else if (icmp_type == 0x0)
                    dbg("ICMP(air_rx) Reply:   %d\n",
                        data[protocol_offset + 11 + 7] | (data[protocol_offset + 11 + 6] << 8));
                #endif
...
uint8_t rwnx_rxdataind(void *pthis, void *hostid)
{
    ...
    if (fhost_usr_cfg.wifi_ext_flags & WIFI_EXT_PARSE_RX_DATARATE_EN_FLAG) {
        if((machdr_ptr->fctl & MAC_FCTRL_TYPE_MASK) == MAC_FCTRL_DATA_T) {
            struct vif_info_tag *mac_vif = NULL;
            uint32_t sta_idx = fhost_search_itf_idx(VIF_STA);

            if(0xFF != sta_idx) {
                mac_vif = fhost_to_mac_vif(sta_idx);
            }
            uint8_t vif_idx = (buf->info.flags & RX_FLAGS_VIF_INDEX_MSK) >> RX_FLAGS_VIF_INDEX_OFT;
            //dbg("sta_idx %d %d\r\n", sta_idx, vif_idx);
            if (mac_vif && (mac_vif->index == vif_idx)) {
                rx_data_rate_save(0, &sa.array, rxhdr);
            } else {
                rx_data_rate_save(1, &sa.array, rxhdr);
            }
        }
    }
    #endif
    ...
    if (hostif_filter_type_get() == VNET_FILTER_DIRECT) {    // If Use this filter, mcu don`t think about reorder!
        if ((hostif_mode_get() == HOST_VNET_MODE) &&
            ...
            if (!wlan_get_softap_status()) {                // STA mode
                ...
                if (pkt_type == VNET_TRANSFER_MCU_HOST) {
                    ...
                    fhost_rx_buf_forward(buf);
                    #else
                    fhost_rx_buf_forward_copy_mcu(buf);
                    send_pkg_to_host(info);
                    #endif
        ...
        // If Use this filter, mcu need think about reorder! AIC8800MC close reorder temporary.
        if ((hostif_mode_get() == HOST_VNET_MODE) && (get_hostif_wlan_status() == HOSTIF_ST_AWAKE)) {
            ...
            // hostif is not AWAKE.
            // If 1. filter rules are exist,
            //    2. the connection between hostif and cloud server does not transform into a connection
            //       between AIC8800x and cloud server,
            // then it is OK to drop pkt that is defined to transfer to hostif.
            #if 0
            uint8_t machdr_len;
            uint8_t filter_result = rwnx_rxbuf_shared_filtered(buf, &da, &machdr_len);
            if (filter_result == VNET_TRANSFER_HOST_ONLY) {
                //dbg("drop host pkt\n");
                return 0;
            }
            #endif
