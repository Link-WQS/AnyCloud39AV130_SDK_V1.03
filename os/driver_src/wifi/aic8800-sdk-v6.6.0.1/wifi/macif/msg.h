...
enum mm_msg_tag
{
    ...
    MM_SET_TXPWR_LVL_CFM = MM_SET_TXPWR_IDX_CFM,                            // 120
...
enum vendor_hwconfig_tag{
    ACS_TXOP_REQ        = 0,
    CHANNEL_ACCESS_REQ  = 1,
    MAC_TIMESCALE_REQ   = 2,
    CCA_THRESHOLD_REQ   = 3,
    BWMODE_REQ          = 4,
    VHC_RESERVED0       = 5,//CHIP_TEMP_GET_REQ,
    AP_PS_LEVEL_SET_REQ = 6,
    CUSTOMIZED_FREQ_REQ = 7,
    WAKEUP_INFO_REQ     = 8,
    KEEPALIVE_PKT_REQ   = 9,
};
...
#define USER_PWROFST_COVER_CALIB_FLAG   CO_BIT(0)
#define USER_CHAN_MAX_TXPWR_EN_FLAG     CO_BIT(1)
#define USER_TX_USE_ANA_F_FLAG          CO_BIT(2)
#define USER_APM_PRBRSP_OFFLOAD_DISABLE_FLAG    CO_BIT(3) // apm probe resp offload disable flag
#define USER_HE_MU_EDCA_UPDATE_DISABLE_FLAG     CO_BIT(4) // he mu edca params updating disable flag
#define USER_DYNAMIC_DPD_CALIB_FLAG     CO_BIT(5)
...
struct mm_set_dynamic_dpd_cal_res_req
{
    uint8_t dpd_temp_lvl; // dpd temp lvl
    uint8_t set_val;
    uint8_t set_mask;
};

struct mm_set_dynamic_dpd_cal_res_cfm
{
    uint8_t dpd_temp_lvl; // dpd temp lvl
    uint8_t cur_val;
};

struct mm_dynamic_dpd_cal_res_ind
{
    uint8_t ch_idx;
    uint8_t dpd_temp_lvl; // dpd temp lvl
    int8_t  dpd_degree_val; // dpd degree val
    uint8_t reserved;
...
struct mm_set_vendor_swconfig_req
{
    ...
    union {
        ...
        struct mm_set_dynamic_dpd_cal_res_req dyn_dpd_cal_res_set_req;
    };
};
...
struct mm_set_vendor_swconfig_cfm
{
    ...
    union {
        ...
        struct mm_set_dynamic_dpd_cal_res_cfm dyn_dpd_cal_res_set_cfm;
    };
};

struct mm_vendor_swconfig_ind
{
    uint32_t swconfig_id;
    union {
        struct mm_dynamic_dpd_cal_res_ind dyn_dpd_cal_res_ind;
    };
};
...
typedef struct {
    uint8_t enabled;
    int8_t pwrlvl_adj_tbl_2g4[3]; //ch1-4, ch5-9, ch10-13
    int8_t pwrlvl_adj_tbl_5g[6]; //ch42, ch58, ch106, ch122, ch138, ch155
