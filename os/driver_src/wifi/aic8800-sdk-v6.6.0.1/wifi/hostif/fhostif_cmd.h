...
enum CUSTOM_MSG_TAG
{
    ...
    CUSTOM_MSG_SOC_POWER_OFF_REQ,
    CUSTOM_MSG_BLEWIFI_CFG_REQ,
    CUSTOM_MSG_NET_WAKEUP_CFG_REQ,
    CUSTOM_MSG_HOSTIF_RESET_REQ,
    CUSTOM_MSG_HOST_STOP_KEEP_ALIVE_REQ,

    CUSTOM_MSG_MAX = LMAC_FIRST_MSG(TASK_CUSTOM) + 50,
};

/// driver status
typedef enum HOST_DRIVER_STATUS
{
    HOST_DRIVER_INSMOD,
    HOST_DRIVER_RMMOD,
} host_driver_t;
...
struct wifi_noblock_msg
{
    ...
    struct {
        uint8_t akm[AKM_LEN];
        uint8_t unicast_cipher[CIPHER_LEN];
        uint8_t group_cipher[CIPHER_LEN];
    } ap;
};
...
struct custom_msg_start_ap_req
{
    ...
    u8_l channel;
    u8_l akm[AKM_LEN];
    u8_l unicast_cipher[CIPHER_LEN];
    u8_l group_cipher[CIPHER_LEN];
};
...
struct custom_msg_net_wakeup_cfg
{
    u8_l server_ip[64];
    u16_l server_port;
...
extern host_driver_t driver_st;
