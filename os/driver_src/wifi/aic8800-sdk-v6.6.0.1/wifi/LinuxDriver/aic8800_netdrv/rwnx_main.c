...
char *strsep_with_escape(char **s, char ct)
{
    char *sbegin = *s;
    char *end;
    char *p;
...
int custom_msg_cmd_parse(char *command, u32 cmd_len, u32 *argc, char buf[][APP_CMD_BUF_MAX])
{
    ...
    char *cmd_cpy = NULL, *cmd_head = NULL, *token = NULL;

    cmd_head = cmd_cpy = kzalloc((cmd_len+1), GFP_KERNEL);
    if (cmd_head == NULL) {
        printk("%s: alloc err\n", __func__);
        return 0;
    }
    ...
    token = strsep_with_escape(&cmd_cpy, ' ');
...
int handle_custom_msg(struct net_device *net, char *command, u32 cmd_len)
{
    ...
    switch(ret) {
        ...
        case APP_CMD_START_AP:
            ...
            if((argc < 4) && (argc > 8)) {
                printk("Please input valid args: "
                       "custom_msg vnet0 9 ssid password band [channel] [akm unicast_cipher group_cipher]\n");
                ret = -1;
                break;
            }
            if(strlen(buf[1]) >= AP_SSID_BUF_MAX || strlen(buf[2]) >= AP_PSWD_BUF_MAX) {
            ...
            if(!strncasecmp(buf[3], "2.4G", 4))
                ...
                cust_app_cmd.ap_req.band = 0;
            cust_app_cmd.ap_req.channel = 0;
            if (argc > 4) {
                int val = 0, ret = 0;
                ret = kstrtouint(buf[4], 10, &val);
                if (!ret && (val >= 0)) {
                    if (((val >= 1) && (val <= 13))
                            || ((val >= 36) && (val <= 64))
                            || ((val >= 149) && (val <= 165)))
                        cust_app_cmd.ap_req.channel = val;
                }
            }
            strcpy(cust_app_cmd.ap_req.akm, "");
            if (argc > 5) {
                if (!strncasecmp(buf[5], "OPEN", 4))
                    strcpy(cust_app_cmd.ap_req.akm, "OPEN");
                //else if (!strncasecmp(buf[5], "WEP", 3))
                //    strcpy(cust_app_cmd.ap_req.akm, "WEP");
                else if (!strncasecmp(buf[5], "WPA/PSK", 7))
                    strcpy(cust_app_cmd.ap_req.akm, "WPA");
                else if (!strncasecmp(buf[5], "WPA2/PSK", 8))
                    strcpy(cust_app_cmd.ap_req.akm, "RSN");
                else if (!strncasecmp(buf[5], "WPA3/PSK", 8))
                    strcpy(cust_app_cmd.ap_req.akm, "SAE");
            }
            strcpy(cust_app_cmd.ap_req.unicast_cipher, "");
            if (argc > 6) {
                if (!strncasecmp(buf[6], "CCMP", 4))
                    strcpy(cust_app_cmd.ap_req.unicast_cipher, "CCMP");
                if (!strncasecmp(buf[6], "TKIP", 4))
                    strcpy(cust_app_cmd.ap_req.unicast_cipher, "TKIP");
                //if (!strncasecmp(buf[6], "WEP40", 5))
                //    strcpy(cust_app_cmd.ap_req.unicast_cipher, "WEP40");
                //if (!strncasecmp(buf[6], "WEP104", 6))
                //    strcpy(cust_app_cmd.ap_req.unicast_cipher, "WEP104");
            }
            strcpy(cust_app_cmd.ap_req.group_cipher, "");
            if (argc > 7) {
                if (!strncasecmp(buf[7], "CCMP", 4))
                    strcpy(cust_app_cmd.ap_req.group_cipher, "CCMP");
                if (!strncasecmp(buf[7], "TKIP", 4))
                    strcpy(cust_app_cmd.ap_req.group_cipher, "TKIP");
                //if (!strncasecmp(buf[7], "WEP40", 5))
                //    strcpy(cust_app_cmd.ap_req.group_cipher, "WEP40");
                //if (!strncasecmp(buf[7], "WEP104", 6))
                //    strcpy(cust_app_cmd.ap_req.group_cipher, "WEP104");
            }
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_WAITCFM;
        ...
        case APP_CMD_HOST_SET_IP:
            ...
            break;
        case APP_CMD_SOC_POWER_OFF:
            printk("APP_CMD_SOC_POWER_OFF\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_SOC_POWER_OFF_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_NOWAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = 0;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, NULL);
            break;
        case APP_CMD_BLEWIFI_CFG:
            printk("APP_CMD_BLEWIFI_CFG\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_BLEWIFI_CFG_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_NOWAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = 0;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, NULL);
            break;
        case APP_CMD_NET_WAKEUP_CFG:
            {
                int val, ret;
                printk("APP_CMD_NET_WAKEUP_CFG\n");
                cust_app_cmd.net_wakeup_cfg_req.hdr.cmd_id = CUST_CMD_NET_WAKEUP_CFG_REQ;
                strcpy(cust_app_cmd.net_wakeup_cfg_req.server_ip, "192.168.1.100");
                if (argc > 1)
                    strncpy(cust_app_cmd.net_wakeup_cfg_req.server_ip, buf[1], 64);
                cust_app_cmd.net_wakeup_cfg_req.server_port = 8080;
                if (argc > 2) {
                    ret = kstrtouint(buf[2], 10, &val);
                    if (!ret && (val >= 0))
                        cust_app_cmd.net_wakeup_cfg_req.server_port = (u16_l) val;
                }
                cust_app_cmd.net_wakeup_cfg_req.keepalive_interval_ms = 3 * 1000;
                if (argc > 3) {
                    ret = kstrtouint(buf[3], 10, &val);
                    if (!ret && (val >= 0))
                        cust_app_cmd.net_wakeup_cfg_req.keepalive_interval_ms = (u32_l) val;
                }
                strcpy(cust_app_cmd.net_wakeup_cfg_req.keepalive_msg, "keepalive");
                if (argc > 4) {
                    strncpy(cust_app_cmd.net_wakeup_cfg_req.keepalive_msg, buf[4], 64);
                }
                cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_NOWAITCFM;
                cust_app_cmd.cmd_cfm.cfm_id  = 0;
                ret = rwnx_tx_msg((u8 *)&cust_app_cmd.net_wakeup_cfg_req, sizeof(cust_app_cmd.net_wakeup_cfg_req),
                        &cust_app_cmd.cmd_cfm, NULL);
            }
            break;
        case APP_CMD_HOSTIF_RESET:
            printk("APP_CMD_HOSTIF_RESET\n");
            cust_app_cmd.common_req.cmd_id = CUST_CMD_HOSTIF_RESET_REQ;
            cust_app_cmd.cmd_cfm.waitcfm = AICWF_CMD_NOWAITCFM;
            cust_app_cmd.cmd_cfm.cfm_id  = 0;
            ret = rwnx_tx_msg((u8 *)&cust_app_cmd.common_req, sizeof(cust_app_cmd.common_req), &cust_app_cmd.cmd_cfm, NULL);
            break;
