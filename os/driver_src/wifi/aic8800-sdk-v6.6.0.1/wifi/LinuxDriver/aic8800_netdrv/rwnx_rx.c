...
void rwnx_rx_handle_msg( void *dev, struct ipc_e2a_msg *msg)
{
    ...
    switch(msg->id) {
        ...
        case CUST_CMD_HOST_KEEP_ALIVE_CFM:
            g_custom_msg_vnet.keep_alive.alive_count = g_custom_msg_vnet.keep_alive.local_count;
            //printk("host_keep_alive: %d\n", g_custom_msg_vnet.keep_alive.alive_count);
            break;
...
static bool rwnx_rx_data_skb( struct sk_buff *skb)
{
    ...
    if (skb->data == NULL) {
        txrx_err("skb data is NULL\n");
    }
    if (vnet_dev->dev_addr == NULL) {
        txrx_err("vnet_dev->dev_addr is NULL\n");
        return 0;
    }
    //skb_push(skb, 2);
