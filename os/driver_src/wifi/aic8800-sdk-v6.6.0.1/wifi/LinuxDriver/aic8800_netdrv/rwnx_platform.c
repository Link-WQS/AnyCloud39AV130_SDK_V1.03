...
void rwnx_aic_cdev_fasync_work(struct work_struct *work)
{
    ...
    while(1) {
        ...
        if (chardev.stop) {
            struct rwnx_fasync_info *n;

            list_for_each_entry_safe(fsy_info, n, &chardev.fasync_list, list) {
...
void rwnx_aic_cdev_driver_deinit(void)
{
    ...
    if (chardev.fasync_comp.wait.task_list.next) {
        chardev.stop = 1;
        rwnx_aic_fasync_ready();
...
static void stop_keep_alive(void)
{
    struct custom_msg_hdr req = {0};
    aicwf_custom_msg_cmd_cfm cmd_cfm;
    req.cmd_id = CUST_CMD_HOST_STOP_KEEP_ALIVE_REQ;
    cmd_cfm.waitcfm = AICWF_CMD_NOWAITCFM;
    cmd_cfm.cfm_id = 0;
    rwnx_tx_msg((u8 *)&req, sizeof(req), &cmd_cfm, NULL);
}
