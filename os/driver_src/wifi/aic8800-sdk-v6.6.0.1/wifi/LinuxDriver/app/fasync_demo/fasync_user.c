...
#define FASYNC_MSG_3    "3015"      // CUST_CMD_GET_WLAN_STATUS_CFM
#define FASYNC_MSG_LEN  3
...
void netaddr_copy(char *dest, char *src)
{
    int i, j;
    for (i = 0;
            ((src[i] >= '0') && (src[i] <= '9'))
    ...
    for (int i=0; i<FASYNC_MSG_LEN; i++) {
        ...
        switch (i) {
            ...
            case 2:
                ...
                if (!ptr2)
                    break;
                netaddr_copy(buff, ptr+4);
                wlan.ip = str2uint(buff);
                netaddr_copy(buff, ptr1+4);
                wlan.mk = str2uint(buff);
                sprintf(buff, "ifconfig vnet0 %d.%d.%d.%d netmask %d.%d.%d.%d &> /dev/null",
                        (unsigned int)((wlan.ip >> 0 )&0xFF), (unsigned int)((wlan.ip >> 8 )&0xFF),
                        (unsigned int)((wlan.ip >> 16)&0xFF), (unsigned int)((wlan.ip >> 24)&0xFF),
                        (unsigned int)((wlan.mk >> 0 )&0xFF), (unsigned int)((wlan.mk >> 8 )&0xFF),
                        (unsigned int)((wlan.mk >> 16)&0xFF), (unsigned int)((wlan.mk >> 24)&0xFF));
                system(buff);


                // set vnet0 gw
                netaddr_copy(buff, ptr2+4);
                wlan.gw = str2uint(buff);
                sprintf(buff, "route add default gw %d.%d.%d.%d &> /dev/null",
