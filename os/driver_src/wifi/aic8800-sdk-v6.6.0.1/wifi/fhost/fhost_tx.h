...
enum fhost_tx_msg_id
{
    ...
    /// Start TXQ associated to a VIF
    FHOST_TX_VIF_TXQ_START,
    /// Stop TXQ associated to a VIF
    FHOST_TX_VIF_TXQ_STOP,
    /// Send net elem from netif or rx task
    FHOST_TX_NET_ELEM_SEND,
};
...
int fhost_tx_net_elem_send(struct fhost_tx_task_elem *elem);
...
#ifdef CFG_FHOSTIF_IPC_FC
int fhost_ipc_fc_timer_init(void);
void fhost_ipc_fc_timer_deinit(void);
void fhost_ipc_fc_start_timer(void);
void fhost_ipc_fc_stop_timer(void);
void fhost_ipc_fc_low_set(uint32_t value);
void fhost_ipc_fc_high_set(uint32_t value);
void fhost_ipc_fc_timer_set(uint32_t value);
void fhost_lwip_tx_try_times_set(uint32_t value);
#endif /* CFG_FHOSTIF_IPC_FC */
