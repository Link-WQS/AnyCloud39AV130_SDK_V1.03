...
#define FHOST_PASSIVE_SCAN_FLAG     (0x01UL << 24)

// Channel config update flag related to country code
#define FHOST_CHAN_CC_UPD_HOST_FLAG (0x01UL << 0)
#define FHOST_CHAN_CC_UPD_EMB_FLAG  (0x01UL << 1)
...
struct fhost_vif_sta_cfg
{
    ...
    int timeout_ms;
    /**
     * Flags to enable or not
     */
    uint32_t disable_auto_reconn:1;
...
enum fhost_dynamic_txpwr_timer_st
{
    DYNAMIC_TXPWR_TIMER_NULL,
    DYNAMIC_TXPWR_TIMER_CREATE,
    DYNAMIC_TXPWR_TIMER_START,
    DYNAMIC_TXPWR_TIMER_STOP,
};

#define WIFI_EXT_PARSE_RX_DATARATE_EN_FLAG      (0x01UL << 0) // enable parsing rx data rate
#define WIFI_EXT_RF_SWITCH_EN_FLAG              (0x01UL << 1) // enable rf switch
#define WIFI_EXT_WIFI_RAM_VER_ON_FLAG           (0x01UL << 2)
#define WIFI_EXT_FAST_RAM_ON_FLAG               (0x01UL << 3) // if shutdown fast memory
#define WIFI_EXT_RETRANSMIT_VIA_APP_FLAG        (0x01UL << 4)
#define WIFI_EXT_DPD_ON_FLAG                    (0x01UL << 5)
#define WIFI_EXT_NO_WILDCARD_SCAN_FLAG          (0x01UL << 6)
#define WIFI_EXT_DYNAMIC_DPD_CALIB_EN_FLAG      (0x01UL << 7) // enable dynamic dpd calib

typedef struct {
    uint32_t ipc_irq_prio;
    uint32_t ipc_e2a_msg_buf_num;
    struct {
        char cur_cc[3]; // current country code
        uint8_t cc_upd_flags;
    };
    uint32_t wifi_ext_flags;
#if (PLF_AIC8800MC || PLF_AIC8800M40)
    uint8_t dynamic_txpwr_loss_en;
    int8_t  dynamic_txpwr_loss_value;
    uint8_t dynamic_txpwr_timer_st;
    int8_t  dynamic_txpwr_loss_lvl;
    TimerHandle_t dynamic_txpwr_timer;
#endif
} fhost_user_cfg_t;
