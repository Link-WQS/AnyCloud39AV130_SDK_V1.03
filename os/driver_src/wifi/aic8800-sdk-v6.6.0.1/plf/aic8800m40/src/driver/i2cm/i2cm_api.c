...
void i2cm_init(int i2cm_chan)
{
    ...
    #if I2CM_ABORT_ISR
    AIC_I2CM0->IC_INTR_MASK = I2CM_R_RX_UNDER | I2CM_R_RX_OVER | I2CM_R_TX_OVER | I2CM_R_TX_ABRT;
    NVIC_SetVector(I2CM_IRQn, (uint32_t)i2cm_isr_handler);
...
void i2cm_deinit(void)
{
    ...
    #if I2CM_DMA_ISR
    if (i2cm_txdma_sema) {
        rtos_semaphore_delete(i2cm_txdma_sema);
    }
    if (i2cm_rxdma_sema) {
        rtos_semaphore_delete(i2cm_rxdma_sema);
    }
...
#if I2CM_ABORT_ISR
void i2cm_reinit(void)
{
    // user need to change the init params.
    i2cm_init(I2CM_CHANNAL_1);
    i2cm_clock_setup(80, I2C_SPEED_STANDARD);
    i2cm_setMode(DEV_ADDR_MPU);
}

void i2cm_isr_handler(void)
{
    uint32_t reg_value = AIC_I2CM0->IC_INTR_STAT;
    if ( reg_value & ((1<<0) | (1<<1) | (1<<3) | (1<<6)) ) {
        dbg("i2cm_isr %x\n", reg_value);
        // step 1
        reg_value = AIC_IOMUX->pad_gpioa_0_cfg_reg & (~IOMUX_PAD_GPIOA_0_OUT_REG);
        reg_value = reg_value | (IOMUX_PAD_GPIOA_0_OUT_FRC | IOMUX_PAD_GPIOA_0_OEN_FRC);
        AIC_IOMUX->pad_gpioa_0_cfg_reg = reg_value;
        delay_about_1ns(10000);  // delay 10us

        // step 2
        reg_value = AIC_IOMUX->pad_gpioa_0_cfg_reg |
                    IOMUX_PAD_GPIOA_0_OUT_REG | IOMUX_PAD_GPIOA_0_OUT_FRC | IOMUX_PAD_GPIOA_0_OEN_FRC;
        AIC_IOMUX->pad_gpioa_0_cfg_reg = reg_value;
        delay_about_1ns(10000);  // delay 10us
...
uint8_t i2cm_byte_rd(uint8_t raddr)
{
    ...
    if((AIC_I2CM0->IC_TX_ABRT_SOURCE & I2CM_ABRT_7B_ADDR_NOACK) == I2CM_ABRT_7B_ADDR_NOACK) {
        dbg("address matches error.\n");
        AIC_I2CM0->IC_CLR_TX_ABRT;
        return 0;
    }
...
void i2cm_dma_rd(uint32_t i2c_raddr, uint32_t i2c_rlen, uint32_t *rbuff)
{
    ...
    reg = 0xffffffff;
    while((reg & I2CM_MAT_ACTIVITY) == I2CM_MAT_ACTIVITY) {
        reg = AIC_I2CM0->IC_STATUS;
    }
    // Wait dma finish
    #if I2CM_DMA_ISR
