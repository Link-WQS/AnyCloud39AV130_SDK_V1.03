...
unsigned int flash_otp_memory_lock(unsigned int lck)
{
    unsigned int ret;
    critical_section_start();
    ret = ROM_FlashOTPMemoryLock((lck & 0x0FUL) << 2);
    critical_section_end();
    return (ret >> 2);
}
 ...
 * |                         |     v
 * +=========================+  <--------------- wifi_calib_info_base
 * |                         |     ^
 * |     wifi_calib_info     |     |  84KB
 * |                         |     v
...
#define FLASH_DPD_RES_SECTOR_CNT    (21)
#define FLASH_DPD_CHGRP_SECTOR_CNT  (3)
...
#define FLASH_BOOT_INFO_OFFSET      (3 * FLASH_SECTOR_SIZE)
...
#define FLASH_BOOT_INFO_MAGIC_NUM   0x43525355  /* "USRC" */
#define FLASH_WFCAL_INFO0_MAGIC_NUM 0x30524344  /* "DCR0" */
#define FLASH_WFCAL_INFO1_MAGIC_NUM 0x31524344  /* "DCR1" */
#define FLASH_WFCAL_INFO2_MAGIC_NUM 0x32524344  /* "DCR2" */

/* boot info */
#define BOOT_INFO_USRC_VAL_ENABLE   0xA3
...
int flash_boot_info_delay_get(unsigned int *delay_ms)
{
    ...
    flash_read((void *)(sys_info_flash_addr + FLASH_BOOT_INFO_OFFSET), sizeof(boot_info_t), (void *)&boot_info);
    if (boot_info.magic_num == FLASH_BOOT_INFO_MAGIC_NUM) {
        if (boot_info.boot.delay == BOOT_INFO_USRC_VAL_ENABLE) {
            unsigned int dly_ms = boot_info.boot.delay_ms;
            *delay_ms = (dly_ms) ? (dly_ms * 5) : 400;
        } else {
...
int flash_wifi_dpd_cal_res_valid(int ch_grp_idx, int dpd_temp_lvl, int *dpd_degree_out)
{
    int ret_valid = 0;
    unsigned int info_flag_bits;
    wifi_rf_cal_info_t *wifi_rf_cal_info_fls;
    uint32_t magic_num, flag;
    int dpd_temp_sect_idx, dpd_temp_ram_idx;
    const uint32_t magic_num_array[] = {
        FLASH_WFCAL_INFO0_MAGIC_NUM,
        FLASH_WFCAL_INFO1_MAGIC_NUM,
        FLASH_WFCAL_INFO2_MAGIC_NUM,
    };
    if (!sys_info_inited) {
        flash_sys_info_init();
    }
    if ((ch_grp_idx < DPD_CHAN_GRP_2G4_5G_NUM) && (dpd_temp_lvl < DPD_TEMP_LVL_NUM)) {
        dpd_temp_sect_idx = dpd_temp_lvl / 3;
        dpd_temp_ram_idx  = dpd_temp_lvl % 3;
        wifi_rf_cal_info_fls = (wifi_rf_cal_info_t *)(sys_info_flash_addr
            - (FLASH_DPD_RES_SECTOR_CNT * FLASH_SECTOR_SIZE)
            + (ch_grp_idx * FLASH_DPD_CHGRP_SECTOR_CNT * FLASH_SECTOR_SIZE)
            + (dpd_temp_sect_idx * FLASH_SECTOR_SIZE));
        magic_num = magic_num_array[dpd_temp_sect_idx];
        flag = (0x01U << dpd_temp_ram_idx);
        if (wifi_rf_cal_info_fls->magic_num == magic_num) {
            info_flag_bits = flag & wifi_rf_cal_info_fls->info_flag;
            if (info_flag_bits) {
                ret_valid = 1;
                if (dpd_degree_out) {
                    *dpd_degree_out = wifi_rf_cal_info_fls->rf_dpd_info[dpd_temp_ram_idx].dpd_degree_val;
                }
            }
        }
    }
...
int flash_wifi_dpd_cal_res_read(int ch_grp_idx, int dpd_temp_lvl, void *buf_out)
{
    ...
    if ((ch_grp_idx < DPD_CHAN_GRP_2G4_5G_NUM) && (dpd_temp_lvl < DPD_TEMP_LVL_NUM) && buf_out) {
        dpd_temp_sect_idx = dpd_temp_lvl / 3;
        dpd_temp_ram_idx  = dpd_temp_lvl % 3;
    } else {
        return INFO_LEN_ERR;
    ...
    if (wifi_rf_cal_info_fls->magic_num == magic_num) {
        ...
        memcpy(buf_out, &wifi_rf_cal_info_fls->rf_misc_ram[dpd_temp_ram_idx], sizeof(rf_misc_ram_t));
    } else {
        return MAGIC_NUM_ERR;
    }
    return INFO_READ_DONE;
...
{
    wifi_rf_cal_info_t *wifi_rf_cal_info_fls, *wifi_rf_cal_info_ram;
    uint32_t magic_num, flag;
    int dpd_temp_sect_idx, dpd_temp_ram_idx;
    const uint32_t magic_num_array[] = {
        FLASH_WFCAL_INFO0_MAGIC_NUM,
    ...
    if ((ch_grp_idx < DPD_CHAN_GRP_2G4_5G_NUM) && (dpd_temp_lvl < DPD_TEMP_LVL_NUM) && buf_tmp && buf_in) {
        dpd_temp_sect_idx = dpd_temp_lvl / 3;
        dpd_temp_ram_idx  = dpd_temp_lvl % 3;
    } else {
        return -1;
    ...
    magic_num = magic_num_array[dpd_temp_sect_idx];
    flag = (0x01U << dpd_temp_ram_idx);
    flash_read((void *)wifi_rf_cal_info_fls, sizeof(wifi_rf_cal_info_t), buf_tmp);
    wifi_rf_cal_info_ram = (wifi_rf_cal_info_t *)buf_tmp;

    if (wifi_rf_cal_info_ram->magic_num != magic_num) {
        memset(wifi_rf_cal_info_ram, 0xFF, sizeof(wifi_rf_cal_info_t));
        wifi_rf_cal_info_ram->magic_num = magic_num;
        wifi_rf_cal_info_ram->info_flag = 0;
    }
    wifi_rf_cal_info_ram->info_flag |= flag;
    wifi_rf_cal_info_ram->rf_dpd_info[dpd_temp_ram_idx].ch_cur_idx = ch_grp_idx;
    wifi_rf_cal_info_ram->rf_dpd_info[dpd_temp_ram_idx].dpd_temp_lvl = dpd_temp_lvl;
    wifi_rf_cal_info_ram->rf_dpd_info[dpd_temp_ram_idx].dpd_degree_val = dpd_degree_val;
    memcpy(&wifi_rf_cal_info_ram->rf_misc_ram[dpd_temp_ram_idx], buf_in, sizeof(rf_misc_ram_t));
    flash_erase((void *)wifi_rf_cal_info_fls, sizeof(wifi_rf_cal_info_t));
    flash_write((void *)wifi_rf_cal_info_fls, sizeof(wifi_rf_cal_info_t), wifi_rf_cal_info_ram);
    flash_cache_invalid_range((void *)wifi_rf_cal_info_fls, sizeof(wifi_rf_cal_info_t));
    return 0;
}
...
int flash_user_data_addr_length_set(unsigned int addr, unsigned int len)
{
    ...
    if ((addr + len) > (FLASH_BASE_ADDR + flash_size - (FLASH_RESERVED_SECTOR_CNT + FLASH_DPD_RES_SECTOR_CNT) * FLASH_SECTOR_SIZE)) {
        return -3;
    }
