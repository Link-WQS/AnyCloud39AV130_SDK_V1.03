...
msadc_cal_res_t msadc_cal_res = {0,};
...
void msadc_input_sel_vrtc1_u04(void)
{
    uint32_t reg_val;
    reg_val = AIC_MSADC->cfg_msadc_sw_ctrl1 & ~(MSADC_REG_CFG_MSADC_SW_MUX_BITS_MASK | MSADC_REG_CFG_MSADC_SW_DIFF_MODE);
    AIC_MSADC->cfg_msadc_sw_ctrl1 = reg_val | (MSADC_REG_CFG_MSADC_SW_MUX_BITS(9) | 0);
    MSADC_DEBUG_PRINTF("wr       : [%p] = %08x\n", (&AIC_MSADC->cfg_msadc_sw_ctrl1), (MSADC_REG_CFG_MSADC_SW_MUX_BITS(9) | 0));
...
void msadc_cal_avdd13(void)
{
    ...
    #endif
    dbg(D_INF "Init avdd13: %d mV, reg: 0x%x\n", volt_inital, reg_inital);
...
void msadc_cal_vcore(void)
{
    ...
    #endif
    dbg(D_INF "Init vcore: %d mV, reg: 0x%x\n", volt_inital, reg_inital);
...
void msadc_cal_usb_dm(void)
{
    ...
    ana_ctrl0 |=
        (0 | // chap_sel
         ...
         0);
    sw_ctrl1 |=
        (MSADC_REG_CFG_MSADC_SW_MUX_BITS(0xC) |
         0 | // diff_mode
         0);
...
void msadc_cal_vrf_lp_vref(void)
{
    ...
    uint32_t reg_val;
    int vrf13_1, vrf13_2;
    int vrf13_reg, dslp_vrf13_ofst, dslp_vrf13_reg;
    vrf13_1 = msadc_volt_measure(MSADC_VOLT_AVDD13);
    // 0x70001000, pu lpvref
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_pd_ctrl0),
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_PU_LPVREF),
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_PU_LPVREF));
    // 0x70001000, sw lpvref set
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_pd_ctrl0),
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_SW_LPVREF_SET),
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_SW_LPVREF_SET));
    vrf13_2 = msadc_volt_measure(MSADC_VOLT_AVDD13);
    // 0x70001000, sw lpvref clr
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_pd_ctrl0),
                    (0), // sw_lpvref_set
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_SW_LPVREF_SET));
    // 0x70001000, pd lpvref
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_pd_ctrl0),
                    (0), // pu_lpvref
                    (PMU_RTC1_RTC_RG_PMU_CHOPP_BG_PU_LPVREF));
    // 0x70001030
    reg_val = psim_read((unsigned int)(&AIC_PMU_RTC1->pmu_dcdc_rf_ctrl1));
    vrf13_reg = (int)(reg_val & PMU_RTC1_RTC_RG_DCDC_RF_VBIT_NOR_TUNE_MASK);
    dslp_vrf13_ofst = (vrf13_1 - vrf13_2 + 13) / 25;
    dslp_vrf13_reg = vrf13_reg - 0x14 + dslp_vrf13_ofst;
    dbg(D_INF "vrf1=%d, vrf2=%d, nor=0x%x, ofst=%d, ds=0x%x\n",vrf13_1, vrf13_2, vrf13_reg, dslp_vrf13_ofst, dslp_vrf13_reg);
    if ((dslp_vrf13_ofst >= -6) && (dslp_vrf13_ofst <= 6)) {
        if ((dslp_vrf13_reg >= 0) && (dslp_vrf13_reg <= 15)) {
            msadc_cal_res.avdd13_lp_vref = dslp_vrf13_reg;
            msadc_cal_res.cal_res_flags |= CAL_RES_AVDD13_LP_VREF_VALID;
        }
    }
    #endif
}
...
{
    int vrtc1_mv, vrtc1_reg_ofst;
    vrtc1_mv = msadc_volt_measure(MSADC_VOLT_VRTC1);
    if (ChipIdGet(2) >= 4) {
        vrtc1_reg_ofst = (900 - vrtc1_mv) / 12;
        if (vrtc1_reg_ofst < -4) {
            vrtc1_reg_ofst = -4;
        } else if (vrtc1_reg_ofst > 4) {
            vrtc1_reg_ofst = 4;
        }
        msadc_cal_res.vrtc1_lp_reg = (uint8_t)(0x6 + vrtc1_reg_ofst);
    } else {
        vrtc1_reg_ofst = (795 - vrtc1_mv) / 10;
        if (vrtc1_reg_ofst < -6) {
            vrtc1_reg_ofst = -6;
        } else if (vrtc1_reg_ofst > 8) {
            vrtc1_reg_ofst = 8;
        }
        msadc_cal_res.vrtc1_lp_reg = (uint8_t)(0xA + vrtc1_reg_ofst);
    }
    msadc_cal_res.cal_res_flags |= CAL_RES_VRTC1_LP_REG_VALID;
    dbg(D_INF "vrtc1 volt=%d, reg=0x%x\n", vrtc1_mv, msadc_cal_res.vrtc1_lp_reg);
}
    ...
    if (vst == MSADC_VOLT_VBAT) {
        ...
        if (ChipIdGet(2) >= 4) {
            ...
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl),
                            PMU_RTC1_RTC_RG_LDO_VCORE08_IBIT_LIM(0x0),
                            PMU_RTC1_RTC_RG_LDO_VCORE08_IBIT_LIM(0x1));
            #endif
            msadc_input_sel_vrtc1_u04();
            ms_val = msadc_ms();
            #if PLF_PMIC
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl),
                            PMU_RTC1_RTC_RG_LDO_VCORE08_IBIT_LIM(0x1),
            ...
            #if PLF_PMIC
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl),
                            (PMU_RTC1_RTC_RG_LDO_VRTC08_VOSEL_NOR_TUNE(0x1f) |
                            PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_VOL(0x1) |
                            ...
                            PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_EN));
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vcore08_ctrl),
                            (PMU_RTC1_RTC_RG_LDO_VCORE08_VOSEL_NOR_TUNE(0xc) |
            ...
            psim_read((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl)); // for delay, about 87us
            #endif
            dbg(D_INF "ms_vrtc1\n");
            msadc_input_sel_vcore();
            ms_val = msadc_ms();
            #if PLF_PMIC
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vcore08_ctrl),
                            (PMU_RTC1_RTC_RG_LDO_VCORE08_VOSEL_NOR_TUNE(0x16) |
                            PMU_RTC1_RTC_RG_LDO_VCORE08_DVFS_STEP_EN |
                            PMU_RTC1_RTC_RG_LDO_VCORE08_DVFS_STEP_VOL(0x1)),
                            (PMU_RTC1_RTC_RG_LDO_VCORE08_VOSEL_NOR_TUNE_MASK |
                            PMU_RTC1_RTC_RG_LDO_VCORE08_DVFS_STEP_EN |
                            PMU_RTC1_RTC_RG_LDO_VCORE08_DVFS_STEP_VOL_MASK));
            psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl),
                            (PMU_RTC1_RTC_RG_LDO_VRTC08_VOSEL_NOR_TUNE(0x19) |
                            PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_VOL(0x1) |
                            PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_EN),
                            (PMU_RTC1_RTC_RG_LDO_VRTC08_VOSEL_NOR_TUNE_MASK |
                            PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_VOL_MASK |
        ...
        // msadc_param.mult = 1.0;
        // ms_mV = (ms_val / (float)MSADC_DENOM_REC - 1) * 1214 * msadc_param.mult;
        ret = ((int)ms_val * 1214 / MSADC_DENOM_REC - 1214) * val_mult;// Convert to mV
    }
    return ret;
}
