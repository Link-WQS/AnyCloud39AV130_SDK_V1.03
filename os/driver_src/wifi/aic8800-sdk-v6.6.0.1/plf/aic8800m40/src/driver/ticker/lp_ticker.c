...
#if (LP_TICKER_REBOOT_RECOVER_EN)
#include "flash_api.h"
#endif
...
void lp_ticker_init()
{
    uint32_t us_hi_set = 0x00UL, us_lo_set = 0x00UL;
    if (lp_ticker_inited) {
    ...
    #if (LP_TICKER_REBOOT_RECOVER_EN)
    if (pwrctrl_chipinfo_lptmr_inited_getb()) {
        uint32_t us_hi, us_lo;
        us_lo = aonsysctrl_hosttclsb_get();
        us_hi = aonsysctrl_hosttcmsb_get();
        if (us_lo || us_hi) {
            uint32_t boot_dly_ms;
            uint64_t target_us;
            int ret = flash_boot_info_delay_get((unsigned int *)&boot_dly_ms);
            if (ret != INFO_READ_DONE) {
                boot_dly_ms = 0;
            }
            target_us = ((uint64_t)us_hi << 32) + us_lo + boot_dly_ms * 1000 + LP_TICKER_REBOOT_TIME_COMP_US;
            us_hi_set = (uint32_t)(target_us >> 32);
            us_lo_set = (uint32_t)target_us;
        }
    }
    else {
        pwrctrl_chipinfo_lptmr_inited_setb();
    }
    #endif
    ...
    if (pwrctrl_pwrmd_cpusys_sw_record_getf() == CPU_SYS_POWER_ON_RESET) {
        ...
        if ((reg & 0x01UL) == 0x00UL) { // Init if disabled
            ...
            LP_TICKER_TIMER->TCH = us_hi_set;
            LP_TICKER_TIMER->TCL = us_lo_set;   // time counter
