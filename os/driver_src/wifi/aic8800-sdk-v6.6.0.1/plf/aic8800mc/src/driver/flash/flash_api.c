...
#define CALIB_INFO_RF_MISC_FLAG_BIT         (0x01UL << 7)
...
int flash_calib_rf_misc_read(rf_misc_ram_t *rf_misc)
{
    int index = 0;
    unsigned int info_flag_bits;
    calib_extra_info_t *calib_extra_info = rtos_malloc(sizeof(calib_extra_info_t));
	if (!calib_extra_info) {
		return INFO_FLAG_INVALID;
	}
    if (!sys_info_inited) {
    ...
    flash_read((void *)(sys_info_flash_addr + FLASH_CALIB_INFO_OFFSET), sizeof(calib_extra_info_t), (void *)calib_extra_info);
    if (calib_extra_info->normal_calib_info.magic_num == FLASH_CALIB_INFO_MAGIC_NUM) {
        info_flag_bits = CALIB_INFO_RF_MISC_FLAG_BIT & calib_extra_info->normal_calib_info.info_flag;
        if(info_flag_bits == 0) {
            rtos_free(calib_extra_info);
            return INFO_FLAG_INVALID;
        }

        if (info_flag_bits & CALIB_INFO_RF_MISC_FLAG_BIT) {
            memcpy((char *)rf_misc, &calib_extra_info->rf_misc_calib_val, sizeof(rf_misc_ram_t));
            index += sizeof(rf_misc_ram_t);
        }
        if (index > sizeof(rf_misc_ram_t)) {
            rtos_free(calib_extra_info);
            return INFO_LEN_ERR;
        }
    } else {
