...
struct aicbt_patch_init_desc *bt_common_get_patch_init_desc(void)
{
    ...
    if(chipid == 0){
        ...
        case 2:{
            desc = &patch_init_desc;
            desc->chip_id = AICBT_CHIP_ID_U02H;
            desc->adid_size = AICBT_ADID_SIZE_U02H;
            desc->adid_ram_base_addr = (uint8_t *)AICBT_ADID_RAM_BASE_ADDR_U02H;
            desc->adid_rom_base_addr = (uint8_t *)AICBT_ADID_ROM_BASE_ADDR_U02H;
            desc->patch_base_addr = (uint8_t *)AICBT_PATCH_BASE_ADDR_U02H;
            desc->patch_data_ptr = (uint8_t *)AICBT_PATCH_DATA_PTR_U02H;
            desc->patch_data_size = AICBT_PATCH_DATA_SIZE_U02H;
            desc->patch_table_ptr = (uint8_t *)AICBT_PATCH_TABLE_PTR_U02H;
            desc->patch_table_size = AICBT_PATCH_TABLE_SIZE_U02H;
            desc->head = NULL;
        }break;
        default:
            desc = NULL;
