...
/**
 * @brief: Sw shutdown, execute in ram
 */
void pmic_chip_shutdown_ram(void);
...
/**
 * @brief       Get the recorded rtc epoch time
 * @param[out]  sec    Get the number of seconds
 * @param[out]  usec   Get the number of microseconds
 * @return:     0 on success and != 0 otherwise
*/
int pmic_rtc_epoch_time_get(unsigned int *sec, unsigned int *usec);

/**
 * @brief       Set the rtc epoch time to record
 * @param[in]   sec    Set the number of seconds
 * @param[in]   usec   Set the number of microseconds
*/
