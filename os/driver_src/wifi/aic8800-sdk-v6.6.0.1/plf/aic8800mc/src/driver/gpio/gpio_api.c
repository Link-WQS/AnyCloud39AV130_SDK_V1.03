...
__RAMTEXT void gpio_regs_recover(void)
{
    ...
    int wakeup_bit = pwrctrl_irqstatus_get();
    for (idx = 0; idx < GPIOA_IDX_MAX; idx++) {
    ...
    int_status = AIC_GPIOA->RISR & gpio_registiers.gpio.ICR;
    if (!(wakeup_bit & ((0x01UL << 2) | (0x01UL << 4) | (0x01UL << 5) | (0x01UL << 9))) && int_status) { // add this to avoid wakeup from fall edge
        AIC_GPIOA->IRR = 0xFFFFFFFF;
    }
