    ...
    switch(param->operation)
    {
        ...
        case (GAPM_RESET):
        {
            ...
            if(param->status == GAP_ERR_NO_ERROR)
            {
                ...
                if(cmd->privacy_cfg == GAPM_PRIV_CFG_PRIV_ADDR_BIT){
                    ble_user_info.own_addr.addr[5] |= 0xC0;
                }
                    ...
                    if(ble_wakeup_user->ad_filter[i].ad_len>0 && ble_wakeup_user->ad_filter[i].ad_data_mask){
                        if(ele_type == ble_wakeup_user->ad_filter[i].ad_type && ele_len == ble_wakeup_user->ad_filter[i].ad_len){
                            ...
                            if((ad_role & AD_ROLE_FLAG) == ROLE_COMBO){
                                if(mask_is_valid && need_wakeup && (app_ble_white_list_check(i,param->trans_addr.addr))){
...
static int app_le_get_rssi_handler(ke_msg_id_t const msgid,
                                            struct gapc_con_rssi_ind *param,
                                            ke_task_id_t const dest_id,
                                            ke_task_id_t const src_id)
{
    uint8_t conidx = KE_IDX_GET(src_id);
    TRACE("%s, conidx %d, rssi %d\n",__func__,conidx,param->rssi);
    return (KE_MSG_CONSUMED);
}
...
KE_MSG_HANDLER_TAB(appm)
{
    ...
    {GAPC_CON_RSSI_IND,         (ke_msg_func_t)app_le_get_rssi_handler},
};
