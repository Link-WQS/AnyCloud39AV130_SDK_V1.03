...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,1)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(3,10,24)))
...
struct mbuf *_m_gethdr_with_skb(int how, short type, struct sk_buff *skb, int len)
{
    ...
    m = kzalloc(sizeof(struct mbuf), GFP_KERNEL);
...
{
    struct mbuf *new_m = m_gethdr(how, MT_DATA);
