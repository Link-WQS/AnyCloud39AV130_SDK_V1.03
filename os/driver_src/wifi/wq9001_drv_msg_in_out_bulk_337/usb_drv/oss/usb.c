...
wq_usb_msg_in(struct wq_udev *udev, struct wq_usbreq *req)
{
	...
	skb = dev_alloc_skb(udev->msg_mtu);
	if (!skb) {
...
#ifndef WQ_BUS_TEST
    //3. wait device ready
    left_time = wait_for_completion_timeout(&udev->op_dev_ready , HZ*2);
    if (!left_time) {
...
wq_usb_fw_download(struct wq_udev *udev)
{
    ...
    buf = kzalloc(WQ_USB_FW_DL_MTU, GFP_KERNEL);
    if (!buf)
        return -ENOMEM;
    ...
    dstate = kzalloc(sizeof(struct wq_usb_vreq_get_dstate), GFP_KERNEL);
    if (!dstate) {
...
wq_usb_fw_download(struct wq_udev *udev)
{
    ...
    //2. get rom version
    rom_ver = kzalloc(sizeof(struct wq_usb_vreq_get_rom_ver), GFP_KERNEL);
    if (!rom_ver) {
    ...
    buf = kzalloc(WQ_USB_FW_DL_MTU + tagroom, GFP_KERNEL);
    if (!buf) {
    ...
    while (len) {
        ...
        err = usb_bulk_msg(udev->usbdev,                                    //dev
                           usb_sndbulkpipe(udev->usbdev, WQ_USB_EP_TX_BE),  //pipe
                           buf,                                             //data
...
#else
    //2. load firmware
    ret = request_firmware_nowait(THIS_MODULE, true, fw_name, udev->dev, GFP_KERNEL, udev,
...
extern void indicate_wq_reset_hawk();
