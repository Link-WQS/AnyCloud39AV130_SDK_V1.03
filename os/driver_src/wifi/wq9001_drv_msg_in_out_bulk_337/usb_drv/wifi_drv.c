...
#define TXPKT_MAC_HEADROOM_NEEDED       56  //24(DESC) + 8(IV) + 24(3->11 header, 36-12)
...
extern int wq_mp_cali_efuse_notify(int set, uint8_t *cali_efuse);
...
extern int wq_mp_kfree_cali_notify(uint32_t cali_efuse);
...
int wuqi_tx_mgmt_status(void *vap_ctx_param, char *buf, int size)
{
    ...
    //TODO:cookie value?
    cfg80211_mgmt_tx_status(&ndev_data->wdev,0,buf,size,1, GFP_KERNEL);
...
int wuqi_rx_monitor(void *vap_ctx_param, void *buf, int size)
{
    struct wuqi_vif_context *vif_ctx = (struct wuqi_vif_context *)vap_ctx_param;
...
int wuqi_mic_fail_notify(void *vap_ctx_param, char *mac, int key_id)
{
    ...
    iot_printf("[%s:%d]\n",__func__,__LINE__);
    cfg80211_michael_mic_failure(vif_ctx->ndev,mac,NL80211_KEYTYPE_PAIRWISE ,key_id, NULL, GFP_KERNEL);
...
int wuqi_new_sta_notify(void *vap_ctx_param, char *mac, char *ies, int ies_len)
{
    ...
    cfg80211_new_sta(vif_ctx->ndev,mac, &sinfo ,GFP_KERNEL);
...
int wuqi_del_sta_notify(void *vap_ctx_param, char *mac)
{
    ...
    cfg80211_del_sta(vif_ctx->ndev, mac, GFP_KERNEL);
    return 0;
}
...
void update_bss_info(void *vif_ctx_cxt, char *bssid, char *ie, int ie_len, int chan, uint16_t bcn_interval, uint16_t cap, int last, int signal)
{
...
#elif ((LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,1)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(3,10,24)))
    ...
    if ((chan >= 1) && (chan <= 14))
    {
        ...
        /* also it posible to use cfg80211_inform_bss() instead of cfg80211_inform_bss_data() */
        bss = cfg80211_inform_bss(vif_ctx->wiphy, &channel, bssid, 0, cap, bcn_interval,
                ie, ie_len, signal*100, GFP_KERNEL);
#endif
...
void update_connection_info(void *vif_ctx_ctx, uint8_t *bssid, uint8_t *essid, uint8_t essid_len)
{
    ...
    if (vif_ctx->vif_ctx_state & VIF_STATE_CONNECTING)
    {
        ...
        cfg80211_connect_result(vif_ctx->ndev, bssid, NULL, 0, NULL, 0, 0, GFP_KERNEL);
...
void update_connection_fail_info(void *vif_ctx_ctx)
{
    ...
    {
        vif_ctx->vif_ctx_state &= (~VIF_STATE_CONNECTING);
        cfg80211_connect_result(vif_ctx->ndev, NULL, NULL, 0, NULL, 0, WLAN_STATUS_UNSPECIFIED_FAILURE, GFP_KERNEL);
    }
    else
    {
...
void update_disconnection_info(void *vif_ctx_ctx)
{
    ...
    if (vif_ctx->vif_ctx_state & VIF_STATE_CONNECTED)
    {
        if (vif_ctx->vif_ctx_state & VIF_STATE_SCANNING)
        {
             iot_printf("In scanning, report scanning done!\n");
            /* finish scan */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,8,0)
            cfg80211_scan_done(vif_ctx->scan_request, &info);
#else
...
#elif ((LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,1)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(3,10,24)))
        cfg80211_disconnected(vif_ctx->ndev, vif_ctx->disconnect_reason_code, NULL, 0, GFP_KERNEL);
#endif
...
void wq_update_rssi_nf(void *vap_ctx_param, uint8_t rssi, uint8_t nf)
{
    ...
    vif_ctx->iwstats.qual.level=(vif_ctx->iwstats.qual.level*7+rssi+4)/8;
...
static int wuqi_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
    ...
    memset(vif_ctx->scanreq_channels_conf, 0, WUQI_MAX_CHANNELS);
        ...
        struct cfg80211_connect_params *sme)
{
    ...
    if(sme->ie_len) {
        wuqi_vap_config_appie(vif_ctx->vap,sme->ie,sme->ie_len);
    }

    iot_printf("cipher_pairwise:0x%x, supported unicast cipher number:%d\n",
        sme->crypto.ciphers_pairwise[0], sme->crypto.n_ciphers_pairwise);
...
int wuqi_cfg80211_del_iface(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if ((vif_ctx->vif_ctx_state & VIF_STATE_CONNECTED) == VIF_STATE_CONNECTED)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,4,0)
        cfg80211_disconnected(vif_ctx->ndev, 0, NULL, 0, true, GFP_KERNEL);
#elif ((LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,1)) && (LINUX_VERSION_CODE <= KERNEL_VERSION(3,10,24)))
        cfg80211_disconnected(vif_ctx->ndev, 0, NULL, 0, GFP_KERNEL);
#endif
...
static s32
wuqi_cfg80211_start_ap(struct wiphy *wiphy, struct net_device *ndev,
...
static int create_thread()
{
    ...
    cache_log_buf1 = kmalloc(CACHE_LOG_LEN, GFP_KERNEL);
    cache_log_buf2 = kmalloc(CACHE_LOG_LEN, GFP_KERNEL);
    ...
    size_t count, loff_t *pos)
{
    ...
    if (strstr(tmp_buf, "calib set")) {
        wq_mp_cali_efuse_notify(1, NULL);
    }
    ...
    size_t count, loff_t *pos)
{
    ...
    ret = wq_mp_cali_efuse_notify(0, cali);
    if (ret) {
        printk("read efuse fail\n");
        return -1;
    }

    snprintf(numstr, sizeof(numstr), "cali efuse: 0x%08X\n", *(uint32_t *) cali);
    ...
    size_t count, loff_t *pos)
{
    ...
    if(BUILD_VERSION == 0) {
        ...
        snprintf(numstr, sizeof(numstr), "[HAWK] build version: 7.0.0.%d\n", BUILD_VERSION);
    }
    if(!copy_to_user(buffer, numstr, strlen(numstr) + 1))
    {
        *pos = 1;
                  ...
                  size_t count, loff_t *pos)
{
    ...
    if (count == 11) {
        if ((ret = kstrtol(tmp_buf, 0, &regaddr)) == 0)
        {
        ...
        if (((ret = kstrtol(tmp_buf, 0, &regaddr)) == 0) &&
            ((ret = kstrtol(tmp_buf+space_idx+1, 0, &regval))==0))
        {
...
int wq_wifi_attach(void *wq_ipc)
{
    ...
    if(BUILD_VERSION == 0) {
        ...
        printk("[HAWK] build version: 7.0.0.%d\n", BUILD_VERSION);
    }
...
int wq_mp_cali_free() {
    ...
    if (ret) {
        ...
        iot_printf("cali efuse: 0x%08X\n", *(uint32_t *) cali);

        if (cali[0]==0 && cali[1]==0 && cali[2]==0 && cali[3]==0) {
            ...
            if (IS_ERR(fp_cali_efuse)){
                ...
                if (sysfs_streq(buf_pn, "WQ9001") && sysfs_streq(buf_ver, "1.0") && strlen(buf_con)==8) {
                    ret = kstrtou32(buf_con, 16, (uint32_t *) cali);
                    iot_printf("cali efuse from WQ9001_WiFi_Mapfile.dat:0x%08X\n", *(uint32_t *) cali);
                    wq_mp_kfree_cali_notify(*(uint32_t *) cali);
                }
                else {
