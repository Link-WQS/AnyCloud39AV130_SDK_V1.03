...
extern void wq_notify_hawk_stop_and_reset(struct wq_softc *sc);
...
void wuqi_scan_req(void *param, char ssid[], uint8_t len, u16 scanreq_channels_conf[WUQI_MAX_CHANNELS], u16 scanreq_n_channels)
{
	...
	memset(channels_conf, 0, WUQI_MAX_CHANNELS);
...
void wdrv_start(void *wq_ipc)
{
    ...
    wdrv_sc = kmalloc(sizeof(struct wq_softc), GFP_KERNEL);
...
{
    wq_notify_hawk_stop_and_reset(wdrv_sc);
}
...
int wuqi_vap_config_appie(void *vap_param, const u8 *ie, int ie_len)
{
