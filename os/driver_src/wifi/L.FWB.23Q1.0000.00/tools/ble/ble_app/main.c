...
static const char *ssv_gatts_evt_name[] = {
...
static const char *ssv_gap_evt_name[] = {
...
#define EVT_ADV_REPORT_MAC_ADDR_OFT         7
#define EVT_ADV_REPORT_DATA_OFT             14

void ssv_ble_ll_packet_handler(uint8_t *rx_packet, uint32_t rx_len)
{
        ...
        switch(rx_packet[1])
        {
            case 0x3E: //LE Event Code
                ...
                switch(rx_packet[3])
                {
                    ...
                    case 0x02:
                        printf("BLE RX: LE Advertising Report.  len=%d\n",rx_len); 

                        uint8_t *p_addr = rx_packet + EVT_ADV_REPORT_MAC_ADDR_OFT;
                        
                        printf("Find:%02X:%02X:%02X:%02X:%02X:%02X\n",
                        *(p_addr+5), *(p_addr+4), *(p_addr+3), *(p_addr+2), *(p_addr+1), *p_addr);


                        uint8_t *p_adv_data=rx_packet+EVT_ADV_REPORT_DATA_OFT;
                        uint8_t *adv_name = NULL;
                        uint8_t adv_name_len = 0;
                        char str[32];                   
                        adv_name = ssv_ble_resolve_adv_data(p_adv_data, SSV_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);
                        if (adv_name_len && (adv_name_len < 30)) {
                            memcpy(str, adv_name, adv_name_len);
                            str[adv_name_len] = 0;
                            printf("Name:%s\n", str);
            ...
            case 0x0E:printf("BLE RX: Command Complete. len=%d\n", rx_len); break;
