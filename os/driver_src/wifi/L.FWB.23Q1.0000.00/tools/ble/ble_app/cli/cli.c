...
#ifdef BLE_USE_LL_ONLY
extern void ssv_ble_ll_packet_handler(uint8_t *rx_packet, uint32_t rx_len);
#endif
//----------------------------------------
...
#ifdef BLE_USE_LL_ONLY
static uint8_t adv_data_sample[] = {
            0x02, 0x01, 0x06,
            0x02, 0x0a, 0xec,
            0x03, 0x03, 0xd0, 0xfd,
...
ssv_ble_ll_adv_params_t adv_params = {
...
#endif //end of #ifdef BLE_USE_LL_ONLY
...
static void cmd_init(int argc, char *argv[])
{
    ...
    if(bInit) {
#ifdef BLE_USE_LL_ONLY
        ssv_ble_ll_set_rx_pkt_cb(ssv_ble_ll_packet_handler);
        ssv_ble_ll_init();
#else
...
#ifdef BLE_USE_LL_ONLY
        {
            ssv_ble_ll_set_advertising_data(adv_data_sample, sizeof(adv_data_sample));
            ssv_ble_ll_set_scan_rsp_data(p_rsp, rsp_len-1);
            ssv_ble_ll_start_advertising(&adv_params);
        }
#else
        //config adv data
    #ifdef OLD_PRIMARY_SERVICE
...
{
#ifdef BLE_USE_LL_ONLY
    ssv_ble_ll_scan_params_t ble_scan_params;
#else
...
#ifdef BLE_USE_LL_ONLY
    ssv_ble_ll_set_scan_params(&ble_scan_params);
#else
...
static void cmd_scanswlist(int argc, char *argv[])
{
    ...
    if(argc == 2)
    {
        if(action_val == SCAN_SWLIST_ACTION_LIST)
        {
            ret = ssv_ble_ll_set_scan_swlist(SCAN_SWLIST_ACTION_LIST, 0, 0, NULL, 0, mac_type);
        }
    ...
    else if(argc == 3)
    {
        ...
        {
            ret = ssv_ble_ll_set_scan_swlist(SCAN_SWLIST_ACTION_CLR, type_val, 0, NULL, 0, mac_type);
        }
    }
    else if(argc == 4)
    {
        ...
        {
            ret = ssv_ble_ll_set_scan_swlist(SCAN_SWLIST_ACTION_DEL, type_val, index_val, NULL, 0, mac_type);
        }
    }
    else if(argc == 5)
    {
        ...
        if(index_val >= SCAN_SWLIST_MAX_NUM)
        {
            printf("Over SCAN_SWLIST_MAX_NUM\n");
            return;
        }
		
        if(action_val == SCAN_SWLIST_ACTION_ADD && type_val == SCAN_SWLIST_TAGS_NAME) 
        {
            ret = ssv_ble_ll_set_scan_swlist(SCAN_SWLIST_ACTION_ADD, SCAN_SWLIST_TAGS_NAME, index_val, tag_str, strlen(tag_str), mac_type);
        }
    }
    else if(argc == 6)
    {
        ...
        {
            ret = ssv_ble_ll_set_scan_swlist(SCAN_SWLIST_ACTION_ADD, SCAN_SWLIST_TAGS_MAC, index_val, tag_str, 6, mac_type);
        }
    }
    if(ret < 0)
...
#ifdef BLE_USE_LL_ONLY
static void scan_timeout_handler(void)
{
    ssv_ble_ll_stop_scanning();
}
#endif
...
static void cmd_startscan(int argc, char *argv[])
{
    ...
    if(duration)
    {
        ssv_ble_ll_start_scanning();
        ssv_ble_services_set_timer(duration, scan_timeout_handler);
    }
    else
    {
        ssv_ble_ll_stop_scanning();
