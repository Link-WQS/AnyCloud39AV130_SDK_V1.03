...
int hgic_gatt_send_frag(char *att_data, int len, int tot_len, int first)
{
    ...
    int pb = first ? 0 : 1;
    int ret = -ENOMEM;
    char data[BT_LL_LENGTH+HCI_ACL_HDR_SIZE+4];

    if(len > (BT_LL_LENGTH+HCI_ACL_HDR_SIZE)){
        printf("invalid frag size: %d - %d\r\n", len, BT_LL_LENGTH+HCI_ACL_HDR_SIZE);
        return 0;
    }
...
int hgic_gatt_send_data(char *att_data, int tot_len)
{
    ...
    //hgic_dump_hex("\r\nSEND:\r\n", att_data, tot_len, 1);
    while(off < tot_len){
        flen = ((off==0)?(BT_LL_LENGTH-4):BT_LL_LENGTH);
        flen = (tot_len-off)>flen?flen:(tot_len-off);
        off += hgic_gatt_send_frag(att_data+off, flen, tot_len, (off==0));
    }
    return off;
}
