...
static int SENSOR_I2C_ADDR = 0x30;
#define ACTUAL_SENSOR_ID      0xcb5c0f
// #define SENSOR_ID             0x23310a      //sc2331x = 0x23310a
static int SENSOR_ID = 0x23310a;
...
#define MIPI_MBPS             600 //930
#define MIPI_LANES            2
#if defined(CONFIG_MACH_AK3918AV130)
static int MAX_FPS = 46;
#else
static int MAX_FPS = 50;
#endif
...
#define SLAVER_MODE_ACTIVE_STATE_RESERVED (0) //unit:row time
...
static const struct v4l2_ctrl_ops sc2331x_ctrl_ops;
static const struct v4l2_ctrl_config config_sensor_get_id = {
    .ops = &sc2331x_ctrl_ops,
    .id = SENSOR_GET_ID,
    .name = "get sensor id",
...
struct sc2331x_priv {
    ...
    u32 cfmt_code;
    const struct sc2331x_win_size* win;
    ...
    struct sc2331x_aec_parms aec_parms;
...
static struct ae_fast_struct ae_fast_default = { .sensor_exp_time = 3404,
    .sensor_a_gain = 256,
    .sensor_d_gain = 256,
    .isp_d_gain = 256,
    { .r_gain = 445,
...
static AK_ISP_SENSOR_CB sc2331x_cb;
 ...
 * @-1：不修改
 */
static int soft_addr0 = -1, soft_addr1 = -1, soft_addr2 = -1;
...
module_param(soft_addr0, int, 0644);
module_param(soft_addr1, int, 0644);
module_param(soft_addr2, int, 0644);
...
static void sc2331x_set_fps_async(struct sc2331x_priv* priv);
static int sc2331x_probe_id_func(void* arg); // no use IIC bus
static int sc2331x_read_id_func(void* arg); // no use IIC bus
static int sc2331x_get_resolution_func(
    void* arg, int* width, int* height);
static int sc2331x_set_fps_func(void* arg, const int fps);
static int sc2331x_fps_to_vts(struct sc2331x_priv* priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);
static int sc2331x_read(const struct i2c_client* client, int reg);
static int sc2331x_write(struct i2c_client* client, int reg, int value);

static u32 sc2331x_codes[] = {
 ...
 * General functions
 */
static struct sc2331x_priv* to_sc2331x(const struct i2c_client* client)
{
    return container_of(i2c_get_clientdata(client), struct sc2331x_priv, subdev);
}

static struct v4l2_subdev* ctrl_to_sd(struct v4l2_ctrl* ctrl)
{
    return &container_of(ctrl->handler, struct sc2331x_priv, hdl)->subdev;
}
 ...
 * @enable:         enable flags
 */
static int sc2331x_s_stream(struct v4l2_subdev* sd, int enable) { return 0; }

static int sc2331x_g_volatile_ctrl(struct v4l2_ctrl* ctrl)
{
 ...
 */
static int sc2331x_s_ctrl(struct v4l2_ctrl* ctrl) { return 0; }
 ...
 * @on:         power flags
 */
static int sc2331x_core_s_power(struct v4l2_subdev* sd, int on) { return 0; }
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_get_sensor_id(struct v4l2_control* ctrl)
{
    ctrl->value = sc2331x_read_id_func(NULL); // no use IIC bus
    return 0;
}
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_get_sensor_cb(
    struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);

    ctrl->value = (int)&priv->cb_info;
    return 0;
}

static int sc2331x_get_max_exp_for_fps(
    struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
    int fps = ctrl->value;
    int vts = sc2331x_fps_to_vts(priv, fps);

    ctrl->value = vts*2 - MAX_EXP_LIMIT; 	//2*{16'h320e,16’h320f}-‘d13
    return 0;
}
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_core_g_ctrl(struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    int ret;

    switch (ctrl->id) {
        case GET_SENSOR_ID:
            ret = sc2331x_get_sensor_id(ctrl);
            break;

        case GET_SENSOR_CB:
            ret = sc2331x_get_sensor_cb(sd, ctrl);
            break;

        case GET_MAX_EXP_FOR_FPS:
            ret = sc2331x_get_max_exp_for_fps(sd, ctrl);
            break;
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_set_isp_timing_cb(
    struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_set_fps_direct(
    struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
    ...
    sc2331x_set_fps_func(priv, fps);
    sc2331x_set_fps_async(priv);
    return 0;
}

static int sc2331x_set_master_or_slave(struct v4l2_subdev* sd, int is_master)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
...
static int sc2331x_set_flip_mirror(struct v4l2_subdev* sd, int enable)
{
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
    int value = 0;
    ...
    sc2331x_write(client, (0x3221), value);
 ...
 * @ctrl:           pointer to ctrl
 */
static int sc2331x_core_s_ctrl(struct v4l2_subdev* sd, struct v4l2_control* ctrl)
{
    int ret;

    switch (ctrl->id) {
        case SET_ISP_MISC_CALLBACK:
            ret = sc2331x_set_isp_timing_cb(sd, ctrl);
            break;

        case SET_FPS_DIRECT:
            ret = sc2331x_set_fps_direct(sd, ctrl);
            break;

        case SET_MASTER:
            ret = sc2331x_set_master_or_slave(sd, 1);
            break;

        case SET_SLAVER: /*for dual sensor*/
            ret = sc2331x_set_master_or_slave(sd, 0);
            break;

        case SET_FLIP_MIRROR:
            ret = sc2331x_set_flip_mirror(sd, ctrl->value);
            break;
...
static int sc2331x_store_initial_regs(struct sc2331x_priv* priv, void* arg)
{
...
static long sc2331x_core_ioctl(
    struct v4l2_subdev* sd, unsigned int cmd, void* arg)
{
    ...
    switch (cmd) {
        case AK_SENSOR_SET_INIT:
            sc2331x_store_initial_regs(priv, arg);
            break;
 ...
 */
static int sc2331x_get_fmt(struct v4l2_subdev* sd,
    struct v4l2_subdev_pad_config* cfg, struct v4l2_subdev_format* format)
{
    struct v4l2_mbus_framefmt* mf = &format->format;
    struct i2c_client* client = v4l2_get_subdevdata(sd);
    struct sc2331x_priv* priv = to_sc2331x(client);
    static struct sc2331x_win_size win = {
        .name = "default",
 ...
 */
static int sc2331x_set_fmt(struct v4l2_subdev* sd,
    struct v4l2_subdev_pad_config* cfg, struct v4l2_subdev_format* format)
{
    struct v4l2_mbus_framefmt* mf = &format->format;
 ...
 */
static int sc2331x_get_selection(struct v4l2_subdev* sd,
 ...
 */
static int sc2331x_enum_mbus_code(struct v4l2_subdev* sd,
    struct v4l2_subdev_pad_config* cfg, struct v4l2_subdev_mbus_code_enum* code)
{
    if (code->pad || code->index >= ARRAY_SIZE(sc2331x_codes))
        return -EINVAL;
 ...
 * @a:              return crop
 */
static int sc2331x_g_crop(struct v4l2_subdev* sd, struct v4l2_crop* a)
{
 ...
 */
static int sc2331x_cropcap(struct v4l2_subdev* sd, struct v4l2_cropcap* a)
{
    sc2331x_get_resolution_func(
 ...
 * @a:              return crop capbilities
 */
static int sc2331x_video_probe(struct i2c_client* client)
{
    struct sc2331x_priv* priv = to_sc2331x(client);
    int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc2331x_ctrl_ops = {
    .g_volatile_ctrl = sc2331x_g_volatile_ctrl,
    .s_ctrl = sc2331x_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc2331x_subdev_core_ops = {
    .s_power = sc2331x_core_s_power,
    .g_ctrl = sc2331x_core_g_ctrl,
    .s_ctrl = sc2331x_core_s_ctrl,
    .ioctl = sc2331x_core_ioctl,
};
 ...
 * @cfg:            return config
 */
static int sc2331x_g_mbus_config(
 ...
 * @crop:           crop to set
 */
static int sc2331x_s_crop(struct v4l2_subdev* sd, const struct v4l2_crop* crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc2331x_subdev_video_ops = {
    .s_stream = sc2331x_s_stream,
    .cropcap = sc2331x_cropcap,
    .g_crop = sc2331x_g_crop,
    .g_mbus_config = sc2331x_g_mbus_config,
    .s_crop = sc2331x_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc2331x_subdev_pad_ops = {
    .enum_mbus_code = sc2331x_enum_mbus_code,
    .get_fmt = sc2331x_get_fmt,
    .set_fmt = sc2331x_set_fmt,
    .get_selection = sc2331x_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc2331x_subdev_ops = {
    .core = &sc2331x_subdev_core_ops,
    .video = &sc2331x_subdev_video_ops,
    .pad = &sc2331x_subdev_pad_ops,
};
 ...
 * @priv:           sensor struct
 */
static int sensor_of_parse(struct i2c_client* client, struct sc2331x_priv* priv)
{
 ...
 */
static int sc2331x_match(struct sc2331x_priv* priv)
{
    int pid;
...
{
    struct sc2331x_priv* t_priv = NULL;
    struct i2c_client* client;
...
static int sc2331x_set_aec_update(struct sc2331x_priv* priv)
{
    struct i2c_client* client   = priv->client;

    //TODO:小核改了vts，读出来与MAX_FPS并不匹配，暂时先写死
    // priv->aec_parms.reg_frame_vts =
    //     (sc2331x_read(client,VTS_REG_H) << 8 | sc2331x_read(client,VTS_REG_L));
    // priv->aec_parms.reg_frame_hts =
    //     (sc2331x_read(client,HTS_REG_H) << 8 | sc2331x_read(client,HTS_REG_L)) * 2;
    priv->aec_parms.reg_frame_vts = 1125;
    priv->aec_parms.reg_frame_hts = 2200;
    ...
    priv->fps_info.current_fps = 15;
    priv->fps_info.to_fps = priv->fps_info.current_fps;
    // priv->fps_info.reg_fps_value = priv->aec_parms.reg_frame_vts;
    priv->fps_info.to_fps_value = 
        priv->aec_parms.reg_frame_vts * MAX_FPS / priv->fps_info.current_fps;
    ...
    struct i2c_client* client, const struct i2c_device_id* did)
{
    ...
    if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
        dev_err(&adapter->dev, "sc2331x: I2C-Adapter doesn't support SMBUS\n");
        return -EIO;
    }

    priv = devm_kzalloc(&client->dev, sizeof(struct sc2331x_priv), GFP_KERNEL);
    if (!priv) {
    ...
    /*subdev init*/
    v4l2_i2c_subdev_init(&priv->subdev, client, &sc2331x_subdev_ops);
    priv->subdev.flags
        |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */ V4L2_SUBDEV_FL_HAS_DEVNODE;
    v4l2_ctrl_handler_init(&priv->hdl, 2);
    v4l2_ctrl_new_std(&priv->hdl, &sc2331x_ctrl_ops, V4L2_CID_VFLIP, 0, 1, 1, 0);
    v4l2_ctrl_new_std(&priv->hdl, &sc2331x_ctrl_ops, V4L2_CID_HFLIP, 0, 1, 1, 0);
    ...
    ret = sc2331x_video_probe(client);
    if (ret < 0) {
        dev_err(&client->dev, "sc2331x probe fail\n");
        goto err_videoprobe;
    }
...
#ifdef CONFIG_SYS_FAST_BOOT
    sc2331x_set_aec_update(priv);
#endif

    priv->master_or_slave = SINGLE_MODE;

    dev_err(&adapter->dev, "sc2331x Probed success, subdev:%p\n", &priv->subdev);
 ...
 * @client:         pointor to i2c client
 */
static int sc2331x_remove(struct i2c_client* client)
{
    struct sc2331x_priv* priv = to_sc2331x(client);
...
static const struct i2c_device_id sc2331x_id[] = { { "sc2331x", 0 }, {} };
MODULE_DEVICE_TABLE(i2c, sc2331x_id);

static const struct of_device_id sc2331x_of_match[] = {
...
MODULE_DEVICE_TABLE(of, sc2331x_of_match);

static struct i2c_driver sc2331x_i2c_driver = {
    .driver = {
        .name = "sc2331x",
        .of_match_table = of_match_ptr(sc2331x_of_match),
    },
    .probe    = sc2331x_probe,
    .remove   = sc2331x_remove,
    .id_table = sc2331x_id,
};

module_i2c_driver(sc2331x_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc2331x sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.03");

//1.0.00:由sc2331筛芯片而来，200w 15fps 三目驱动，c版芯片

/*sensor i2c write*/
static int __sc2331xx_write(const struct i2c_client* client, int reg, int value)
{
    ...
    struct i2c_client* client, int reg, int value, int sensor_switch)
{
    ...
    if (sensor_switch == 1) {
        ...
        __sc2331xx_write(client_p, reg, value);

        client_p->addr = SENSOR_I2C_ADDR1;
        return __sc2331xx_write(client_p, reg, value);
    } else {
        return __sc2331xx_write(client_p, reg, value);
    }
}

static int sc2331x_write_cisx(struct sc2331x_priv* priv, int reg, int value)
{
    struct sc2331x_priv* t_priv = priv;

    switch (t_priv->subdev.devnode->num) {
        case 0:
            return _sc2331xx_write(t_priv->client, reg, value, sensor0_switch);
            break;

        case 1:
            return _sc2331xx_write(t_priv->client, reg, value, sensor1_switch);
            break;
    ...
    list_for_each_entry(t_priv, &privs_list, list)
    {
        ...
        if (t_priv->master_or_slave == SLAVER_MODE
            ...
            if (client == t_priv->client) {
                return sc2331x_write_cisx(t_priv, reg, value);
            }
        } else {
            // single CSI
            // when use Two_way_I2C + MIPI switch need write register twice.
            sc2331x_write_cisx(t_priv, reg, value);
        }
    }
    return 0;
}
...
static void sc2331x_agc_mode(struct sc2331x_priv* priv)
{
    int value;
    struct i2c_client* client = priv->client;

    value = sc2331x_read(client, 0x3e03) & 0xf0;
    value |= 0xb;
    sc2331x_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc2331x_init_func(
    void* arg, const AK_ISP_SENSOR_INIT_PARA* npara)
{
    /*cleaned_0x54_SC500AI_24MInput_MIPI_
    * 2lane_396Mbps_10bit_2880x1620_15fps.ini*/
    int i, dev_addr = 0;
    AK_ISP_SENSOR_REG_INFO* preg_info;
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;
    ...
    dev_err(&adapter->dev, "sc2331x I2c communication, subdev:%p\n", &priv->subdev);

    if ((soft_addr0 != -1) || (soft_addr1 != -1) || (soft_addr2 != -1)) {
        switch(priv->subdev.devnode->num) {
            case 0:
                soft_addr = soft_addr0;
                client->addr = addr0;
                break;
            case 1:
                soft_addr = soft_addr1;
                client->addr = addr1;
                break;
            case 2:
                soft_addr = soft_addr2;
                client->addr = addr2;
                break;
            default:
                soft_addr = -1;
    ...
    for (i = 0; i < para->num; i++) {
#if 0
        {
            ...
            value = sc2331x_read(client, preg_info->reg_addr);
            pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
        }
#endif
        if (DELAY_FLAG==preg_info->reg_addr)
            msleep(preg_info->value);
        else
            sc2331x_write(client, preg_info->reg_addr, preg_info->value);
#if 0
        {
            int value;

            value = sc2331x_read(client, preg_info->reg_addr);
            pr_err("addr:%x, reg:%x write value:%x, read back value:%x\n",
                    client->addr, preg_info->reg_addr, preg_info->value, value);
        }
#endif

        if ((soft_addr0 != -1) || (soft_addr1 != -1) || (soft_addr2 != -1)) {
            if (preg_info->reg_addr == 0x103) {
                switch(soft_addr) {
                    case -1:
                        break;
                    case 0:
                        sc2331x_write(client, 0x3004, SENSOR_I2C_ADDR1 * 2);
                        client->addr = SENSOR_I2C_ADDR1;
                        break;
                    case 1:
                        sc2331x_write(client, 0x3011, SENSOR_I2C_ADDR0 * 2);
                        client->addr = SENSOR_I2C_ADDR0;
                        break;
                    default:
                        break;
                }
...
#if AGC_EN
    sc2331x_agc_mode(priv);
#endif
    priv->aec_parms.reg_frame_vts
        = sc2331x_read(client, 0x320e) << 8 | sc2331x_read(client, 0x320f);

    priv->aec_parms.reg_frame_hts
        = sc2331x_read(client, 0x320c) << 8 | sc2331x_read(client, 0x320d);

    priv->fps_info.rb_rows
        = sc2331x_read(client, 0x3230) << 8 | sc2331x_read(client, 0x3231);
    ...
    pr_info("MarJ pclk %d vts %d hts %d calc_tmp %d\n",
    ...
    priv->aec_parms.r0x3225_merge_enable = sc2331x_read(client, 0x3225);
    priv->aec_parms.r0x3225_merge_enable &= 0x30;

#if 0
    int value;
    value = sc2331x_read(client, 0x320e)&0x7f;
    pr_err("reg:%04x , read back value:%04x\n", 0x320e, value);

    value = sc2331x_read(client, 0x320f)&0xff;
    pr_err("reg:%04x , read back value:%04x\n", 0x320f, value);

    value = sc2331x_read(client, 0x320c)&0xff;
    pr_err("reg:%04x , read back value:%04x\n", 0x320c, value);

    value = sc2331x_read(client, 0x320d)&0xff;
    pr_err("reg:%04x , read back value:%04x\n", 0x320d, value);
#endif
    return 0;
}
...
static int sc2331x_read_reg_func(void* arg, const int reg_addr)
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;

    return sc2331x_read(client, reg_addr);
}
/*write sensor register*/
static int sc2331x_write_reg_func(
    void* arg, const int reg_addr, int value)
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;

    return sc2331x_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc2331x_priv* priv)
{
...
/*set sensor again*/
static int sc2331x_update_a_gain_func(
...
#if AGAIN_EN
    sc2331x_write(client, 0x3e08, ana_gain);
    dig_fine_gain = dig_fine_gain & 0xfc;
    sc2331x_write(client, 0x3e06, dig_gain);
    sc2331x_write(client, 0x3e07, dig_fine_gain);
#endif
    return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc2331x_update_d_gain_func(
...
/*set sensor exp time*/
static int __set_reg_exp_time(struct sc2331x_priv* priv, int exp_time)
{
    int ret = 0;
    struct i2c_client* client = priv->client;
...
#if EXP_EN
    sc2331x_write(client, 0x3e00, priv->aec_parms.r0x3e00_value);
    sc2331x_write(client, 0x3e01, priv->aec_parms.r0x3e01_value);
    sc2331x_write(client, 0x3e02, priv->aec_parms.r0x3e02_value);
#endif
...
static int __set_reg_frame_vts(struct sc2331x_priv* priv, int vts)
{
    ...
    pr_debug("%s, vts:%d\n", __func__, vts);
#if FPS_EN
    sc2331x_write(client, 0x320e, (vts >> 8));
    sc2331x_write(client, 0x320f, (vts & 0xff));

    if (priv->master_or_slave == SLAVER_MODE) {
        if (priv->aec_parms.r0x3225_merge_enable==0x10) {
            sc2331x_write(client, 0x322e, (active_blank_rows >> 8));
            sc2331x_write(client, 0x322f, (active_blank_rows & 0xff));
            }
    }
...
static int __get_reg_frame_vts(struct sc2331x_priv* priv)
{
    struct i2c_client* client = priv->client;

    return (sc2331x_read(client, 0x320e)) << 8 | sc2331x_read(client, 0x320f);
}

static void update_frame_vts_and_exp_ctrl(struct sc2331x_priv* priv)
{
    struct i2c_client* client = priv->client;
    ...
    int max_exp_ctrl = priv->fps_info.to_fps_value * 2 - MAX_EXP_LIMIT; // Vts*2-13
...
/*set sensor exp time*/
static int sc2331x_updata_exp_time_func(void* arg, unsigned int exp_time)
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;
...
/*sensor timer*/
static int sc2331x_timer_func(void* arg) { return 0; }
/*standby in*/
static int sc2331x_set_standby_in_func(void* arg) { return 0; }
/*standby out*/
static int sc2331x_set_standby_out_func(void* arg) { return 0; }
/*low level read sensor ID, user i2c ops*/
static int sc2331x_probe_id_func(void* arg) // use IIC bus
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;
    int id;
    ...
    value = sc2331x_read(client, 0x3107);
    id = value << 8;

    value = sc2331x_read(client, 0x3108);
    id |= value;

    id <<= 8;
    value = sc2331x_read(client, 0x801f);
    id |= value;
...
/*get resolution*/
static int sc2331x_get_resolution_func(void* arg, int* width, int* height)
{
    *width = SENSOR_OUTPUT_WIDTH;
    *height = SENSOR_OUTPUT_HEIGHT;
    return 0;
...
/*get mclk*/
static int sc2331x_get_mclk_func(void* arg) { return SENSOR_MCLK; }
/*get current fps*/
static int sc2331x_get_fps_func(void* arg)
{
    struct sc2331x_priv* priv = arg;
...
static int sc2331x_get_valid_coord_func(void* arg, int* x, int* y)
{
...
/*get bus type*/
static enum sensor_bus_type sc2331x_get_bus_type_func(void* arg)
{
...
static int sc2331x_get_parameter_func(void* arg, int param, void* value)
{
    int ret = 0;
    struct sc2331x_priv* priv = arg;
...
/*sensor power on*/
static int sc2331x_set_power_on_func(void* arg)
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;
    struct i2c_adapter* adapter = to_i2c_adapter(client->dev.parent);
...
/*sensor power off*/
static int sc2331x_set_power_off_func(void* arg)
{
    struct sc2331x_priv* priv = arg;
    struct i2c_client* client = priv->client;
...
static int sc2331x_set_fps_func(void* arg, const int fps)
{
    ...
    tmp = sc2331x_fps_to_vts(priv, fps);
...
static AK_ISP_SENSOR_CB sc2331x_cb = {
    .sensor_init_func = sc2331x_init_func,
    .sensor_read_reg_func = sc2331x_read_reg_func,
    .sensor_write_reg_func = sc2331x_write_reg_func,
    .sensor_read_id_func = sc2331x_read_id_func,
    .sensor_update_a_gain_func = sc2331x_update_a_gain_func,
    .sensor_update_d_gain_func = sc2331x_update_d_gain_func,
    .sensor_updata_exp_time_func = sc2331x_updata_exp_time_func,
    .sensor_timer_func = sc2331x_timer_func,
    .sensor_set_standby_in_func = sc2331x_set_standby_in_func,
    .sensor_set_standby_out_func = sc2331x_set_standby_out_func,
    .sensor_probe_id_func = sc2331x_probe_id_func,
    .sensor_get_resolution_func = sc2331x_get_resolution_func,
    .sensor_get_mclk_func = sc2331x_get_mclk_func,
    .sensor_get_fps_func = sc2331x_get_fps_func,
    .sensor_get_valid_coordinate_func = sc2331x_get_valid_coord_func,
    .sensor_get_bus_type_func = sc2331x_get_bus_type_func,
    .sensor_get_parameter_func = sc2331x_get_parameter_func,
    .sensor_set_power_on_func = sc2331x_set_power_on_func,
    .sensor_set_power_off_func = sc2331x_set_power_off_func,
    .sensor_set_fps_func = sc2331x_set_fps_func,
};

static int sensor_id_func(void) { return sc2331x_read_id_func(NULL); }
