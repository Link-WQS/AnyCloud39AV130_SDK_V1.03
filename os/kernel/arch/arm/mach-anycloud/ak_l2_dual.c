...
static void __iomem *ak_l2mem;
static void __iomem *ak_l2_1_mem;
static void __iomem *ak_l2ctrl;
static void __iomem *ak_l2_1_ctrl;
static unsigned int irq_l2mem;
static unsigned int irq_l2mem1;
static void *l2_dma_vaddr = NULL;
static dma_addr_t l2_dma_handle;

#define L2_DMA_SIZE (16)

#undef AK_VA_L2MEM
#undef AK_VA_L2_1_MEM
#undef AK_VA_L2CTRL
#undef AK_VA_L2_1_CTRL

#define AK_VA_L2MEM     ak_l2mem
#define AK_VA_L2_1_MEM  ak_l2_1_mem
#define AK_VA_L2CTRL    ak_l2ctrl
#define AK_VA_L2_1_CTRL ak_l2_1_ctrl

#define IRQ_L2MEM   irq_l2mem
#define IRQ_L2MEM1  irq_l2mem1
#endif
...
#define DMA_INFO_INDEX(id)     ((id < L2_1_BUF_INDEX) ? \
              id : (id - L2_1_BUF_INDEX + L2_0_COMMON_BUFFER_NUM))
...
static bool l2_get_dma_param(unsigned int bytes, 
...
void l2_dump_registers(void)
{
    ...
    pr_info("  rL2_DMAREQ(C080)        = 0x%08X,"
        "rL2_FRACDMAADDR(C084)     = 0x%0X\n",
        __raw_readl(res + rL2_DMAREQ),
        __raw_readl(res + rL2_FRACDMAADDR));

    pr_info("  rL2_CONBUF0_7(C088) = 0x%08X, rL2_CONBUF8_15(C08C) = 0x%0X\n",
        __raw_readl(res + rL2_CONBUF0_7),
        __raw_readl(res + rL2_CONBUF8_15));

    pr_info("  rL2_BUFASSIGN1(C090)    = 0x%08X, rL2_BUFINTEN(C09C)  = 0x%0X\n",
        __raw_readl(res + rL2_BUFASSIGN1),
        __raw_readl(res + rL2_BUFINTEN));

    pr_info("  rL2_BUFSTAT1(C0A0) = 0x%08X, rL2_BUFSTAT2(C0A8)  = 0x%0X\n",
        __raw_readl(res + rL2_BUFSTAT1),
        __raw_readl(res + rL2_BUFSTAT2));
...
static inline void l2_print_array(const char *name, 
 ...
 *  @id:        L2 buffer ID
 *
 *  NOTE: Assert only L2 common buffer and UART buffer,
 *            USB buffer is not checked.
 *            Since this function is called internally by other L2 API,
 *            invalid id will cause
...
static void l2_combuf_ctrl(u8 id, bool enable)
{
    ...
    if (enable) {
        ...
        regval |= (1 << (idx + L2_COMMON_BUF_CFG_BUF_DMA_VLD_ST)) |
    ...
    pr_info("%s: idx %d 0x%p - 0x%x\n", __func__, 
        idx, (res + rL2_CONBUF0_7), __raw_readl(res + rL2_CONBUF0_7));
#endif
...
static void l2_select_combuf(l2_device_t device, u8 id)
{
    ...
    pr_info("%s: dev %d idx %d/id %d -0x%p: 0x%x\n", __func__,
        device, idx, id, buf_assignmemt, __raw_readl(buf_assignmemt));
#endif
 ...
 *          so when free a l2 buffer, 
 *          select to this device use buffer 0 as deselect
 */
static void l2_deselect_combuf(l2_device_t device, u8 id)
{
    ...
    pr_info("%s: id %d -0x%p: 0x%x\n", __func__,
        id, buf_assignmemt, __raw_readl(buf_assignmemt));
#endif
...
static void l2_clear_dma(u8 id)
{
    ...
    dmapending = __raw_readl(res + rL2_DMAREQ)
                  & (1 << (idx + L2_DMA_REQ_BUF_START));
 ...
 *                  and l2 common buffer in fraction DMA mode
 *  @ram_addr:      External RAM address(Physical)
 *  @id:        L2 buffer ID involved in DMA transfer
 *  @frac_offset:   The region offset between buffer start address 
 *                     and transfer start address
 ...
 *
 *  NOTE: Data transfer size should be 1~64Bytes, 
 *              frac_offset should be 0~7 (*64Bytes)
 */
static void l2_frac_dma(unsigned long ram_addr, u8 id, u8 frac_offset,
    unsigned int bytes, l2_dma_transfer_direction_t direction,
    bool intr_enable)
{
    ...
    pr_debug("%s(): ram_addr=0x%08X, l2 buffer id=%d,"
        "frac_offset=%d, bytes=%d, direction=%s,"
        "intr_enable=%d.\n",__func__,
        (unsigned int)ram_addr, id, frac_offset, 
        bytes, (direction == BUF2MEM)?"BUF2MEM":"MEM2BUF", intr_enable);

#ifdef L2_DEBUG_TRACE
    pr_info("%s(): ram_addr=0x%08X, l2 buffer id=%d,"
        "frac_offset=%d, bytes=%d, direction=%s, intr_enable=%d.\n",
        __func__,(unsigned int)ram_addr, id, frac_offset,
        bytes, (direction == BUF2MEM)?"BUF2MEM":"MEM2BUF", intr_enable);
#endif
    ...
    regval &= ~(L2_FRAC_DMA_LOW_ADDR_MASK | (3UL<<30))
        ; //modified by anyka chenyingyu
    ...
    if (id < L2_1_BUF_INDEX)
        bufaddr = (id < L2_0_COMMON_BUFFER_NUM) ?
            ((idx & 0x7) << 3) | (frac_offset & 0x7) :
    ...
    regval &= ~(L2_DMA_REQ_FRAC_DMA_LEN_MASK | 
        L2_DMA_REQ_FRAC_DMA_L2_ADDR_MASK |
    ...
    case MEM2BUF:
        if (bytes & 0x1)
            /* Round to even number when read data from external ram */
            bytes = bytes + 1;  
        regval |= L2_DMA_REQ_FRAC_DMA_REQ | L2_DMA_REQ_FRAC_DMA_DIR_WR |
            (bufaddr << L2_DMA_REQ_FRAC_DMA_L2_ADDR_ST) |
 ...
 *  @bytes:     L2 buffer ID
 *  @low:           CNT_cfg (bit[7:0] of DMA Operation
 *                                      Times Configuration Register)
 *  @high:      CNT_cfg_H (bit[23:16] of DMA Operation
 *                                      Times Configuration Register)
 *  Return true when correct counter value (high/low) is found,
 *     else return false.
 *
 *  NOTE: Use a simplified calculation method for
 *          L2 buffer 0~7 and 8~15 for bytes > 8KB
 */
static bool l2_get_dma_param(unsigned int bytes,
    unsigned int *low, unsigned int *high)
{
    ...
    l2_dma_transfer_direction_t direction, bool intr_enable)
{
    ...
    pr_debug("%s(): ram_addr=0x%0X, id=%d,"
        "bytes=%d, direction=%d, intr_enable=%d.\n",
        __func__, (unsigned int)ram_addr, id, bytes, direction, intr_enable);

#ifdef L2_DEBUG_TRACE
    pr_info("%s: ram_addr 0x%0X id=%d/%d bytes=%d "
        "direction=%d intr_enable %d\n",
        __func__, (unsigned int)ram_addr, id,
        DMA_INFO_INDEX(id), bytes, direction, intr_enable);
#endif
    ...
    if (l2_dma_info[DMA_INFO_INDEX(id)].dma_start 
        || l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_start) {
            pr_err("l2: unable to start dma,"
                "dma NOT finished, buf id=%d.\n", (int)id);
            return ;
    }
    ...
    if (l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_data_len > 0) {
        ...
        l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_addr = 
            (void *)(u8 *)l2_dma_info[DMA_INFO_INDEX(id)].dma_addr +
            l2_dma_info[DMA_INFO_INDEX(id)].dma_op_times* DMA_ONE_SHOT_LEN;

        l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_offset = 
            l2_dma_info[DMA_INFO_INDEX(id)].dma_op_times;
    }
    ...
    if (l2_dma_info[DMA_INFO_INDEX(id)].dma_op_times== 0) {
        ...
        l2_frac_dma(
            (unsigned long)l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_addr, id,
            l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_offset,
            l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_data_len,
    ...
    pr_info("%s ADDRBUF 0x%p:0x%x\n", __func__,
        (res + vL2DMA_ADDRBUF0 + idx * 4),
        __raw_readl(res + vL2DMA_ADDRBUF0 + idx * 4));
#endif
    ...
    pr_info("%s CONBUF0 0x%p:0x%x\n", __func__,
        (res + vL2DMA_CONBUF0 + idx * 4),
        __raw_readl(res + vL2DMA_CONBUF0 + idx * 4));
#endif
    ...
    if ((id < L2_0_COMMON_BUFFER_NUM) || (id >= L2_1_BUF_INDEX)) {
        ...
        regval |= (1 << ((id -
            L2_UART_BUF_START_ID + L2_UART_BUF_CFG_BUF_START)));
    }
    __raw_writel(regval, res + rL2_DMAREQ);
#ifdef L2_DEBUG_TRACE
    ...
    if (intr_enable) {
        ...
        if ((id < L2_0_COMMON_BUFFER_NUM) || (id >= L2_1_BUF_INDEX)) {
            ...
            regval |= 1 << (id - L2_UART_BUF_START_ID + 
                L2_DMA_INTR_ENABLE_UART_BUF_ST);
        }
        __raw_writel(regval, res + rL2_BUFINTEN);
    #ifdef L2_DEBUG_TRACE
...
static bool l2_wait_dma_finish(u8 id)
{
    ...
    if (l2_dma_info[DMA_INFO_INDEX(id)].dma_start) {
        if (id < L2_1_BUF_INDEX)
            dma_bit = (id < L2_0_COMMON_BUFFER_NUM) ? 
                (1 << (id + L2_DMA_REQ_BUF_START)) :
                (1 << (id - L2_0_COMMON_BUFFER_NUM +
                    L2_DMA_REQ_UART_BUF_REQ_START));
        else
    ...
    if (l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_start) {
        ...
        do {
            dmareq = __raw_readl(res + rL2_DMAREQ);
        } while((dmareq & L2_DMA_REQ_FRAC_DMA_REQ) &&
        ...
        if ((l2_dma_info[DMA_INFO_INDEX(id)].direction == MEM2BUF) && 
            ...
            __raw_writel(0, 
                bufaddr + 
                (l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_offset & 0x1FF) + 60);
        }
    }

    return true;
}
/*end of l2_wait_dma_finish*/
    ...
    for (i = 0; i < L2_BUF_COMM_NUM; i++) {
        ...
        if (l2_dma_info[i + idx_delta].dma_start && !dmapending) {
            if (!l2_frac_started && l2_dma_info[i + idx_delta].need_frac) {
                ...
                l2_frac_dma(
                    (unsigned long)l2_dma_info[i + idx_delta].dma_frac_addr, i,
                    l2_dma_info[i + idx_delta].dma_frac_offset, 
                    l2_dma_info[i + idx_delta].dma_frac_data_len,
                ...
                if (l2_dma_info[i + idx_delta].callback_func != NULL)
                    l2_dma_info[i + idx_delta].callback_func(
                        l2_dma_info[i + idx_delta].data);
            }
        }
    ...
    l2_dma_transfer_direction_t direction)
{
    ...
    case MEM2BUF:
        if (ram_addr % 4) {
            ...
            if (frac_no)
                __raw_writel(read_raml(ram_addr + trans_no*4),
                    (bufaddr + trans_no*4));
        }
        
        /*
         * If we do NOT write data to L2 in multiple of 64Bytes,
         * we must write something to the 4Bytes in 64Bytes-
         * boundary so that CPU knows writing ends..
         */
        if ((buf_remain > 0) && (buf_remain <= 
            L2_BUF_STATUS_MULTIPLY_RATIO - 4))
            __raw_writel(0, (bufaddr - buf_offset +
                buf_count*L2_BUF_STATUS_MULTIPLY_RATIO + 
                L2_BUF_STATUS_MULTIPLY_RATIO - 4));
        break;
    case BUF2MEM:
        if (ram_addr % 4) {
            for (i = 0; i < trans_no; i++) {
                ...
                for (j = 0; j < 4; j++)
                    write_ramb((u8)((temp_buf>>j*8) & 0xFF),
                        (ram_addr + i*4 + j));
            }
            if (frac_no) {
        ...
        /*
         * If we do NOT read data from L2 in multiple of 64Bytes,
         * we must read the 4Bytes in 64Bytes-
         * boundary so that CPU knows reading ends..
         */
        if ((buf_remain > 0) &&
                (buf_remain <= L2_BUF_STATUS_MULTIPLY_RATIO - 4))
            temp_buf = __raw_readl(bufaddr-buf_offset+buf_count*
                            L2_BUF_STATUS_MULTIPLY_RATIO+
                            L2_BUF_STATUS_MULTIPLY_RATIO - 4);
        break;
    default:
        BUG();
...
void __init l2_init(void)
{
    ...
    np = of_find_node_by_name(NULL, "l2-buffer");
    if (!np) {
        pr_err("can not find l2buffer0 node!\n");
        BUG();
    }

    retval = of_address_to_resource(np, 0, &iomem);
    if (retval) {
        pr_err("could not get l2buffer0 controller memory!\n");
        BUG();
    }

    ak_l2ctrl = ioremap(iomem.start, resource_size(&iomem));
    if (IS_ERR(ak_l2ctrl)) {
        pr_err("ak_l2ctrl devm_ioremap_resource failed %ld!\n",
            PTR_ERR(ak_l2ctrl));
        BUG();
    }

    retval = of_address_to_resource(np, 1, &iomem);
    if (retval) {
        pr_err("could not get l2buffer0 memory!\n");
        BUG();
    }

    ak_l2mem = ioremap(iomem.start, resource_size(&iomem));
    if (IS_ERR(ak_l2mem)) {
        pr_err("ak_l2mem devm_ioremap_resource failed %ld!\n",
            PTR_ERR(ak_l2mem));
        BUG();
    }

    irq_l2mem = irq_of_parse_and_map(np, 0);

    np = of_find_node_by_name(NULL, "l2-buffer_1");
    if (!np) {
        pr_err("can not find l2buffer1 node!\n");
        BUG();
    }

    retval = of_address_to_resource(np, 0, &iomem);
    if (retval) {
        pr_err("could not get l2buffer1 controller memory!\n");
        BUG();
    }

    ak_l2_1_ctrl = ioremap(iomem.start, resource_size(&iomem));
    if (IS_ERR(ak_l2_1_ctrl)) {
        pr_err("devm_ioremap_resource failed %ld!\n",
            PTR_ERR(ak_l2_1_ctrl));
        BUG();
    }

    retval = of_address_to_resource(np, 1, &iomem);
    if (retval) {
        pr_err("could not get l2buffer1 memory!\n");
        BUG();
    }

    ak_l2_1_mem = ioremap(iomem.start, resource_size(&iomem));
    if (IS_ERR(ak_l2_1_mem)) {
        pr_err("devm_ioremap_resource failed %ld!\n",
            PTR_ERR(ak_l2_1_mem));
        BUG();
    }

    irq_l2mem1 = irq_of_parse_and_map(np, 0);
    ...
    __raw_writel(L2_FRAC_DMA_AHB_FLAG_EN | L2_FRAC_DMA_LDMA_FLAG_EN,
        AK_VA_L2CTRL + rL2_FRACDMAADDR);
    __raw_writel(0x0, AK_VA_L2CTRL + rL2_CONBUF0_7);
    __raw_writel(L2_UART_BUF_CFG_UART_EN_MASK | L2_UART_BUF_CFG_UART_CLR_MASK,
        AK_VA_L2CTRL + rL2_CONBUF8_15);
    ...
    __raw_writel(L2_FRAC_DMA_AHB_FLAG_EN | L2_FRAC_DMA_LDMA_FLAG_EN,
        AK_VA_L2_1_CTRL + rL2_FRACDMAADDR);
    __raw_writel(0x0, AK_VA_L2_1_CTRL + rL2_CONBUF0_7);
    __raw_writel(L2_UART_BUF_CFG_UART_EN_MASK | L2_UART_BUF_CFG_UART_CLR_MASK,
        AK_VA_L2_1_CTRL + rL2_CONBUF8_15);
    ...
    if ((retval = request_irq(IRQ_L2MEM, &l2_interrupt_handler,
                     0/*IRQF_DISABLED*/, "l2_0", (void *)L2_BUF_0)) < 0)
        pr_err("l20: failed to request_irq, irq number: %d, retval=%d.\n",
            IRQ_L2MEM, retval);

    if ((retval = request_irq(IRQ_L2MEM1, &l2_interrupt_handler,
                    0/*IRQF_DISABLED*/, "l2_1", (void *)L2_BUF_1)) < 0)
        pr_err("l20: failed to request_irq, irq number: %d, retval=%d.\n",
            IRQ_L2MEM1, retval);

    l2_dma_vaddr = dma_alloc_coherent(NULL, L2_DMA_SIZE,
                        &l2_dma_handle, GFP_KERNEL);
    if (!l2_dma_vaddr ) {
        pr_err("can not dma alloc size:%d!\n", L2_DMA_SIZE);
        BUG();
    }
 ...
 *  Other L2 buffers (UART/USB used) is handled
 *  by corresponding devices directly.
 */
static u8 __l2_alloc(l2_device_t device, bool need_wait)
{
    ...
    if (l2_buffer_info[l2_get_buffer_info_idx(id)].used_time == 0) {
        /*
         * In case when the new allocated L2 buffer has 
         * been used MAX_L2_BUFFER_USED_TIMES,
         * we just clear all L2 buffer used times as a 
         * simpfied method of balancing 8 L2 buffer usage.
         */
        for (i = 0; i < L2_COMMON_BUFFER_NUM; i++)
            l2_buffer_info[i].used_time = 0;
    }
 ...
 *  Other L2 buffers (UART/USB used) is handled by
 *  corresponding devices directly.
 *  NOTE: Return the previous L2 buffer ID if a L2 buffer 
 *            has been allocated to the device.
...
/**
 * l2_set_dma_callback - Set callback function when L2 DMA/fraction DMA
 *                           interrupt handler is done
 *  @id:    L2 buffer ID
 *  @func:  Callback function
 *  Return true(Always)
 *  
 *  NOTE: Caller MUST guarantee that L2 buffer ID is valid.
 *  And since the callback function is called
...
bool l2_set_dma_callback(u8 id, l2_callback_func_t func, unsigned long data)
{
    ...
    if (unlikely(l2_dma_info[DMA_INFO_INDEX(id)].dma_start ||
        l2_dma_info[DMA_INFO_INDEX(id)].dma_frac_start)) {
 ...
 *                  and l2 common buffer in DMA mode
 *  @ram_addr:      External RAM address(Physical)
 *  @id:        L2 buffer ID involved in DMA transfer
 *  @bytes:     Data transfer size
 *  @direction:     Data transfer direction between L2 memory and external RAM 
 *  @intr_enable:   Open interrupt for this L2 buffer or not
 */
void l2_combuf_dma(unsigned long ram_addr, u8 id, unsigned int bytes,
    l2_dma_transfer_direction_t direction, bool intr_enable)
{
    if (unlikely(id >= L2_BUF_MAX)) {
 ...
 *                  and l2 common buffer in CPU mode
 *  @ram_addr:  External RAM address(Physical)
 *  @id:    L2 buffer ID
 *  @bytes: Data transfer size
 *  @direction: Data transfer direction between L2 memory and external RAM
 *
 *  NOTE: According to XuChang, if one transfer data
 *            from Peripheral --> L2 Buffer --> RAM, 
 *            special care need to be taken
 *            when data size is NOT multiple of 64Bytes.
 *            Pheripheral driver must check hardware signals
 *            to confirm data has been transfer from
 *            peripheral to L2 buffer since L2 do NOT 
 *            provide some mechanism to confirm data has
 *            been in L2 Buffer. Driver can and only can
 *            call l2_combuf_cpu() to copy data from L2
 *            Buffer --> RAM after checking hardware signals.
 *            As to 64Bytes * n size data, L2 could check Buffer Status
 *            Status Counter to confirm that
 *            Data has been transfer from peripheral to L2 buffer,
 *            so no hardware signals checking needed.
 */
    ...
    unsigned int bytes, l2_dma_transfer_direction_t direction)
{
    ...
    case MEM2BUF:
        for (i = 0; i < loop; i++) {
            ...
            while (l2_get_status(id) == (L2_BUFFER_SIZE /
                                         L2_BUF_STATUS_MULTIPLY_RATIO))
                ;   /* Waiting for L2 buffer to NOT full(means writable) */
            
            l2_cpu(ram_addr + i * L2_BUF_STATUS_MULTIPLY_RATIO, id,
                (i % 8) * L2_BUF_STATUS_MULTIPLY_RATIO,
                L2_BUF_STATUS_MULTIPLY_RATIO, direction);
        }
        if (remain > 0) {
...
u8 l2_get_status(u8 id)
{
    ...
    if (id < L2_1_BUF_INDEX) {
        return (id < L2_0_COMMON_BUFFER_NUM) ?
            (__raw_readl(res + rL2_BUFSTAT1) >> (id * 4)) & 0xF :
            (__raw_readl(res + rL2_BUFSTAT2) >> (
            (id - L2_0_COMMON_BUFFER_NUM) << 1)) & 0x3;
    } else {
        return (id < L2_2_BUF_INDEX) ? 
                (__raw_readl(res + rL2_BUFSTAT1) >> (idx * 4)) & 0xF : 0;
    }
}
...
void l2_clr_status(u8 id)
{
    ...
    if ((id < L2_0_COMMON_BUFFER_NUM) || (id >= L2_1_BUF_INDEX)) {
        __raw_writel(__raw_readl(res + rL2_CONBUF0_7) |
            (1 << (idx + L2_COMMON_BUF_CFG_BUF_CLR_START)),
            res + rL2_CONBUF0_7);
    } else {
        __raw_writel(__raw_readl(res + rL2_CONBUF8_15) |
            (1 << (id - L2_UART_BUF_START_ID + L2_UART_BUF_CFG_BUF_START)),
            res + rL2_CONBUF8_15);
    }
    ...
    pr_info("%s: idx %d /id %d - 0x%p 0x%x\n", __func__,
        idx, id, (res + rL2_CONBUF0_7), __raw_readl(res + rL2_CONBUF0_7));
#endif
...
void l2_set_status(u8 id, u8 status)
{
    ...
    regval &= ~(L2_UART_BUF_CFG_CPU_BUF_NUM_MASK |
        L2_UART_BUF_CFG_CPU_BUF_SEL_EN |
        L2_UART_BUF_CFG_CPU_BUF_SEL_MASK);
    regval |= ((idx & 0x7) << L2_UART_BUF_CFG_CPU_BUF_SEL_ST) |
        L2_UART_BUF_CFG_CPU_BUF_SEL_EN |
        (status << L2_UART_BUF_CFG_CPU_BUF_NUM_ST);
...
void l2_before_dma_out(void)
{
    int i;
    unsigned int buf[L2_DMA_SIZE>>2];
    volatile unsigned int *p = l2_dma_vaddr;

    for (i = 0; i < L2_DMA_SIZE>>2; i++)
        buf[i] = p[i];
}
EXPORT_SYMBOL(l2_before_dma_out);
