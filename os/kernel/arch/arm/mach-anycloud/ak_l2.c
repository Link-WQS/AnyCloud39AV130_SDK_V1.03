...
void l2_dump_registers(void)
{
    ...
    pr_debug("  rL2_DMAREQ(C080)        = 0x%08X,"
                "rL2_FRACDMAADDR(C084)     = 0x%0X\n",
                __raw_readl(rL2_DMAREQ),
                __raw_readl(rL2_FRACDMAADDR));
    pr_debug("  rL2_CONBUF0_7(C088) = 0x%08X,"
                "rL2_CONBUF8_15(C08C) = 0x%0X\n",
                __raw_readl(rL2_CONBUF0_7),
                __raw_readl(rL2_CONBUF8_15));
    pr_debug("  rL2_BUFASSIGN1(C090)    = 0x%08X,"
                "rL2_BUFINTEN(C09C)  = 0x%0X\n",
                __raw_readl(rL2_BUFASSIGN1),
                __raw_readl(rL2_BUFINTEN));
    pr_debug("  rL2_BUFSTAT1(C0A0)      = 0x%08X,"
                "rL2_BUFSTAT2(C0A8)    = 0x%0X\n",
                __raw_readl(rL2_BUFSTAT1),
                __raw_readl(rL2_BUFSTAT2));
...
static void l2_combuf_ctrl(u8 id, bool enable)
{
    ...
    if (enable) {
        ...
        regval |= (1 << (id + L2_COMMON_BUF_CFG_BUF_DMA_VLD_ST)) |
...
static void l2_select_combuf(l2_device_t device, u8 id)
{
    ...
    if ((u8)device < 10) {
        /*
         * USB Bulkout ~ DAC (Device 0 ~ 9) is controlled
         * by Buffer Assignment Register 1
         */
        ...
        /*
         * SPI2 Rx ~ ADC (Device 10 ~ 14) is controlled
         * by Buffer Assignment Register 2
         */
...
static void l2_clear_dma(u8 id)
{
    ...
    if(l2_dma_info[id].direction == BUF2MEM) {
        ...
        while (dmapending) {
            l2_set_status(id, 8);
            dmapending = __raw_readl(rL2_DMAREQ) &
...
/**
 * l2_frac_dma - Start data tranferring between memory and
 *                  l2 common buffer in fraction DMA mode
 *  @ram_addr:      External RAM address(Physical)
 *  @id:        L2 buffer ID involved in DMA transfer
 *  @frac_offset:   The region offset between buffer start address and
    ...
    bool intr_enable)
{
    ...
    pr_debug("%s(): ram_addr=0x%08X, l2 buffer id=%d,"
        ...
        frac_offset, bytes,
        (direction == BUF2MEM)?"BUF2MEM":"MEM2BUF",
        intr_enable);
    ...
    regval &= ~(L2_FRAC_DMA_LOW_ADDR_MASK |
                   (3UL<<30)); //modified by anyka chenyingyu
    regval |= (ram_addr & L2_FRAC_DMA_LOW_ADDR_MASK) | highaddr;
    __raw_writel(regval, rL2_FRACDMAADDR);

    /* Set fraction DMA address */
    bufaddr = (id < L2_COMMON_BUFFER_NUM) ?
        ((id & 0x7) << 3) | (frac_offset & 0x7) :
    ...
    case MEM2BUF:
        if (bytes & 0x1)
            bytes = bytes + 1;  /* Round to even number 
                                when read data from external ram */
    ...
    l2_dma_transfer_direction_t direction, bool intr_enable)
{
    ...
    pr_debug("%s(): ram_addr=0x%0X, id=%d, bytes=%d,"
    ...
    if (intr_enable) {
        ...
        if (id < L2_COMMON_BUFFER_NUM) {
            ...
            regval |= 1 << (id - L2_COMMON_BUFFER_NUM +
...
static bool l2_wait_dma_finish(u8 id)
{
    ...
    if (l2_dma_info[id].dma_start) {
        dma_bit = (id < L2_COMMON_BUFFER_NUM) ?
            (1 << (id + L2_DMA_REQ_BUF_START)) :
    ...
    if (l2_dma_info[id].dma_frac_start) {
        ...
        if ((l2_dma_info[id].direction == MEM2BUF) && 
            ...
            __raw_writel(0, bufaddr +
                (l2_dma_info[id].dma_frac_offset & 0x1FF) + 60);
        ...
        if (l2_dma_info[i].dma_start && !dmapending) {
            if (!l2_frac_started && l2_dma_info[i].need_frac) {
                ...
                l2_frac_dma((unsigned long)l2_dma_info[i].dma_frac_addr, i,
                    l2_dma_info[i].dma_frac_offset,
                    l2_dma_info[i].dma_frac_data_len,
    ...
    l2_dma_transfer_direction_t direction)
{
    ...
    case MEM2BUF:
        ...
        if ((buf_remain > 0) &&
            ...
            __raw_writel(0, (bufaddr - buf_offset + buf_count*
...
#if defined(CONFIG_PM) && (defined(CONFIG_MACH_KM01A) || defined(CONFIG_MACH_AK3918AV130))

static struct ak_l2_save_regs {
    u32 dma_en;
    u32 frac_dma_addr;
    u32 buf0_7;
    u32 buf8_15;
...
static int l2_suspend(void)
{
    int i;
    struct ak_l2_save_regs *regs = &ak_l2_regs;

    for (i = 0; i < ARRAY_SIZE(l2_dma_info); i++) {
        if (l2_dma_info[i].dma_start || l2_dma_info[i].dma_frac_start) {
            pr_info("wait l2buf %d dma finish\n", i);
            l2_wait_dma_finish(i);
        }
    }
    for (i = 0; i < ARRAY_SIZE(l2_buffer_info); i++) {
        if (l2_buffer_info[i].usable != L2_STAT_IDLE) {
            pr_info("l2buf %d is in use\n", l2_buffer_info[i].id);
        }
    }

    regs->dma_en = __raw_readl(rL2_DMAREQ) & L2_DMA_REQ_EN;
    regs->frac_dma_addr = __raw_readl(rL2_FRACDMAADDR);
    regs->buf0_7 = __raw_readl(rL2_CONBUF0_7);
    regs->buf8_15 = __raw_readl(rL2_CONBUF8_15);
    regs->buf_assign0 = __raw_readl(rL2_BUFASSIGN1);
    regs->buf_assign1 = __raw_readl(rL2_BUFASSIGN2);
    regs->buf_int_en = __raw_readl(rL2_BUFINTEN);

    pr_info("l2 suspend ok\n");
    return 0;
}
...
static void l2_resume(void)
{
    ...
    __raw_writel(regs->dma_en, rL2_DMAREQ);
    __raw_writel(regs->frac_dma_addr, rL2_FRACDMAADDR);
    __raw_writel(regs->buf0_7, rL2_CONBUF0_7);
    __raw_writel(regs->buf8_15, rL2_CONBUF8_15);
    __raw_writel(regs->buf_assign0, rL2_BUFASSIGN1);
    __raw_writel(regs->buf_assign1, rL2_BUFASSIGN2);
    __raw_writel(regs->buf_int_en, rL2_BUFINTEN);

    pr_info("l2 resume ok\n");
}

static struct syscore_ops l2_syscore_ops = {
    .resume		= l2_resume,
    .suspend	= l2_suspend,
};
#endif
...
void __init l2_init(void)
{
    ...
    __raw_writel(L2_FRAC_DMA_AHB_FLAG_EN |
        L2_FRAC_DMA_LDMA_FLAG_EN, rL2_FRACDMAADDR);
    __raw_writel(0x0, rL2_CONBUF0_7);
    __raw_writel(L2_UART_BUF_CFG_UART_EN_MASK |
        L2_UART_BUF_CFG_UART_CLR_MASK, rL2_CONBUF8_15);
    ...
    if ((retval = request_irq(IRQ_L2MEM, &l2_interrupt_handler,
                        0/*IRQF_DISABLED*/, "l2", NULL)) < 0)
        pr_err("l2: failed to request_irq,"
            "irq number: %d, retval=%d.\n", IRQ_L2MEM, retval);
#if !defined(CONFIG_MACH_KM01A) && !defined(CONFIG_MACH_AK3918AV130)
...
#if defined(CONFIG_PM) && (defined(CONFIG_MACH_KM01A) || defined(CONFIG_MACH_AK3918AV130))
    register_syscore_ops(&l2_syscore_ops);
#endif
...
static u8 __l2_alloc(l2_device_t device, bool need_wait)
{
...
#elif defined(CONFIG_MACH_AK39EV330)
    if (unlikely((device == ADDR_RESERVED0) ||
        (device == ADDR_RESERVED1) ||
        (device == ADDR_RESERVED2)))
#elif defined(CONFIG_MACH_KM01A) || defined(CONFIG_MACH_AK3918AV130)
    if (unlikely((device == ADDR_RESERVED0) ||
        (device == ADDR_RESERVED1) ||
        (device == ADDR_RESERVED2) ||
        (device == ADDR_RESERVED3)))
#endif
    {
    ...
    if (l2_buffer_info[id].used_time == 0) {
        /*
         * In case when the new allocated L2 buffer has been
         *     used MAX_L2_BUFFER_USED_TIMES,
         * we just clear all L2 buffer used times as
         *     a simpfied method of balancing 8 L2 buffer usage.
         */
        for (i = 0; i < L2_COMMON_BUFFER_NUM; i++)
            l2_buffer_info[i].used_time = 0;
    }
...
/**
 * l2_set_dma_callback - Set callback function when
 *  L2 DMA/fraction DMA interrupt handler is done
 *  @id:    L2 buffer ID
...
bool l2_set_dma_callback(u8 id, l2_callback_func_t func, unsigned long data)
{
    ...
    if (unlikely(l2_dma_info[id].dma_start ||
        l2_dma_info[id].dma_frac_start)) {
...
/**
 * l2_combuf_dma - Start data tranferring between memory and
 *                  l2 common buffer in DMA mode
 *  @ram_addr:      External RAM address(Physical)
 *  @id:        L2 buffer ID involved in DMA transfer
 *  @bytes:     Data transfer size
 *  @direction:     Data transfer direction between L2 memory and external RAM 
 ...
 *  @direction: Data transfer direction between L2 memory and external RAM
 *
 *  NOTE: According to XuChang, if one transfer
 *                data from Peripheral --> L2 Buffer --> RAM, 
 *            special care need to be taken when
 *                data size is NOT multiple of 64Bytes.
 *            Pheripheral driver must check hardware signals to
 *                confirm data has been transfer from
 *            peripheral to L2 buffer since L2 do NOT provide
 *                some mechanism to confirm data has
 *            been in L2 Buffer. Driver can and only can call l2_combuf_cpu()
 *                to copy data from L2
    ...
    case MEM2BUF:
        for (i = 0; i < loop; i++) {
            ...
            while (l2_get_status(id) ==
                    (L2_BUFFER_SIZE / L2_BUF_STATUS_MULTIPLY_RATIO))
...
void l2_combuf_cpu_frac(u32 ram_addr, u8 buf_id, u32 loop, u32 remain, u8 tran_dir)
{
   l2_cpu(ram_addr+loop*64, buf_id, (loop%8)*64, remain, tran_dir);
}
EXPORT_SYMBOL(l2_combuf_cpu_frac);
#endif
...
u8 l2_get_status(u8 id)
{
    ...
    return (id < L2_COMMON_BUFFER_NUM) ?
        (__raw_readl(rL2_BUFSTAT1) >> (id * 4)) & 0xF :
...
void l2_clr_status(u8 id)
{
    ...
    if (id < L2_COMMON_BUFFER_NUM) {
        __raw_writel(__raw_readl(rL2_CONBUF0_7) |
            (1 << (id + L2_COMMON_BUF_CFG_BUF_CLR_START)), rL2_CONBUF0_7);
    } else {
        __raw_writel(__raw_readl(rL2_CONBUF8_15) |
...
void l2_set_status(u8 id, u8 status)
{
    ...
    regval |= (id << L2_UART_BUF_CFG_CPU_BUF_SEL_ST) |
        L2_UART_BUF_CFG_CPU_BUF_SEL_EN |
...
u32 l2_get_dma_trans_len(u8 id)
{
    ...
    reg = __raw_readl(vL2DMA_CONBUF0 + id * 4);
