...
static void work_func(struct work_struct *work)
{
	...
	send_len = sprintf(buf, "send cnt:%d", cnt++);

	/* send a message to our remote processor */
	ret = rpmsg_send(rd->ept, buf, send_len);
	if (ret)
		dev_err(&rd->dev, "rpmsg_send %d failed: %d\n", send_len, ret);
