 ...
 */
  ...
  * 4. copy Documentation/spi/spi_master_test and 
  *    Documentation/spi/spi_slave_test to Your_rootfs_dir
  * 5. firstly run spi_slave_test in slave 
  *    and then run spi_master_test in master
  */
...
{
    struct spi_anyka_slave *slave = container_of(node->i_cdev,
                struct spi_anyka_slave, cdev);
    ...
    slave->readp = slave->recvp = slave->rdbuf;
    slave->openers++;
...
*  @return          fail or not
*/
static long akspi_slave_ioctl(struct file *filp,
                    unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd){
     ...
     case SPI_IOC_RD_LSB_FIRST:
         retval = __put_user((slave->mode & SPI_LSB_FIRST) ?
                        1 : 0, (__u8 __user *)arg);
         break;
...
*/
static ssize_t akspi_slave_read(struct file *filp, char __user *buf,
                    size_t count, loff_t *f_pos)
{
    ...
    while (slave->recvp == slave->readp){
        ...
        if (wait_event_interruptible(slave->readq,
                (slave->recvp != slave->readp))){
            return -ERESTARTSYS;
        }
        if (mutex_lock_interruptible(&slave->slave_lock)){
            return -ERESTARTSYS;
...
*/
static ssize_t akspi_slave_write(struct file *filp, const char __user *buf,
                    size_t count, loff_t *f_pos)
{
    struct spi_anyka_slave *slave = filp->private_data;
    ...
    while (slave->sentp != slave->writep){
        ...
        if (wait_event_interruptible(slave->writeq,
                    slave->sentp != slave->writep)){
            return -ERESTARTSYS;
        }
        if (mutex_lock_interruptible(&slave->slave_lock)){
            return -ERESTARTSYS;
...
*  @return          fail or not
*/
unsigned int akspi_slave_poll (struct file *filp,
...
static int ak_spi_slave_probe(struct platform_device *pdev)
{
    ...
    slave->ioarea = request_mem_region(res->start,
    ...
    err = request_irq(slave->irq, akspi_slave_int,
                IRQF_DISABLED, pdev->name, slave);
    if (err < 0) {
...
{
    struct spi_anyka_slave * = dev_get_drvdata(dev);
    pr_info("%s: resume\n",  slave->pdev->name);
...
static ssize_t slave_show_version(struct class *cls,
