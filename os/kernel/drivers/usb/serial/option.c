...
#define MEIG_PRODUCT_SLM322_1		0x4d13

/* UKL*/
#define UKL_VENDOR_M2		0x1782
#define UKL_PRODUCT_M2		0x5D22
#define UKL_VENDOR_M3		0x05C6
#define UKL_PRODUCT_M3		0x90B3
...
#define QUECTEL_PRODUCT_EC200			0x6026
#define QUECTEL_PRODUCT_EG95			0x0195
#define QUECTEL_PRODUCT_BG96			0x0296
#define QUECTEL_PRODUCT_EP06			0x0306
#define QUECTEL_PRODUCT_EC200S			0x6002
#define QUECTEL_PRODUCT_EG800Q			0x6007
#define QUECTEL_PRODUCT_EC800G			0x0904
...
static const struct usb_device_id option_ids[] = {
	...
	{ USB_DEVICE(0x19d1, 0x0001) }, //add air780e 4g module
	{ USB_DEVICE(0x2ECC, 0x3010) }, //add se32 4g module

	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EC200) },  /* add ec200t 4g module */
	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EC25) }, //add ec25 4g module
	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EC200S) }, //add ec200s 4g module
	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EC800G) }, //add ec800g-cn 4g module
	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EC800E) }, //add ec800e-cn 4g module
	{ USB_DEVICE(QUECTEL_VENDOR_ID, QUECTEL_PRODUCT_EG800Q) },

	{ USB_DEVICE(0x1782, 0x4d10) }, //add L610-CN 4g module
	{ USB_DEVICE(0x1782, 0x4d11) }, //add L610-CN 4g module
	{ USB_DEVICE(0x2CB7, 0x0A0A) }, //add MC661-CN 4g module
	{ USB_DEVICE(0x2CB7, 0x0A0B) }, //add MC661-CN 4g module
	{ USB_DEVICE(0x2CB7, 0x0A0C) }, //add MC661-CN 4g module
	{ USB_DEVICE(0x3500, 0x0001) }, //add NT26E 4g module
	{ USB_DEVICE(MEIG_VENDOR_ID, MEIG_PRODUCT_SLM320_1) }, //add SLM320P 4g module
	{ USB_DEVICE(MEIG_VENDOR_ID, MEIG_PRODUCT_SLM320_2) }, //add SLM320P 4g module
	{ USB_DEVICE(MEIG_VENDOR_ID, MEIG_PRODUCT_SLM320_3) }, //add SLM320P 4g module
	{ USB_DEVICE(MEIG_VENDOR_ID, MEIG_PRODUCT_SLM322_1) }, //add SLM322P 4g module
	{ USB_DEVICE(UKL_VENDOR_M2, UKL_PRODUCT_M2) }, //add UKL M2 4g module
	{ USB_DEVICE(UKL_VENDOR_M3, UKL_PRODUCT_M3) }, //add UKL M3 4g module
	//### add UMA501 4g module start ###
			...
			const struct usb_device_id *id)
{
	...
	if (serial->dev->descriptor.idVendor == cpu_to_le16(0x1286) &&
		serial->dev->descriptor.idProduct == cpu_to_le16(0x4e3c) &&
		serial->interface->cur_altsetting->desc.bInterfaceNumber <= 1) {
		printk(KERN_INFO "Discover <= 1th interface for UMA501\n");
		return -ENODEV;
	}
	...
	if (serial->dev->descriptor.idVendor == cpu_to_le16(0x1782) &&
		serial->dev->descriptor.idProduct == cpu_to_le16(0x4d11) &&
		serial->interface->cur_altsetting->desc.bInterfaceNumber <= 1) {
		return -ENODEV;
	}

	//quectel module probe add here by zhongxiaohui
	if (serial->dev->descriptor.idVendor == cpu_to_le16(QUECTEL_VENDOR_ID))
	{
		//1.check interface number range  (1 < number <= 3)
		if(serial->interface->cur_altsetting->desc.bInterfaceNumber <= 1
			&& (cpu_to_le16(QUECTEL_PRODUCT_EC200) == serial->dev->descriptor.idProduct
				|| cpu_to_le16(QUECTEL_PRODUCT_EC25) == serial->dev->descriptor.idProduct
				|| cpu_to_le16(QUECTEL_PRODUCT_EC200S) == serial->dev->descriptor.idProduct
				|| cpu_to_le16(QUECTEL_PRODUCT_EC800G) == serial->dev->descriptor.idProduct
				|| cpu_to_le16(QUECTEL_PRODUCT_EC800E) == serial->dev->descriptor.idProduct
				|| cpu_to_le16(QUECTEL_PRODUCT_EG800Q) == serial->dev->descriptor.idProduct
				)) {
			return -ENODEV;
		}
		...
		//2.check interface class invalid
		intf = &serial->interface->cur_altsetting->desc;
		if (intf->bInterfaceClass != 0xFF || intf->bInterfaceSubClass == 0x42) {
			//ECM, RNDIS, NCM, MBIM, ACM, UAC, ADB
			return -ENODEV;
		}
		//3.check qmi interface number??Endpoints??subcalss??Protocol
		...
		pm_runtime_set_autosuspend_delay(&serial->dev->dev, 3000);//usb 3s???????Զ?????,???͹???
	...
	if (serial->dev->descriptor.idVendor == cpu_to_le16(0x2ecc) &&
		serial->dev->descriptor.idProduct == cpu_to_le16(0x3010) &&
		serial->interface->cur_altsetting->desc.bInterfaceNumber <= 1) {
		return -ENODEV;
	}
