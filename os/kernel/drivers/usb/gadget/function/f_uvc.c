...
static struct usb_interface_descriptor uvc_streaming_intf_alt0 = {
	...
	.bAlternateSetting	= 0,
#ifndef CONFIG_USB_F_AK_UVC_BULK
	.bNumEndpoints		= 0,
#else
	.bNumEndpoints		= 1,
#endif
...
#ifdef CONFIG_USB_F_AK_UVC_BULK
    if (((ctrl->bRequestType & USB_RECIP_MASK) == USB_RECIP_INTERFACE)
            && ((ctrl->wValue >> 8) == UVC_VS_COMMIT_CONTROL)
            && (ctrl->bRequest == UVC_SET_CUR)) {
        int ret;

        if (uvc->state != UVC_STATE_CONNECTED) {
            INFO(f->config->cdev, "uvc state %d is not "
                    "UVC_STATE_CONNECTED\n", uvc->state);
        }

        if (!uvc->video.ep) {
            INFO(f->config->cdev, "uvc streaming ep is null\n");
        }

        INFO(f->config->cdev, "reset UVC\n");
        usb_ep_disable(uvc->video.ep);

        ret = config_ep_by_speed(f->config->cdev->gadget,
                &(uvc->func), uvc->video.ep);
        if (ret) {
            INFO(f->config->cdev, "config ep failed %d\n", ret);
        }
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	uvc->video.ep = ep;
    INFO(cdev, "uvc_streaming_ep %s\n", ep->name);
#ifdef CONFIG_USB_F_AK_UVC_BULK
    uvc->video.max_payload_size = max_packet_size;
    INFO(cdev, "max_packet_size %d\n", uvc->video.max_payload_size);
#endif
