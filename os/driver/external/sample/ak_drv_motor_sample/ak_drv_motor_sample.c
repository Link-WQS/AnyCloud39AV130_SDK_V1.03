...
/*
 * long_options:ALL optional_argument
 * example :
 *devno  'n'
 *current 'c'
 *speed 's'
...
/*
 * option_hint:ALL optional_argument hint
 *
 * example :
 *"马达的设备号 从0开始 依具体设备而定",
 *"当前的位置",
...
static void usage(const char *app)
{
    ...
    ak_print_normal("  %s --devno 0 --current=0 --speed=400 "\
          "--step=4096 --total=3600 --boundary=50 --cmd 7\n",app);
    ak_print_normal("\n");
}
/*initialize devno*/
...
/*
 * @BRIEF       test ak_motor drivers app entry
 * @PARAM[in]   argc:input command line parameter count
 * @PARAM[in]   argv:input **argv:command line parameter address pointer
 * @RETURN          int
...
int main (int argc, char **argv)
{
    ...
    if ((cmd == CMD_MOTOR_MOVE_LIMIT) || (cmd == CMD_MOTOR_MOVE_NOLIMIT)
            || (cmd == CMD_MOTOR_BOUNDARY)) {
