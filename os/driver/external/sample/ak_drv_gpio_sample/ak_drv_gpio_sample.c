...
static void usage(const char *app)
{
    ...
    for (i = 0; i < opt_num; i++) {
        ak_print_normal("  -%c, --%s  %s\n", long_options[i].val,
                long_options[i].name, option_hint[i]);
    }
    ak_print_normal("\n");
    ak_print_normal("example:\n");
    ak_print_normal("  %s -r -n 79 -d -v -c -p -e -i -s\n", app);
    ak_print_normal("  %s --read --gpio_no 79 --direction --value \
            --drive --pull_polarity --pull_enable \
            --input_enable --slew_rate\n", app);
    ak_print_normal("  %s -w -n 79 -dout -v1 -c3 -ppullup \
            -e1 -i0 -sslow\n", app);
    ak_print_normal("  %s --write --gpio_no 79 --direction=out \
            --value=1 --drive=3 --pull_polarity=pullup --pull_enable=1 \
            --input_enable=0 --slew_rate=slow\n", app);
}
...
/*
    read 'r' 读操作",
    write 'w' 写操作",
    gpio_no 'n' GPIO pin脚的标识号 从0开始",
    direction 'd'"GPIO pin脚的方向 out表示输入 in表示输出",
    value 'v'"GPIO pin脚的输出值 0标识高电平 1表示低电平",
    drive 'c'"GPIO pin脚的驱动能力 可选0-3共4档",
    pull_polarity 'p'"GPIO pin脚的上下拉配置 pullup标识上拉 pulldown表示下拉",
    pull_enable 'e'"GPIO pin脚的上下拉使能 1表示",
    input_enable 'i'"GPIO pin脚的输入使能 1表示使能输入 0表示关闭输入",
    slew_rate 's'"GPIO pin脚的slew rate能力 fast表示使能 slow表示关闭",
    help 'h'"显示帮助信息",
*/
void ak_set_gpio_value(){
    char value[FILE_PATH_MAX] = {0};
    if (op_mode == OP_MODE_READ) {
        ...
        */
        if (gpio_pull_polarity == OPT_FIND) {
            memset(value, 0, FILE_PATH_MAX);
            if (!ak_drv_gpio_read(gpio_no, "pull_polarity",
                value, FILE_PATH_MAX))
        ...
        */
        if (gpio_pull_enable == OPT_FIND) {
            memset(value, 0, FILE_PATH_MAX);
            if (!ak_drv_gpio_read(gpio_no, "pull_enable",
                value, FILE_PATH_MAX))
        ...
        */
        if (gpio_input_enable == OPT_FIND) {
            memset(value, 0, FILE_PATH_MAX);
            if (!ak_drv_gpio_read(gpio_no, "input_enable",
                value, FILE_PATH_MAX))
        ...
        */
        if (gpio_pull_polarity != DEFULAT_CFG_VALUE) {
            if (gpio_pull_polarity == GPIO_PULL_UP) {
                ak_drv_gpio_write(gpio_no, "pull_polarity",
                    "pullup", strlen("pullup"));
            } else {
                ak_drv_gpio_write(gpio_no, "pull_polarity",
                    "pulldown", strlen("pulldown"));
            }
        }
        /*
        *pull_enable 'e'"GPIO pin脚的上下拉使能 1表示",
...
int main (int argc, char **argv)
{
    ...
    ak_print_notice("\n!!!!%s PIN_%d CFG:dir %d val %d drv %d %s \
            pull_en %d input_en %d slew_rate %d successfully!!!!\n\n",
        (op_mode == OP_MODE_READ) ? "read" : "write", gpio_no, gpio_direction,
        gpio_value, gpio_drive, (gpio_pull_polarity == GPIO_PULL_UP) ?
            "pullup" : "pulldown",
