...
static void usage(const char *app)
{
    ...
    for (i = 0; i < opt_num; i++) {
        ak_print_normal("  -%c, --%-8s  %s\n", long_options[i].val,
            long_options[i].name, option_describe[i]);
    }
    ak_print_normal("\n");
    ...
    while ((opt_val = getopt_long(argc, argv, short_option,
        ...
        switch (opt_val) {
            ...
            case 'w':
                ...
                if(retval) ak_print_error("将系统时间同步到RTC出错, \
                    请先尝试使用date命令设置系统时间!\n");
                break;
            case 'r':
                retval = ak_drv_sync_from_rtc();
                if(retval) ak_print_error("将RTC时间同步到系统出错, \
                    请先尝试将系统时间同步到RTC!\n");
                break;
            case 'u':
                /*
                 * config rtc alarm wakeup time
                 */
                if(strrchr(optarg, ':')) {
