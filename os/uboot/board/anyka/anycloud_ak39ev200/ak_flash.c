...
unsigned long flash_init (void)
{
    ...
    for (i=0; i<fli->sector_count; ++i) {
        fli->start[i] = addr;
        // cdh notes:use erase size , not sector size
...
int spl_device_parse(const char *const mtd_dev, ulong flash_size)
{
    ...
    while (p && (*p != '\0') && (*p != ';')) {
        ...
        if(!strncmp(part->name, PART_APP_NAME, 3)){
            env_set_hex("app_offset", part->offset);
            env_set_hex("app_size", part->size);
            debug("mtd cfg app offset&size ok!\n");
        }
