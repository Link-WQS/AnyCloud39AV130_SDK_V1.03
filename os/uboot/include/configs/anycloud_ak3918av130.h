...
#define PART_FASTSYS_NAME   "FASTSYS"
#define PART_PARAM_NAME     "ARGS"
#define PART_ISPCONF_NAME   "ISPCONF"
...
#else
#define  MTDIDS_DEFAULT  "nor0=spi-nor"
#endif
...
/*"mtdparts=mtdparts=spi-nor:464K(UBOOT),4K(ENV),4K(ENVBK),64K(DTB),2M(KERNEL),2M(ROOTFS),300K(CONFIG),-(APP)\0" \*/
/*"mtdparts=mtdparts=spi-nor:400K@0x0(UBOOT),4K@0x64000(ENV),64K@0x65000(DTB),2048K@0x75000(KERNEL)\0" \*/
...
#define CONFIG_EXTRA_ENV_SETTINGS \
    ...
    "kernel_offset=0x5d000\0" \
    "kernel_size=0x1a9000\0" \
    "kernel_addr=0x0\0" \
    "dtb_offset=0x4d000\0" \
    "dtb_size=0xe3db\0" \
    "dtb_addr=0x0\0" \
    "fdt_high=0xFFFFFFFF\0" \
    "update_flag=0x0\0" \
    ...
    "mem=mem=64M\0" \
    "memsize=memsize=64M\0" \
    "mtdparts=mtdparts=spi-nor:300K@0x0(UBOOT),4K@0x4b000(ENV),4K@0x4c000(ENVBK),64K@0x4d000(DTB),1700K@0x5d000(KERNEL),2000K@0x1f7000(ROOTFS),400K@0x3eb000(CONFIG),2500K@0x45e000(USR),1156K@0x6cf000(APP),64K@0x7f0000(DATA)\0" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
    ...
    "mtdparts=mtdparts=spi-nor:256K@0x0(UBOOT),4K@0x40000(ENV),4K@0x41000(ENVBK),64K@0x42000(DTB),1600K@0x52000(KERNEL),2200K@0x1E2000(ROOTFS),400K@0x408000(CONFIG),3664K@0x46C000(APP),2196K@0x1E3000(ROOTFS_MOUNT)\0" \
    "bootargs=console=ttySAK0,115200n8 root=/dev/mtdblock5 rootfstype=squashfs init=/sbin/init \0" \
    "setcmd=setenv bootargs console=${console} root=${mtd_root} rootfstype=${rootfstype} init=${init} mem=${memsize}\0" \
    "read_kernel=sf probe 0:0 ${sf_hz} 0; sf read ${loadaddr} ${kernel_offset} ${kernel_size}\0" \
    "read_dtb=sf probe 0:0 ${sf_hz} 0; sf read ${fdtcontroladdr} ${dtb_offset} ${dtb_size}\0" \
