 ...
 * on error.
 */
uint32_t aes_get_key_len(char *command)
{
	uint32_t key_len = AES128_KEY_LENGTH;
...
static int do_aes(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (enc)
		aes_cbc_encrypt_blocks(key_len,key_exp, iv_ptr, src_ptr, dst_ptr,
				       aes_blocks);
	else
		aes_cbc_decrypt_blocks(key_len,key_exp, iv_ptr, src_ptr, dst_ptr,
				       aes_blocks);
