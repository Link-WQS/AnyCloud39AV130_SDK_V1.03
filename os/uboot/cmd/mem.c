...
static int do_mem_md(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (addr > (CONFIG_SYS_SDRAM_BASE + gd->ram_size - PHYS_SYS_SDRAM_PROTECT_SIZE)){
...
static int do_mem_cmp(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	*/
	...
	addr1 += base_address;
	/* add check addr whether beyond UBOOT mem eare range, protect UBOOT memory */
	if (addr1 > (CONFIG_SYS_SDRAM_BASE + gd->ram_size - PHYS_SYS_SDRAM_PROTECT_SIZE)){
		printf("** Err %s, address beyond uboot range **\n", __func__);
		return 1;
	}
	...
	addr2 += base_address;
	/* add check addr whether beyond UBOOT mem eare range, protect UBOOT memory */
	if (addr2 > (CONFIG_SYS_SDRAM_BASE + gd->ram_size - PHYS_SYS_SDRAM_PROTECT_SIZE)){
		printf("** Err %s, address beyond uboot range **\n", __func__);
		return 1;
	}
...
static int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	*/
	...
	if (addr2info(dest) != NULL) {
		...
		/* add check addr whether beyond UBOOT mem eare range, protect UBOOT memory */
		if (dest > (CONFIG_SYS_SDRAM_BASE + gd->ram_size - PHYS_SYS_SDRAM_PROTECT_SIZE)){
			printf("** Err %s, address beyond uboot range **\n", __func__);
			return 1;
		}
	}
		       ...
		       char * const argv[])
{
	...
	/* add check addr whether beyond UBOOT mem eare range, protect UBOOT memory */
	if (base_address > (CONFIG_SYS_SDRAM_BASE + gd->ram_size - PHYS_SYS_SDRAM_PROTECT_SIZE)){
		printf("** Err %s, address beyond uboot range **\n", __func__);
		return 1;
	}
