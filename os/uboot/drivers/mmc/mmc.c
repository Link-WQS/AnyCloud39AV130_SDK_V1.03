			   ...
			   lbaint_t blkcnt)
{
	...
	/* Anyka:Waiting for the ready status */
	if (mmc_poll_for_busy(mmc, 500))
		return -ETIMEDOUT;
...
static int mmc_startup(struct mmc *mmc)
{
	...
	// anyka add: cmd16 MMC_CMD_SET_BLOCKLEN
	cmd.cmdidx = MMC_CMD_SET_BLOCKLEN;
	cmd.resp_type = MMC_RSP_R1;
	...
	if (err) {
			pr_err("MMC_CMD_SET_BLOCKLEN Err!\n");
			return err;
	}else {
			pr_debug("MMC_CMD_SET_BLOCKLEN OK!\n");
	}
