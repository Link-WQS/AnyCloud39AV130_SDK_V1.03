...
static int ak_get_gate_ctrlbit(int id)
{
    ...
    switch (id) {
        ...
        case CLOCK_ID_GCLK_TWI0:
            ...
            break;
#if defined(CONFIG_KM01A_CODE) || defined(CONFIG_3918AV130_CODE)
        case CLOCK_ID_GCLK_FFT:
            ctrlbit = GCLK_FFT_CFG_SHIFT;
            break;
#else
...
#if defined(CONFIG_KM01A_CODE) || defined(CONFIG_3918AV130_CODE)
        case CLOCK_ID_GCLK_L2BUF:
            rstbit = GCLK_L2BUF_RST_CFG_SHIFT;
            break;
#else
        case CLOCK_ID_GCLK_L2BUF0:
...
static int ak_clock_reset(u32 shift)
{
    ...
    regval = __raw_readl((void __iomem*)CLOCK_RESET_CTRL);
    regval |= (0x1 << shift);
    __raw_writel(regval, (void __iomem*)CLOCK_RESET_CTRL);
    mdelay(1);
    regval = __raw_readl((void __iomem*)CLOCK_RESET_CTRL);
    regval &= ~(0x1 << shift);
    __raw_writel(regval, (void __iomem*)CLOCK_RESET_CTRL);
    ...
    switch (clk->id) {
        ...
        case CLOCK_ID_FACTOR_MAC_REFCLK:
#if defined(CONFIG_KM01A_CODE) || defined(CONFIG_3918AV130_CODE)
            {
                ...
                regval = __raw_readl(CLOCK_MAC_OPCLK_CTRL);
                regval &= ~((MAC_OPCLK_PLL_SEL_MASK << MAC_OPCLK_PLL_SEL_SHIFT) |
                            (MAC_OPCLK_DIV_NUM_CFG_MASK << MAC_OPCLK_DIV_NUM_CFG_SHIFT));
                regval |= (1 << MAC_OPCLK_PLL_SEL_SHIFT) | (factor << MAC_OPCLK_DIV_NUM_CFG_SHIFT);
                __raw_writel(regval, CLOCK_MAC_OPCLK_CTRL);

                do {
                    timeout--;
                    regval = ((__raw_readl(CLOCK_MAC_OPCLK_CTRL)) >> MAC_OPCLK_SOURCE_READY_SHIFT)
                        & MAC_OPCLK_SOURCE_READY_MASK;
                ...
                regval = __raw_readl(CLOCK_MAC_OPCLK_CTRL);
                regval |= (1 << MAC_OPCLK_SOURCE_EN_CFG_SHIFT);
                __raw_writel(regval, CLOCK_MAC_OPCLK_CTRL);

                do {
                    timeout--;
                    regval = ((__raw_readl(CLOCK_MAC_OPCLK_CTRL)) >> MAC_OPCLK_READ_SHIFT)
                        & MAC_OPCLK_READ_MASK;
                } while ((regval == 0) && (timeout > 0));
                ...
                regval = __raw_readl(CLOCK_MAC_OPCLK_CTRL);
                regval |= (MAC_OPCLK_EN_CFG_MASK
                        << MAC_OPCLK_EN_CFG_SHIFT);
                __raw_writel(regval, CLOCK_MAC_OPCLK_CTRL);
            }
#else
...
U_BOOT_DRIVER(ak3918av130_factor_clk) = {
    .name = "ak3918av130_factor_clk",
    .id = UCLASS_CLK,
...
static int ak_gate_clk_enable(struct clk* clk)
{
    ...
    /* enable */
    regval = __raw_readl((void __iomem*)CLOCK_GATE_CTRL);
    regval &= ~(1 << ctrlbit);
    __raw_writel(regval, (void __iomem*)CLOCK_GATE_CTRL);
...
U_BOOT_DRIVER(ak3918av130_gate_clk) = {
    .name = "ak3918av130_gate_clk",
    .id = UCLASS_CLK,
...
static int ak_fixed_clk_probe(struct udevice* dev)
{
    ...
    __raw_writel(regval, (void __iomem*)CLOCK_PLL2_CTRL);

    /* bandwidth */
    regval = __raw_readl((void __iomem*)CLOCK_PLL1_PLL2_BANDWIDTH);
    regval &= ~0xFFF;
    regval |= param_nb - 1;
    __raw_writel(regval, (void __iomem*)CLOCK_PLL1_PLL2_BANDWIDTH);

    /* Step 3 */
    regval = __raw_readl((void __iomem*)CLOCK_PLL1_CTRL);
    regval |= ((0x1) << PLL2_FREQ_ADJ_CFG_SHIFT);
    __raw_writel(regval, (void __iomem*)CLOCK_PLL1_CTRL);
...
U_BOOT_DRIVER(ak3918av130_fixed_clk) = {
    .name = "ak3918av130_fixed_clk",
    .id = UCLASS_CLK,
