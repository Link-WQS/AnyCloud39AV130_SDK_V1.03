...
static int genphy_config_advert(struct phy_device *phydev)
{
	...
	if (adv != oldadv) {
		err = 0; // must modify here, phy_write(phydev, MDIO_DEVAD_NONE, MII_ADVERTISE, adv);
...
int genphy_update_link(struct phy_device *phydev)
{
	...
	if ((phydev->autoneg == AUTONEG_ENABLE) &&
		...
		while (!(mii_reg & BMSR_ANEGCOMPLETE) && !(mii_reg & BMSR_LSTATUS)){
			/*
			 * Timeout reached ?
			 */
...
#ifdef CONFIG_PHY_SZ18201
	phy_sr_sz18201_init();
#endif

#ifdef CONFIG_PHY_KSZ8041
	phy_sr_ksz8041_init();
#endif

#ifdef CONFIG_PHY_JL1101
	phy_sr_jl1101_init();
#endif
