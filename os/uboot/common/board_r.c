...
extern int read_partition_data(char *partition_name, unsigned long base_addr);
...
static int initr_flash(void)
{
	...
	if (board_flash_wp_on())
		...
		flash_size = flash_init();  //check ok
...
static int initr_env(void)
{
	...
	if (should_load_env())
	{
		...
		printf("env_set_default\r\n");
		env_set_default(NULL, 0);
	}
#if 0 //def CONFIG_OF_CONTROL
	env_set_hex("fdtcontroladdr",
		    (unsigned long)map_to_sysmem(gd->fdt_blob));
	printf("fdtcontroladdr gd->fdt_blob:%d\r\n", gd->fdt_blob);

#endif
#ifdef CONFIG_ANYKA
	/* Initialize from environment */
	load_addr = env_get_ulong("loadaddr", 16, load_addr);
#ifndef CONFIG_Z1_H322D
	{
		const char *p = NULL;
		p = env_get("mtdparts");
		part_num = spl_device_parse(p, gd->bd->bi_flashsize);
	}
#endif	
#ifdef CONFIG_DM_VIDEO
	/*read logo*/
	read_partition_data(PART_LOGO_NAME, gd->video_bottom);
#endif
