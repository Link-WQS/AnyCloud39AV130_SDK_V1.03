		 ...
		 uint unc_len, ulong *load_end)
{
	...
	*load_end = load;
	...
	switch (comp) {
	case IH_COMP_NONE:
		if (g_blob_start == load || load == image_start)
			break;
...
/**
 * boot_get_filesystem - main filesystem handling routine
 * @argc: command argument count
 * @argv: command argument list
 * @images: pointer to the bootm images structure
 * @arch: expected filesystem architecture
 * @file_start: pointer to a ulong variable, will hold filesystem start address
 * @file_end: pointer to a ulong variable, will hold filesystem end
 *
 * returns:
 *     0, if filesystem was found and valid, or skiped
 *     file_start and file_end are set to 0 if
 *     filesystem is found and valid
 *
 *     1, if filesystem image is found but corrupted, or invalid
 *     file_start and file_end are set to 0 if no filesystem exists
 */
int boot_get_filesystem(int argc, char * const argv[], bootm_headers_t *images,
		uint8_t arch, ulong *file_start, ulong *file_end)
{
#if IMAGE_ENABLE_FIT
	ulong rd_addr, rd_data, rd_len;
	void *buf;
	const char	*fit_uname_config = images->fit_uname_cfg;
	const char	*fit_uname_filesystem = NULL;
	int		rd_noffset;
	...
	{
		debug("## Skipping init filesystem\n");
		return 0;
	}

	rd_addr = (select || genimg_has_config(images)) ? simple_strtoul(select, NULL, 16) : map_to_sysmem(images->fit_hdr_os);
	/* use FIT configuration provided in first bootm
	 * command argument. If the property is not defined,
	 * quit silently.
	 */	
	rd_noffset = fit_get_node_from_config(images,
			FIT_FILESYSTEM_PROP, rd_addr);
	...
	/*
	 * Check if there is an filesystem image at the
	...
	default:
		{
			puts("Wrong filesystem Image Format\n");
			rd_data = rd_len =  0;
			return 1;
		}
	}	 
