...
#if IMAGE_ENABLE_FIT
		/*just for verify filesystem image except ramdisk*/
		ret = boot_get_filesystem(argc, argv, &images, IH_INITRD_ARCH,
					   &images.rd_start, &images.rd_end);
		if (ret) {
			puts("filesystem image is corrupt or invalid\n");
			return 1;
		}
#endif
				   ...
				   ulong *os_data, ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_FIT:
		...
		if (simple_strtoul(env_get("rtos_sercure_boot"), NULL, 16)) {
			*os_data += image_get_header_size();  //rtos secureboot need to add image head offset
		}
		break;
#endif
#ifdef CONFIG_ANDROID_BOOT_IMAGE
