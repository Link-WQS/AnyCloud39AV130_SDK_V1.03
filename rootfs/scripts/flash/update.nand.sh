...
check_files()
{
    ...
    for target in ${FILE_SPL} ${FILE_ENV} ${FILE_DTB} ${FILE_KERNEL} ${FILE_ROOT} ${FILE_ETC} ${FILE_USR}
    do
...
{
    for target in ${FILE_SPL} ${FILE_ENV} ${FILE_DTB} ${FILE_KERNEL} ${FILE_ROOT} ${FILE_ETC} ${FILE_USR} ${FILE_TAR_GZ}
    do
...
MTD_UPDATE=0
nand_update $FILE_SPL    $PARTITION_SPL    --no-oob
