...
FILE_ALL_MD5="burnfile.md5"                                                     #使用全局md5的文件名称
CHECK_MD5=1                                                                     #是否检查md5
...
CMD_SYNC=/bin/sync
CMD_KILL=/bin/kill
CMD_UMONT=/bin/umount
CMD_WATCH=/bin/watch
CMD_GETTY=/sbin/getty
CMD_SLEEP=/bin/sleep
CMD_REBOOT="/sbin/reboot -f"

FILE_UBOOT="u-boot-spl.bin"
FILE_ENV="env.img"
FILE_DTB="cloudOS.dtb"
FILE_KERNEL="uImage_fast"
FILE_ROOT="initfs.img"
FILE_ETC="config.jffs2"
FILE_ISPCONF="isp.part"
FILE_APP2="rootfs.ram"
FILE_ARGS="dss.img"
FILE_FASTSYS="fastsys_uImage"

PARTITION_UBOOT="UBOOT"
PARTITION_ENV="ENV"
PARTITION_ENVBAK="ENVBK"
PARTITION_DTB="DTB"
PARTITION_KERNEL="KERNEL"
PARTITION_ROOT="ROOTFS"
PARTITION_ETC="CONFIG"
PARTITION_ISPCONF="ISPCONF"
PARTITION_APP2="APP2"
PARTITION_ARGS="ARGS"
PARTITION_FASTSYS="FASTSYS"
...
check_mtd_md5( )                                                                #检查分区的md5是否正确,参数:分区名 长度 md5数据
{
    if [ $3 != "" ]; then
        FILE_MD5=`$CMD_HEAD -c $2 $1 | $CMD_MD5SUM | $CMD_GREP -Eo "^[0-9a-f]+"`
        if [ "$FILE_MD5" == "$3" ]; then
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_GREEN";"$CB_BLACK"m""$1 $FILE_MD5 MD5 CORRECT.""\e[0m"
            return 1
        else
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1 $FILE_MD5 $3 MD5 ERROR.""\e[0m"
            return 0
        fi
    else
        return 1
...
check_file_md5( )                                                               #检查文件的md5是否正确,参数:目录 升级文件名称
{
    if [ -e $1/$2.md5 ]; then
        FILE_MD5=`$CMD_MD5SUM $1/$2 | $CMD_GREP -Eo "^[0-9a-f]+"`
        FILE_SIGN=`$CMD_CAT $1/$2.md5 | $CMD_GREP -Eo "$FILE_MD5"`
        if [ "$FILE_SIGN" != "" ]; then
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_GREEN";"$CB_BLACK"m""$1/$2 $FILE_MD5 MD5 CORRECT.""\e[0m"
            return 1
        else
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_SIGN MD5 ERROR.""\e[0m"
            return 0
        fi
    elif [ -e $1/$FILE_ALL_MD5 ]; then
        FILE_MD5=`$CMD_MD5SUM $1/$2 | $CMD_GREP -Eo "^[0-9a-f]+"`
        FILE_SIGN=`$CMD_CAT $1/$FILE_ALL_MD5 | $CMD_GREP -Eo "$FILE_MD5"`
        if [ "$FILE_SIGN" != "" ]; then
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_GREEN";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_ALL_MD5 MD5 CORRECT.""\e[0m"
            return 1
        else
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_ALL_MD5 MD5 ERROR.""\e[0m"
            return 0
        fi
    else                                                                        #没有找到md5
        $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 Can't find $FILE_ALL_MD5 or $2.md5.""\e[0m"
        return 0
    fi
}

check_file_md5_first( )                                                          #检查文件的md5是否正确,参数:目录 升级文件名称
{
    if [ -e $1/$2.md5 ]; then
        FILE_MD5=`md5sum $1/$2 | grep -Eo "^[0-9a-f]+"`
        FILE_SIGN=`cat $1/$2.md5 | grep -Eo "$FILE_MD5"`
        if [ "$FILE_SIGN" != "" ]; then
            echo -e "\e["$CM_NORMAL";"$CF_GREEN";"$CB_BLACK"m""$1/$2 $FILE_MD5 MD5 CORRECT.""\e[0m"
            return 1
        else
            echo -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_SIGN MD5 ERROR.""\e[0m"
            return 0
        fi
    elif [ -e $1/$FILE_ALL_MD5 ]; then
        FILE_MD5=`md5sum $1/$2 | grep -Eo "^[0-9a-f]+"`
        FILE_SIGN=`cat $1/$FILE_ALL_MD5 | grep -Eo "$FILE_MD5"`
        if [ "$FILE_SIGN" != "" ]; then
            echo -e "\e["$CM_NORMAL";"$CF_GREEN";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_ALL_MD5 MD5 CORRECT.""\e[0m"
            return 1
        else
            echo -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 $FILE_MD5 $FILE_ALL_MD5 MD5 ERROR.""\e[0m"
            return 0
        fi
    else                                                                        #没有找到md5
        echo -e "\e["$CM_NORMAL";"$CF_RED";"$CB_BLACK"m""$1/$2 Can't find $FILE_ALL_MD5 or $2.md5.""\e[0m"
        return 0
    fi
}
...
check_files()
{
    ...
    FILE_EXIST=0
    for target in ${FILE_UBOOT} ${FILE_ENV} ${FILE_DTB} ${FILE_KERNEL} ${FILE_ROOT} ${FILE_ETC} ${FILE_ISPCONF} ${FILE_APP2} ${FILE_ARGS} ${FILE_FASTSYS}
    do
        if [ -e ${DIR_UPDATE}/${target} ]; then
            FILE_EXIST=1
            if [ $CHECK_MD5 -ne 0 ]; then
                check_file_md5_first ${DIR_UPDATE} ${target}
                if [ "$?" = 0 ];then                                            #md5校验出错
                    exit 0
                fi
            fi
...
{
    for target in ${FILE_UBOOT} ${FILE_ENV} ${FILE_DTB} ${FILE_KERNEL} ${FILE_ROOT} ${FILE_ETC} ${FILE_ISPCONF} ${FILE_APP2} ${FILE_ARGS} ${FILE_FASTSYS} ${FILE_TAR_GZ}
    do
        ...
        if [ -e ${DIR_TMP}/${target}.md5 ]; then
            echo "MOVE ${target}.md5 => ${DIR_RESERVE}"
            mv ${DIR_TMP}/${target}.md5 ${DIR_RESERVE}/
        fi
    done
    if [ -e ${DIR_TMP}/${FILE_ALL_MD5} ]; then
        mv ${DIR_TMP}/${FILE_ALL_MD5} ${DIR_RESERVE}/
    fi
    return 0
}
...
dd_update()                                                                     #dd_update [文件名称] [分区名称]
{
    if [ -e ${DIR_UPDATE}/${1} ]; then
        ...
        if [ -z $NUM_MTD ]; then                                                #判断分区mtd id是否存在
            ...
            $CMD_ECHO
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_BLUE";"$CB_BLACK"m""*******************************""\e[0m"
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_BLUE";"$CB_BLACK"m""Update ${2}""\e[0m"
            $CMD_ECHO -e "\e["$CM_NORMAL";"$CF_BLUE";"$CB_BLACK"m""*******************************""\e[0m"

            if [ $CHECK_MD5 -ne 0 ]; then
                FILE_MD5=""
                check_file_md5 ${DIR_UPDATE} $1
                if [ "$?" = 0 ];then                                                #md5校验出错
                    exit 0
                fi
            fi
            ...
            if [ $DEC_SIZE_FILE -gt $DEC_SIZE_PARTITION ]; then                 #文件大于分区
                ...
                $CMD_ECHO  3 >/proc/sys/vm/drop_caches
                $CMD_FREE
                #$CMD_TOP -n1 -bmH
                ...
                $CMD_ECHO  3 >/proc/sys/vm/drop_caches
                if [ $CHECK_MD5 -ne 0 ]; then
                    check_mtd_md5 /dev/mtdblock$NUM_MTD $DEC_SIZE_FILE $FILE_MD5         #检测升级后写入mtd内容是否正确
                    if [ "$?" = 0 ];then                                                 #md5校验出错
                        exit 0
                    fi
                fi
...
{
    ps
    PID=`ps  | grep -E $1 | grep -v "grep" | grep -v "update.sh" | awk '{ print $1}'`
    for i in $PID
    do
...
do
    ...
    ?)
        echo "$0"
        echo "Option: -n 'No check md5.(在升级的时候不检查md5)'"
...
if [ "$?" = 0 ];then                                                            #没有busybox tar包,则使用root分区创建临时根目录分区
    ...
    if [ -L /usr/bin/bc ]; then
        ln -svf /bin/busybox $DIR_TMP/bc
    else
        cp /usr/bin/bc $DIR_TMP
    fi
    if [ -L /usr/bin/awk ]; then
        ln -svf /bin/busybox $DIR_TMP/awk
    else
        cp /usr/bin/awk $DIR_TMP
    fi
    if [ -L /usr/bin/expr ]; then
        ln -svf /bin/busybox $DIR_TMP/expr
    else
        cp /usr/bin/expr $DIR_TMP
    fi
    if [ -L /usr/bin/md5sum ]; then
        ln -svf /bin/busybox $DIR_TMP/md5sum
    else
        cp /usr/bin/md5sum $DIR_TMP
    fi
    if [ -L /usr/bin/free ]; then
        ln -svf /bin/busybox $DIR_TMP/free
    else
        cp /usr/bin/free $DIR_TMP
    fi
    if [ -L /usr/bin/top ]; then
        ln -svf /bin/busybox $DIR_TMP/top
    else
        cp /usr/bin/top $DIR_TMP
    fi
    if [ -L /usr/bin/head ]; then
        ln -svf /bin/busybox $DIR_TMP/head
    else
        cp /usr/bin/head $DIR_TMP
    fi
    CMD_AWK=$DIR_TMP/awk
    CMD_BC=$DIR_TMP/bc
    CMD_EXPR=$DIR_TMP/expr
    CMD_MD5SUM=$DIR_TMP/md5sum
    CMD_FREE=$DIR_TMP/free
    CMD_TOP=$DIR_TMP/top
    CMD_HEAD=$DIR_TMP/head
else
    ...
    CMD_FREE=$DIR_ROOT_TMP/usr/bin/free
    CMD_TOP=$DIR_ROOT_TMP/usr/bin/top
    CMD_HEAD=$DIR_ROOT_TMP/usr/bin/head
fi

export LD_LIBRARY_PATH="$DIR_ROOT_TMP/lib"
export PATH="/bin:/sbin:/usr/bin:/usr/sbin:$DIR_TMP"
export
...
dd_update $FILE_ENV     $PARTITION_ENVBAK
...
dd_update $FILE_ISPCONF $PARTITION_ISPCONF
dd_update $FILE_APP2    $PARTITION_APP2
dd_update $FILE_ARGS    $PARTITION_ARGS
dd_update $FILE_FASTSYS $PARTITION_FASTSYS
