...
mount_reserve_dir()
{
    HEXDUMP_ADDR=`cat /proc/ion | grep -E "^base-address" | grep -Eo '[0-9A-Fa-f]+$' | awk '{print toupper($0)}'`
    HEXDUMP_SIZE=`cat /proc/ion | grep -E "^size" | grep -Eo '[0-9A-Fa-f]+$' | awk '{print toupper($0)}'`
    ...
    #mount -t vfat /dev/reserved_ram0 ${DIR_RESERVE}
    df
    mount
}
mount_reserve_dir